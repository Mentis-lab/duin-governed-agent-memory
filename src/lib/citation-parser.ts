...
//     sourceMap.
//
// STATUS: PARKED — this parser, CitationChip and SourcePreviewPane are all
// unit-tested but have NO production caller. The wire they need does not exist:
// `Message` (src/lib/types.ts) has no sourceMap field, and ipc/chat.ts reads
// only `context.block` off augmentForChat, dropping the sourceMap that
// rag/context-builder.ts returns. Because both ends looked finished, the gap
// only ever showed up as literal `[1]` text in the transcript. The prompt no
// longer asks for `[N]` markers (see context-builder.ts), so nothing generates
// input for this parser today. Wiring order if R12 is picked back up:
// sourceMap → chat:done payload → Message.citationSources → MessageBubble/
// MarkdownRenderer segment splice → CitationChip.onOpen → SourcePreviewPane.
