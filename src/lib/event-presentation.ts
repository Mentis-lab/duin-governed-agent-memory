...
const TYPE_LABELS: Record<EventType, string> = {
  ...
  'persistence.backup_rejected': 'DB backup rejected',
...
export function eventSubtitle(event: EventRecord, maxChars = 120): string | null {
  ...
  switch (event.type) {
    ...
    case 'persistence.backup_rejected': {
      // The refusal reason is the whole point of this event — surface the
      // verification error, not the (always-'periodic') trigger reason.
      const error = typeof p.error === 'string' ? p.error : undefined
      s = error ?? 'fresh copy failed verification; previous backup kept'
      break
    }
