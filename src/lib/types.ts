...
/** Renderer mirror of a proposed-edit CARD row. Kept 1:1 with
 *  electron/services/proposed-edit-store.ts `ProposedEditProposal`. A card is
 *  a reviewable / reversible EDIT to existing workspace files that the user
 *  applies (atomically) or discards — the non-coder counterpart to a raw
 *  apply_patch. Persisted server-side, so it survives reload / AFK. */
...
export interface ProposedEditProposal {
  ...
  /** Absolute workspace root the card was reviewed against. The card's file
   *  paths are relative to THIS root; applying it anywhere else is refused.
   *  null on rows written before the binding shipped. */
...
export interface Skill {
  ...
  pluginId?: string
  /** True when seeded from the shipped bundle (per skill-loader's seed
   *  manifest). Editing one forks it: the operator's bytes win from then on. */
  bundled?: boolean
}
...
/** Provenance vocabulary — IMPORTED, not mirrored.
 *
 *  This file used to redefine MemorySource and MEMORY_SOURCE_LABELS with a comment reading
 *  "Mirrors MemorySource in electron/services/memory-frontmatter.ts... Keep the two in step."
 *  That is one concept with two definitions, kept in agreement by asking a human to remember —
 *  the exact shape constitution property 1 forbids. The owner is dependency-free precisely so the
 *  renderer can import it, and `src/lib/canvas-flow.ts` is the precedent for reaching into
 *  `electron/` for a shared contract. Drift is now structurally impossible rather than discouraged. */
export {
  type MemorySource,
  MEMORY_SOURCES,
  MEMORY_SOURCE_LABELS,
  isMemorySource
} from '../../electron/shared/memory-source'
import type { MemorySource } from '../../electron/shared/memory-source'
...
export interface MemoryFile {
  ...
  description: string
  type: MemoryType
  /** Absent on rows written before provenance existed — treat as 'unknown'. */
  source?: MemorySource
  body: string
  filePath: string
  ...
  createdAt: number
  updatedAt: number
  /** Set when a create landed on a taken slug and was disambiguated (see memory-store). */
...
/** What a capability needs present before it can run. Renderer mirror of
 *  `electron/services/capability-requires.ts`. */
export type Requirement =
  | { kind: 'binary'; name: string; hint?: string }
  | { kind: 'file'; path: string; hint?: string }
  | { kind: 'env'; name: string; hint?: string }

/** One probed requirement. `detail` is present only when unsatisfied and is the
 *  sentence to show the operator. A satisfied `env` result carries no value — only
 *  that it was set. */
...
export interface McpServerConfig {
  id: string
  name: string
  // 'http' = Streamable HTTP (supersedes SSE). Enum mirrors the electron side.
  ...
  enabled: boolean
  // 'unavailable' is distinct from 'error' on purpose: the connector's runtime is
  // absent, so it was never installable here. Nothing was spawned and retrying
  // cannot help until the operator installs what `missing` names.
  ...
  pluginId?: string
  /** Declared prerequisites — a binary on PATH, a file, an env var. */
  requires?: Requirement[]
  /** Probe results for `requires`, stamped by the manager on list. Only the
   *  failures; empty or absent means nothing is missing. */
  missing?: RequirementResult[]
}

// MR — MCP Resources surface (renderer mirror of the electron types).
export interface McpResource {
  ...
  /** Server provenance stamped by the manager/tool pack. */
  server?: string
}
...
// SOURCE-LOCK: this renderer mirror MUST stay member-identical to the
// main-process ProviderId union in electron/services/providers/registry.ts.
// The renderer can't import electron code, so the mirror is hand-maintained;
// provider-parity.test.ts fails the build if the two unions ever drift.
...
export interface ProviderInfo {
  ...
  docsUrl: string
  hasKey?: boolean
  /** Whether a key for this provider can reach any model today (catalog entry,
   *  custom model, or keyless ollama). False = a valid key still routes nothing
   *  until models are imported — key-entry surfaces must say so. */
  routable?: boolean
}
...
export interface ModelInfo {
  ...
  tier?: ModelTier
  description?: string
  /** True for shipped MODEL_CATALOG entries; absent/false for hand-added or
   *  imported models. ModelSettings derives its custom-models list from this. */
  builtin?: boolean
  /** Internal routing target (e.g. the DUIN brain), not a user-pickable model.
   *  The model picker should hide these; they remain valid as the default. */
  internal?: boolean
}
...
/** Brain graph color scheme id. Mirrors BrainGraphSchemeId in
 *  `src/duin/lib/graph-schemes.ts` (the renderer source of truth for the
 *  palettes themselves); kept as a string-literal union here so AppSettings
 *  stays self-contained without a cross-module import. */
...
export interface AppSettings {
   ...
   *  palette. 'default' reproduces the original DUIN colors. */
  brainGraphScheme?: BrainGraphSchemeId
  /** Recall-style brain-graph force + depth controls. The four force axes are
   *  normalized 0..100 with 50 == today's d3-force defaults (charge -30, link
   *  distance 30, link strength 1/min(deg), center strength 1) so an untouched
   *  graph is pixel-identical; connectionDepth is the 1..5 N-hop focus radius
   *  (default 2). Persisted like brainGraphScheme; applied in brain-shell's
   *  d3Force effect. Mirrors DefaultAppSettings.brainGraphLayout. */
  brainGraphLayout?: {
    nodeSpacing: number
    linkLength: number
    linkForce: number
  ...
  /** Agent voice/tone preset id (balanced | warm | concise | caveman |
   *  professional | playful | custom). Injected as a <voice> directive. */
  agentTone?: string
  /** Custom voice directive, used when agentTone === 'custom'. */
  agentToneCustom?: string
  /** Whether harness hooks run. Mirrors DefaultAppSettings.enableHooks. */
  enableHooks: boolean
  /** Has the operator consented to DUIN sending vault content to a cloud model on its own
   *  (boot-time and edit-driven extraction/construction)? Recorded main-side when a provider
   *  key is saved after the disclosure line. Mirrors DefaultAppSettings.cloudExtractionConsent. */
  ...
  /** Full computer access. OFF by default: CONFINED mode — jailed to the vault + active
   *  workspace + sandboxWritePaths, with the exec-token gate and per-action approvals. When
   *  the operator turns it on, the agent's file tools, the file browser, and the sandboxed
   *  shell are UNCONFINED — DUIN reads, writes, moves, and deletes anywhere on the machine, for
   *  every turn (local app AND inbound channels), with no folder to pre-declare. The
   *  catastrophic-command floor (format / rm -rf / / shutdown / diskpart …) and the .trash
   *  reversibility hold in both modes. Mirrors DefaultAppSettings.fullComputerAccess. */
  fullComputerAccess: boolean
  /** Response language. 'auto' = emit NO reply-language directive (byte-identical default);
   *  'en' | 'zh' | 'ja' pin the assistant's user-visible prose. Mirrors
   *  DefaultAppSettings.language. */
  language?: 'auto' | 'en' | 'zh' | 'ja'
  /** Whether text-to-speech output is enabled. Mirrors DefaultAppSettings.ttsEnabled (default off). */
  ttsEnabled: boolean
  ...
  /** Scheduled cron automations. Separate from `backgroundAutonomy` so arming the
   *  self-improve tick cannot arm billable scheduled agents as a side effect. */
  automationsEnabled?: boolean
  /**
   * OCR — extract text from image files (screenshots) and scanned documents so
   * they're searchable / groundable. Default ON; best-effort (no bundled models
   * → empty text, never a crash). The `DUIN_OCR` env var overrides this setting
   * main-side (e.g. `DUIN_OCR=0` force-off for debugging). Mirrors
   * DefaultAppSettings.ocrEnabled. */
  ocrEnabled?: boolean
  /** OCR engine: 'tesseract' (default) or 'paddle' (local PP-OCRv5, higher CJK
   *  quality). `DUIN_OCR_ENGINE` env overrides. Mirrors DefaultAppSettings.ocrEngine. */
  ocrEngine?: 'tesseract' | 'paddle'
  /** Proactive — default outbound destination for scheduled / agent-initiated
   *  sends. {kind,target}; kind 'push' = OS notification (no external creds).
   *  Mirrors DefaultAppSettings.homeChannel. */
  homeChannel?: { kind: string; target: string }
  /** Ingest — RSS/Atom feed URLs the `rss` source adapter pulls each sync.
   *  Mirrors DefaultAppSettings.rssFeeds. */
  rssFeeds?: string[]
  /** Proactive approval loop (#1) — the DESIGNATED OPERATOR per channel. Only this
   *  (channelId,userId) identity's reply may approve an AFK-gated action; any other
   *  paired user is refused + audited. Empty = unset (no channel approval possible).
   *  Mirrors DefaultAppSettings.operator. */
  operator?: { channelId: string; userId: string }
  /** Proactive approval loop (#1) — how long an unanswered channel approval waits
   *  before defaulting to DENY (ms). Mirrors DefaultAppSettings.approvalTimeoutMs. */
   ...
   *  Each watcher is individually enable-flagged and default OFF. driftThreshold =
   *  |observed−expected| a tier must exceed; debounceMs coalesces bursts; quietHours
   *  (local-clock hours; start===end = disabled, may wrap midnight) suppresses
   *  overnight. Mirrors DefaultAppSettings.watchers. */
  watchers?: {
    forecast: boolean
    calibration: boolean
    task: boolean
  ...
  /** RAG block (settings.json `rag`). Partial: only keys the main process must state a default
   *  for live in DEFAULT_APP_SETTINGS; RagSettings.tsx owns the full panel shape. */
  rag?: { multiQueryRewrite?: boolean; [key: string]: unknown }
}
...
// `sandboxBypass` is part of this union because the main process treats it as a real
// risk: action-class.ts lists it in BOTH MUTATING_RISKS and CAP_RISKS, and ~15 files
// under electron/ set or read it. Omitting it here meant the renderer's type could not
// even NAME the highest-severity class the governance layer acts on — a descriptor
// carrying it did not typecheck on this side, so the risk chip and any renderer-side
// reasoning about it were unreachable for the one category that matters most.
...
/** What a standing policy has decided on its own, joined onto its row in
 *  PermissionsSettings. Mirrors `electron/services/event-log.ts` PolicyUsage. */
...
export interface ToolApprovalRequest {
  ...
  args: Record<string, unknown>
  conversationId?: string
  /** Set by the main process when this prompt is a forced re-ask: every
   *  persisted "always allow" policy was deliberately skipped (sandbox bypass,
   *  or a mutating fallback-provenance call). Crosses IPC intact — see
   *  permissions-store `askUser`, which strips only `signal`. */
  dangerous?: boolean
}
...
export type ArtifactType = 'html' | 'svg' | 'mermaid' | 'jsx' | 'react' | 'markdown' | 'canvas'
...
export interface ProcessedFile {
  ...
  /** OCR-extracted text for `kind: 'image'` attachments. Set by the main-process
   *  file handler only when DUIN_OCR is enabled and OCR found text; lets a pasted
   *  screenshot stay groundable on non-vision models. Undefined otherwise. */
  ocrText?: string
}
