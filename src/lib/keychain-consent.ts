...
// Lazy (not module-const): the prompt must resolve in the language active at
// CONFIRM time, and this module loads before settings do.
function plaintextConsentPrompt(): string {
  return t(
    'Encryption is unavailable on this system. Your key will be stored as plaintext on disk (userData/keys.json). Once you grant consent it applies for the rest of this DUIN session. Continue?'
  )
}
...
export async function ensurePlaintextConsentIfNeeded(): Promise<boolean> {
  ...
  const ok = window.confirm(plaintextConsentPrompt())
  if (!ok) return false
