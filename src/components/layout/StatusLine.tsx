...
export function StatusLine() {
  ...
  useEffect(() => {
    ...
    refresh()
    const interval = setInterval(() => { if (document.visibilityState === 'visible') refresh() }, 30_000)
    return () => {
      cancelled = true
  ...
  useEffect(() => {
    ...
    refresh()
    const interval = setInterval(() => { if (document.visibilityState === 'visible') refresh() }, 10_000)
  ...
  return (
    ...
    >
      ...
      {config.source === 'user' && (
        <span
          className="ml-auto text-[11px] uppercase tracking-wider opacity-60"
          title={t('Loaded from userData/statusline.md')}
        >
          custom
        </span>
      )}
