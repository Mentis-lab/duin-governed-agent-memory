...
describe('WC-8 PRJ-10 closure — sidebar "+" → NewProjectModal regression', () => {
  ...
  it('the "+" button has an accessible label (aria-label="New project")', () => {
    ...
    // Accepts the localized form too: the label is now aria-label={t('New project')} so a
    // zh/ja screen reader announces it in the operator's language. The assertion is about the
    // label EXISTING, not about the literal that happens to express it.
    expect(sidebar).toMatch(/aria-label=(?:"New project"|\{t\('New project'\)\})/)
  })
})
...
describe('WC-8 PRJ-10 closure — NewProjectModal accessibility contract', () => {
  it('renders as a dialog with aria-modal and aria-label', () => {
    ...
    expect(modal).toMatch(/aria-label=(?:"New project"|\{t\('New project'\)\})/)
  })
