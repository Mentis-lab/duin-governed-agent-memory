  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-3">
        ...
        <span className="font-mono text-[11px] text-[var(--text-muted)]">{entries.length}</span>
      </div>
      ...
      >
        {entries.length === 0 && !loading ? (
          ...
          <div className="space-y-2">
            ...
            {loading && <div className="px-3 py-2 text-[11px] text-[var(--text-muted)]">{t('Loading...')}</div>}
            {!hasMore && entries.length > 0 && (
              <div className="px-3 py-2 text-[11px] text-[var(--text-muted)]">{t('End')}</div>
            )}
          </div>
        )}
      </div>
...
function SessionRow({
  ...
  return (
    ...
    >
      {draggable && (
        <span className="cursor-grab font-mono text-[11px] text-[var(--text-muted)]" aria-hidden>
          ::
        </span>
      )}
      <button type="button" onClick={onSelect} className="min-w-0 flex-1 text-left" title={entry.title}>
        <span className="flex items-center gap-1">
          ...
          {unreadCount > 0 && (
            <span className="shrink-0 rounded-full bg-[var(--accent)] px-1.5 py-0.5 font-mono text-[11px] text-[var(--bg-primary)]">
              {unreadCount}
            </span>
          )}
        </span>
      ...
      >
        <MenuButton label={t('Resume here')} onClick={onResume} onClose={() => setMenuOpen(false)} />
        <MenuButton label={t('Duplicate')} onClick={onDuplicate} onClose={() => setMenuOpen(false)} />
        <MenuButton
          label={entry.archived ? 'Unarchive' : 'Archive'}
          onClick={() => onArchive(!entry.archived)}
          onClose={() => setMenuOpen(false)}
        />
        <MenuButton label={t('Delete')} destructive onClick={onDelete} onClose={() => setMenuOpen(false)} />
      </PopoverMenu>
    </div>
  )
}
