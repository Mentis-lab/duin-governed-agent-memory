  ...
  return (
    <div className="border-b border-[var(--panel-border)] pb-2" data-testid="activity-dashboard">
      <div className="mx-3 mt-3 flex items-center justify-between">
        ...
        <span className="rounded-full bg-[var(--bg-tertiary)] px-2 py-0.5 font-mono text-[11px] text-[var(--text-muted)]">
          {runningCount} live
        </span>
      </div>
      ...
      {!collapsed && (
        <div className="mx-3 mt-2 max-h-80 overflow-y-auto pr-1 scrollbar-visible">
          {/* U1: a failed read is NOT "nothing is running". `activityError` is set by
              the store whenever any section's IPC read fails; without it this panel
              reported an idle machine over a main process it could not reach. */}
          {activityError && (
            <p
              role="alert"
              className="mb-1 rounded-md border border-[var(--panel-border)] bg-[var(--app-bg)] px-2 py-1.5 text-[11px] leading-relaxed text-[var(--text-secondary)]"
            >
              Couldn&apos;t read activity — {activityError}. Agents or automations may still
              be running.{' '}
              <button
                type="button"
