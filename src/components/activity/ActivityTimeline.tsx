      ...
      <div className="space-y-1.5">
        {rows.length === 0 && !loading && !error && (
          <p className="font-mono text-[11px] text-[var(--text-muted)]">
            {t('No events yet for this scope.')}
          </p>
        )}
        {rows.map((event) => (
...
function EventRow({ event }: { event: EventRecord }) {
  ...
  return (
    <div className="flex items-start gap-2.5 rounded border border-[var(--panel-border)]/50 bg-[var(--bg-primary)]/50 px-2.5 py-1.5">
      <span
        className={`mt-[5px] h-2 w-2 shrink-0 rounded-full ${style.dotClass}`}
        aria-label={style.label}
        title={style.label}
      />
      <div className="flex-1 space-y-0.5 overflow-hidden">
        <div className="flex items-baseline justify-between gap-2">
          <span className="truncate font-mono text-[12px] leading-5 text-[var(--text-primary)]">
        ...
        {subtitle && (
          <div className="truncate font-mono text-[11px] leading-4 text-[var(--text-secondary)]">
            {subtitle}
          </div>
        )}
        {event.correlationId && (
          <div className="truncate font-mono text-[11px] leading-4 text-[var(--text-muted)]">
