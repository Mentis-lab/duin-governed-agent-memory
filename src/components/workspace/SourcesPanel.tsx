...
function KindBadge({ kind }: { kind: string }): React.ReactElement {
  ...
  return (
    <span className="shrink-0 rounded border border-[var(--panel-border)] bg-[var(--bg-tertiary)] px-1 font-mono text-[11px] text-[var(--text-muted)]">
      {label}
    </span>
  )
}
  ...
  if (sources.length === 0) {
    return (
      <PanelEmptyState
        ...
        action={
          <Button variant="secondary" className="bg-[var(--bg-tertiary)] hover:border-[var(--accent)]"
        ...
        return (
          <div key={groupKey} className="mb-3">
            <div className="mb-1 px-1 text-[11px] font-medium uppercase tracking-wide text-[var(--text-muted)]">
              {labels[groupKey]}{' '}
              <span className="font-mono text-[11px]">({group.length})</span>
            </div>
