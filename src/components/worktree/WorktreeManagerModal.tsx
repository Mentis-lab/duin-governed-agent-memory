    ...
    >
      <div className="flex w-full max-w-lg flex-col overflow-hidden rounded-lg border border-[var(--panel-border)] bg-[var(--bg-primary)] shadow-xl">
        <div className="flex items-center justify-between border-b border-[var(--panel-border)] px-4 py-3">
          <h2 className="text-[14px] font-medium text-[var(--text-primary)]">{t('Worktrees')}</h2>
          <IconButton
            onClick={close}
            aria-label={t('Close')}
          >
        ...
        <div className="border-b border-[var(--panel-border)] p-4">
          ...
          <div className="flex flex-col gap-2 text-[12px]">
            <input
              ...
              className="w-full rounded border border-[var(--panel-border)] bg-[var(--bg-secondary)] px-2 py-1 text-[12px] text-[var(--text-primary)] outline-none focus:border-[var(--accent)]"
            />
            <input
              type="text"
              value={newPath}
        ...
        <div className="max-h-[40vh] overflow-y-auto p-4">
          ...
          {list.length === 0 && (
            <p className="text-[12px] text-[var(--text-muted)]">{t('None.')}</p>
          )}
          {list.map((wt, i) => (
            <div
              key={wt.path}
