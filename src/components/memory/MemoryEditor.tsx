...
export function MemoryEditor({ initial, initialDraft, onClose, onDeleted }: Props) {
  ...
  const handleSave = async () => {
    ...
    const result = await write({
      ...
      body: body.trim(),
      // This editor IS the operator typing. A new memory here is the strongest
      // provenance there is; an edit keeps whatever the memory already had, since
      // rewording a fact does not change where it came from.
      source: editing ? (initial?.source ?? 'unknown') : 'user-explicit',
      // Editing targets an existing file on purpose (the name field is locked above,
      // so the slug cannot move). Creating does NOT: the name here is free text and
      // the backend slug-normalizes it, so an unrelated existing memory could share
      // the resulting slug. 'create' makes the backend disambiguate instead of
      // replacing that memory's contents.
      mode: editing ? 'overwrite' : 'create'
    })
    setSaving(false)
    if (result) onClose()
  }
    ...
    <div className="flex flex-col gap-3 px-1 pb-2">
      ...
      <div className="grid grid-cols-[80px_1fr] items-center gap-2 px-2">
        ...
        <input
          ...
          className="rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] px-2 py-1 text-[12px] text-[var(--text-primary)] disabled:opacity-50"
        />
        ...
        <input
          ...
          placeholder={t('One-line hook shown in MEMORY.md')}
          className="rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] px-2 py-1 text-[12px] text-[var(--text-primary)]"
        />
      </div>
      ...
      <div className="px-2">
        ...
        <textarea
          ...
          rows={10}
          placeholder={t('Body: use [[other-memory-name]] to cross-reference.')}
