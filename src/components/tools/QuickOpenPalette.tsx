...
export function QuickOpenPalette() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // Reuse the cache ONLY when it belongs to the CURRENT workspace root.
      //
      // The guard used to be `if (index) return` against a MODULE-level CACHED, with no
      // dependency on the workspace at all — so after switching workspace or vault,
      // Quick Open kept listing the previous root's files and opening paths from it.
      // FileIndex has always carried its own `root`; nothing compared it.
    ...
    >
      <div className="flex w-full max-w-xl flex-col overflow-hidden rounded-lg border border-[var(--panel-border)] bg-[var(--bg-primary)] shadow-xl">
        <input
          ...
          className="w-full bg-transparent px-4 py-3 text-[14px] text-[var(--text-primary)] outline-none placeholder:text-[var(--text-muted)]"
        />
        <div className="border-t border-[var(--panel-border)]" />
        <div ref={listRef} className="max-h-[40vh] overflow-y-auto py-1">
          {error && (
            <p className="px-4 py-3 text-[12px] text-[var(--error)]">{error}</p>
          )}
          ...
          {matches.map((rel, i) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-baseline gap-2 px-4 py-1.5 text-left text-[12px] transition-colors ${
                  i === activeIdx
        ...
        {index?.truncated && (
          <div className="border-t border-[var(--panel-border)] px-4 py-1.5 text-[11px] text-[var(--text-muted)]">
            {t('Index truncated at 5000 files.')}
          </div>
        )}
      </div>
    </div>
