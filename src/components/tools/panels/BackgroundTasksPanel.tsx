...
function SectionHeader({ label, count }: { label: string; count: number }) {
  return (
    <div className="flex items-center gap-2 px-3 pt-3 pb-1">
      ...
      <span className="rounded border border-[var(--panel-border)] px-1.5 py-0 font-mono text-[11px] text-[var(--text-muted)]">
        {count}
      </span>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-none text-[11px] text-[var(--text-muted)]" aria-hidden>
          {expanded ? '▲' : '▼'}
        </span>
      </button>
      {expanded && (
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-2 border-b border-[var(--panel-border)] px-3 py-2 text-[12px]">
        <span className="rounded border border-[var(--panel-border)] px-1.5 py-0.5 font-mono text-[11px] text-[var(--text-muted)]">
          {totalLive} live
        </span>
        <button
          type="button"
          ...
          title={t('Refresh agent runs + wakeups + automations')}
        >
      ...
      <div className="flex flex-1 flex-col gap-1 overflow-y-auto pb-2">
        {totalLive === 0 && automations.length === 0 && allTools.length === 0 && (
          <div className="flex flex-1 flex-col items-center justify-center px-6 text-center text-[12px] text-[var(--text-muted)]">
            ...
            <span>
              {t('No agent runs, tool calls, wakeups, or scheduled tasks are active.')}
            </span>
          </div>
        )}
        ...
        {allTools.length > 0 && (
          <>
            <SectionHeader label={t('Tool calls')} count={allTools.length} />
        ...
        {liveAgents.length > 0 && (
          <>
            <SectionHeader label={t('Agents')} count={liveAgents.length} />
            <ul className="space-y-1 px-3">
              {liveAgents.map((run) => (
                <li
                  key={run.id}
                ...
                >
                  ...
                  <Button variant="danger" className="border-[var(--error)] px-1.5 hover:bg-[var(--error)] hover:text-white"
        ...
        {pendingWakeups.length > 0 && (
          <>
            <SectionHeader label={t('Wakeups')} count={pendingWakeups.length} />
            <ul className="space-y-1 px-3">
              {pendingWakeups.map((wakeup) => (
                ...
                >
                  ...
                  <Button variant="danger" className="px-1.5 hover:border-[var(--error)]"
        ...
        {automations.length > 0 && (
          <>
            <SectionHeader label={t('Scheduled')} count={automations.length} />
            <ul className="space-y-1 px-3">
