  ...
  if (!conversationId || !snapshot || snapshot.steps.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center px-6 text-center text-[12px] text-[var(--text-muted)]">
        <span className="font-mono uppercase tracking-wider text-[11px] mb-1">{t('No plan yet')}</span>
        <span>
          {t('A plan appears here once the model emits a checklist or enters plan mode.')}
        </span>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-[var(--panel-border)] px-3 py-2 text-[12px]">
        ...
        {planModeActive && (
          <span className="rounded border border-[var(--warning)] bg-[var(--warning)]/10 px-1.5 py-0.5 text-[11px] text-[var(--warning)]">
            gated
          </span>
        )}
        {planModeActive && (
          <div className="ml-auto flex gap-1">
            ...
            <Button variant="danger" className="border-[var(--error)] bg-[var(--bg-primary)] hover:bg-[var(--error)] hover:text-white"
              onClick={rejectPlan}
              disabled={busyAction !== null}
            >
