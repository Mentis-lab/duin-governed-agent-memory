...
import { isLightMode } from '@/duin/lib/light-color'
...
// Status letters + diff rows use the theme's semantic tokens (mode-tuned, so they
// stay legible on both fields) rather than fixed Tailwind color literals that were
// calibrated for a dark field.
function statusLabel(s: FileStatus): { letter: string; tone: string } {
  ...
  if (s.indexStatus === 'M' || s.workStatus === 'M') return { letter: 'M', tone: 'text-[var(--warning)]' }
  if (s.indexStatus === 'A' || s.workStatus === 'A') return { letter: 'A', tone: 'text-[var(--success)]' }
  if (s.indexStatus === 'D' || s.workStatus === 'D') return { letter: 'D', tone: 'text-[var(--error)]' }
  if (s.indexStatus === 'R' || s.workStatus === 'R') return { letter: 'R', tone: 'text-[var(--accent)]' }
  return { letter: '?', tone: 'text-[var(--text-muted)]' }
}

function lineClass(line: string): string {
  // Diff rows: dark-inverted (bg-*-900/30 + *-300 text) reads as a scar on white,
  // so pick a light palette in light mode.
  const light = isLightMode()
  if (line.startsWith('+') && !line.startsWith('+++'))
    return light ? 'bg-green-100 text-green-800' : 'bg-green-900/30 text-green-300'
  if (line.startsWith('-') && !line.startsWith('---'))
    return light ? 'bg-red-100 text-red-800' : 'bg-red-900/30 text-red-300'
...
export function ReviewPanel() {
  // Subscribe so a theme toggle re-renders the diff (lineClass reads the mode).
  useSettingsStore((s) => s.settings.themeMode)
  ...
  useEffect(() => {
    ...
    setDiffLoading(true)
    setDiff('')
    // Generation guard. Without it every in-flight fetch called setDiff unconditionally
    // on resolution, so clicking file A then quickly file B let A's slower diff land
    // LAST — showing A's contents under B's filename, with nothing indicating a race.
    let current = true
    void (async () => {
      ...
      <div className="flex items-center justify-between border-b border-[var(--panel-border)] px-3 py-1.5 text-[12px] text-[var(--text-secondary)]">
        <div className="flex items-center gap-2">
          ...
          {status && status.ahead > 0 && (
            <span className="text-[11px] text-[var(--text-muted)]">↑{status.ahead}</span>
          )}
          {status && status.behind > 0 && (
            <span className="text-[11px] text-[var(--text-muted)]">↓{status.behind}</span>
          )}
        </div>
        <IconButton
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="w-1/3 min-w-[180px] overflow-auto border-r border-[var(--panel-border)] py-1 text-[12px]">
          ...
          {!error && files.length === 0 && (
            <p className="px-3 py-2 text-[var(--text-muted)]">{t('No changes — clean working tree.')}</p>
          )}
          {stagedFiles.length > 0 && (
            <>
              <div className="px-3 pb-1 pt-1 text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)]">
              ...
              {stagedFiles.map((f) => {
                ...
                return (
                  ...
                  >
                    <span className={`w-3 shrink-0 text-center font-mono text-[11px] font-bold ${lbl.tone}`}>{lbl.letter}</span>
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      className="rounded px-1 text-[11px] text-[var(--text-muted)] opacity-0 transition-opacity hover:text-[var(--text-primary)] group-hover:opacity-100"
                      title={t('Unstage')}
                    >
                      −
                    </button>
              ...
              {unstagedFiles.map((f) => {
                ...
                return (
                  ...
                  >
                    ...
                    {f.indexStatus !== '?' && (
                      <Button variant="danger" className="px-1 opacity-0 group-hover:opacity-100"
        ...
        <div className="flex w-2/3 min-w-0 flex-col overflow-hidden">
          {!selected ? (
            <p className="m-auto text-[12px] text-[var(--text-muted)]">{t('Select a file to view its diff.')}</p>
          ) : diffLoading ? (
            <p className="m-auto text-[12px] text-[var(--text-muted)]">Loading diff…</p>
          ) : (
            <div className="min-h-0 flex-1 overflow-auto code-font text-[12px] leading-snug">
              {hunks.length === 0 && (
                ...
                <div key={i} className="border-b border-[var(--panel-border)]">
                  <div className="flex items-center justify-between bg-[var(--bg-tertiary)] px-3 py-1">
                    ...
                    <button
                      type="button"
                      onClick={() => askFix(h)}
                      className="rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] px-2 py-0.5 text-[11px] text-[var(--text-secondary)] transition-colors hover:border-[var(--accent)] hover:text-[var(--text-primary)]"
                      title={t('Send this hunk to the chat input')}
                    >
                      Fix this →
                    </button>
                  </div>
