...
async function ensureSpawned(id: string, shellKind: ShellKind): Promise<boolean> {
  ...
  // Cache only LIVE sessions. A failed (or thrown) spawn used to park its false/rejected
  // promise here forever, so every later mount of the panel silently reused the failure —
  // a permanently dead terminal with no path back short of an app restart.
  promise.then(
    (ok) => {
      if (!ok) spawnPromises.delete(id)
    },
    () => spawnPromises.delete(id)
  )
  return promise
}

// The backend is SHELL-MODE (child_process pipes, no PTY — see pty-manager.ts): the shell
// never sees a TTY, so it prints no prompt and echoes nothing, and PowerShell/cmd only run a
// line when it ends in CRLF. xterm alone therefore rendered a black rectangle that ate
// keystrokes (QA 2026-08-24, F5). This minimal line discipline supplies what the missing TTY
// would have: local echo, backspace, a visible prompt, and EOL-correct line submission.
const promptFor = (kind: ShellKind): string => (kind === 'git-bash' || kind === 'wsl' ? '$ ' : '> ')
const eolFor = (kind: ShellKind): string => (kind === 'git-bash' || kind === 'wsl' ? '\n' : '\r\n')
...
export function TerminalPanel() {
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      cursorBlink: true,
      // xterm renders to canvas and needs a literal list (CSS vars don't resolve here),
      // so this mirrors --font-code. The CJK entries matter: CJK is double-width, and
      // without a FIXED-WIDTH CJK fallback, command output containing 中文 falls back to
      // a proportional face and every column in the terminal shears.
      fontFamily:
        'ui-monospace, "SF Mono", Consolas, "Liberation Mono", Menlo, ' +
        '"Sarasa Mono SC", "MS Gothic", "Noto Sans Mono CJK SC", monospace',
      fontSize: 13,
      theme: {
        background: '#000000',
        foreground: '#e8e8e8'
    ...
    // Line-mode input: echo locally, edit locally, ship whole lines with the
    // shell's EOL on Enter. Raw pass-through fed bare '\r' into a pipe-mode
    // shell — which never ran anything and echoed nothing back.
    let lineBuf = ''
    const inputDisposable = term.onData((data) => {
      for (const ch of data) {
        if (ch === '\r' || ch === '\n') {
          term.write('\r\n')
          void window.api.terminal.write({ id: sessionId, data: lineBuf + eolFor(activeShell) })
          lineBuf = ''
        } else if (ch === '\x7f' || ch === '\b') {
          if (lineBuf.length > 0) {
          ...
          // Ctrl+C: drop the local line and forward the interrupt so a running
          // child at least gets the byte (pipe-mode shells vary in honoring it).
          lineBuf = ''
          term.write('^C\r\n' + promptFor(activeShell))
          void window.api.terminal.write({ id: sessionId, data: '\x03' })
        } else if (ch >= ' ' || ch === '\t') {
          lineBuf += ch
          term.write(ch)
        }
    ...
    void (async () => {
      ...
      // First attach for this session: say what this surface is (shell-mode, no TTY —
      // output appears after each command) and paint a prompt so it reads alive.
      if (!historyBuffers.get(sessionId)) {
        const banner = `\x1b[2m[${activeShell} — line mode: type a command, Enter runs it]\x1b[0m\r\n${promptFor(activeShell)}`
        recordHistory(sessionId, banner)
        term.write(banner)
      }
    })()
