  ...
  if (!status?.connected) {
    return (
      <div className="p-3 text-[12px] text-[var(--text-muted)]">
        {t('Connect GitHub from Settings to browse pull requests.')}
      </div>
    )
  }
    ...
    <div className="flex h-full flex-col gap-2 p-2 text-[12px] text-[var(--text-primary)]">
      ...
      <div className="flex flex-1 flex-col gap-1 overflow-y-auto pr-1">
        ...
        {!loading && prs.length === 0 && !error && (
          <p className="text-[11px] text-[var(--text-muted)]">{t('No PRs match this filter.')}</p>
        )}
        {prs.map((pr) => (
          <div
            key={pr.number}
          ...
          >
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {pr.draft && (
                  <span className="rounded border border-[var(--panel-border)] bg-[var(--bg-tertiary)] px-1 text-[11px] uppercase tracking-wider text-[var(--text-muted)]">
                    draft
                  </span>
                )}
                {pr.merged && (
                  <span className="rounded border border-violet-900/50 px-1 text-[11px] uppercase tracking-wider text-violet-300">
                    merged
                  </span>
                )}
                {!pr.merged && pr.state === 'closed' && (
                  <span className="rounded border border-red-900/50 px-1 text-[11px] uppercase tracking-wider text-red-300">
                    closed
                  </span>
                )}
                ...
                <span className="text-[11px] text-[var(--text-muted)]">{pr.user.login}</span>
              </div>
            </button>
