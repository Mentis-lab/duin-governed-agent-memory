...
export function PRStatusChecks({ owner, repo, number }: Props) {
  ...
  if (summary.checks.length === 0) {
    return <p className="px-2 py-1 text-[11px] text-[var(--text-muted)]">{t('No status checks yet.')}</p>
  }

  return (
    <div className="flex flex-col gap-1 px-2 py-1">
      <div className="flex items-center gap-2 text-[11px] text-[var(--text-muted)]">
        <span className="uppercase tracking-wider">{t('Overall')}</span>
        <span className={`rounded border px-1.5 py-0.5 text-[11px] font-medium uppercase tracking-wider ${STATE_TONES[summary.overall] ?? STATE_TONES.neutral}`}>
          {summary.overall}
        </span>
        {loading && <span className="text-[11px]">refreshing…</span>}
      </div>
      <ul className="flex flex-col gap-0.5">
        {summary.checks.map((c) => (
          ...
          >
            ...
            <span className={`shrink-0 rounded border px-1 text-[11px] uppercase tracking-wider ${STATE_TONES[c.state] ?? STATE_TONES.neutral}`}>
              {c.state}
            </span>
            {c.targetUrl && (
              <button
