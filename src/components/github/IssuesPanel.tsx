  ...
  if (!status?.connected) {
    return (
      <div className="p-3 text-[12px] text-[var(--text-muted)]">
        {t('Connect GitHub from Settings to browse issues.')}
      </div>
    )
  }
    ...
    <div className="flex h-full flex-col gap-2 p-2 text-[12px]">
      ...
      <div className="flex flex-1 flex-col gap-1 overflow-y-auto pr-1">
        ...
        {!loading && issues.length === 0 && !error && (
          <p className="text-[11px] text-[var(--text-muted)]">{t('No issues match.')}</p>
        )}
        {issues.map((issue) => (
          <div
            key={issue.number}
          ...
          >
            ...
            <span className={`rounded border px-1 text-[11px] uppercase tracking-wider ${
              issue.state === 'open'
            ...
            {issue.labels.length > 0 && (
              <div className="flex shrink-0 gap-1">
                {issue.labels.slice(0, 3).map((l) => (
                  <span
                    key={l.name}
                    className="rounded border border-[var(--panel-border)] px-1 text-[11px] text-[var(--text-secondary)]"
