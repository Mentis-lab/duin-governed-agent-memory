  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-[16px] font-medium text-[var(--text-primary)]">{doc.name}</div>
      ...
      <div ref={menuRef} className="relative">
        ...
        >
          Open in
          <span aria-hidden className="text-[11px]">{menuOpen ? '▴' : '▾'}</span>
        </button>
        {menuOpen && (
          <div
            role="menu"
          ...
          >
            ...
            />
            <MenuItem label={t('Copy contents')} onClick={copy} />
            <MenuItem label={t('Download…')} onClick={download} />
          </div>
        )}
      </div>
    </div>
...
function MenuItem({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="flex w-full items-center justify-between px-3 py-2 text-left text-[12px] text-[var(--text-primary)] transition-colors hover:bg-[var(--accent-dim)] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-transparent"
    >
      <span>{label}</span>
