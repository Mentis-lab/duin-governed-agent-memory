...
describe('SP-7 era chrome — no raw harness internals in user copy', () => {
  ...
  it('UB-7: the Agents settings tab and the work-mode popover are both deleted', () => {
    expect(existsSync(join(here, '..', 'settings', 'AgentSettings.tsx'))).toBe(false)
    // The popover used to be asserted mode-switch-free. It went with the Environment
    // surface (2026-08-08) — it was only ever reachable from EnvironmentPanel — and a
    // file that does not exist cannot regrow a mode switch.
    expect(existsSync(join(here, '..', 'workspace', 'WorkModePopover.tsx'))).toBe(false)
  })
