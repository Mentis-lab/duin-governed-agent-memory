...
function Tile({ file, index }: { file: ProcessedFile; index: number }) {
  ...
  return (
    <div
      className={`group flex flex-col gap-1.5 rounded-2xl border bg-[var(--bg-primary)] px-3 py-2 text-[12px] shadow-sm ${borderClass}`}
    >
      <div className="flex items-center gap-3">
        {isImage ? (
          ...
          <span
            aria-hidden
            className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg font-mono text-[11px] tracking-wider ${
              ragReady
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate font-mono text-[12px] text-[var(--text-primary)]">{file.name}</div>
