...
export function TokenTicker() {
  ...
  return (
    <div className="mb-2 flex items-center justify-end gap-3 px-1 font-mono text-[11px] text-[var(--text-muted)]">
      <span title={t('Estimated user input tokens this conversation')}>
        ↑ {formatTokens(input)}
      </span>
      <span title={t('Estimated assistant output tokens this conversation')}>
        ↓ {formatTokens(output)}
      </span>
      <span aria-hidden className="text-[var(--border)]">·</span>
      <span title={t('Estimated total tokens this conversation')}>
