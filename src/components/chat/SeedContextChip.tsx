...
export function SeedContextChip({ seed }: { seed: ParsedSeedContext }) {
  ...
  return (
    <div className="mb-2 rounded-md border border-[var(--panel-border)] bg-[var(--bg-primary)]/70 text-[12px]">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <pre className="m-0 max-h-64 overflow-auto border-t border-[var(--panel-border)] p-3 text-[12px] text-[var(--text-primary)]">
          {seed.body}
        </pre>
      )}
    </div>
