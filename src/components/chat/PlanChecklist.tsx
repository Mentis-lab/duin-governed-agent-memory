...
function StatusIcon({ status }: { status: PlanStepStatus }) {
  if (status === 'done') {
    return (
      <span
        className="inline-flex h-3.5 w-3.5 flex-none items-center justify-center rounded-full bg-[var(--success)] text-[11px] font-bold text-[var(--bg-primary)]"
        aria-label="done"
      >
        &#10003;
      </span>
  ...
  return (
    ...
    >
      <div className="mb-1.5 flex items-center justify-between">
        <span className="font-mono uppercase tracking-wider text-[var(--text-muted)]">{t('Plan')}</span>
        <span
          className={
            allDone
