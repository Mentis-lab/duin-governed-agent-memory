    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55">
      <div className="w-[360px] rounded-lg border border-[var(--panel-border)] bg-[var(--bg-secondary)] p-4 shadow-2xl">
        <div className="mb-3 font-mono text-[12px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">
          {t('Pin as memory chapter')}
        </div>
        <label className="mb-3 block text-[12px] text-[var(--text-secondary)]">
          <span className="mb-1 block">{t('Title')}</span>
          <input
            type="text"
        ...
        <label className="mb-3 block text-[12px] text-[var(--text-secondary)]">
          <span className="mb-1 block">{t('Summary (optional)')}</span>
          <textarea
            rows={3}
            placeholder={t('One-line note shown on hover…')}
            value={summary}
            onChange={(e) => setSummary(e.target.value)}
            disabled={busy}
            className="w-full resize-none rounded border border-[var(--border)] bg-[var(--bg-primary)] px-2 py-1 text-[12px]"
          />
        </label>
        {error && (
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            className="rounded border border-[var(--accent)] bg-[var(--accent)]/15 px-2 py-1 text-[12px] hover:bg-[var(--accent)]/25 disabled:opacity-50"
          >
