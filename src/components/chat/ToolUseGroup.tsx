...
export function ToolUseGroup({ group }: ToolUseGroupProps) {
  ...
  // Reviewable proposed-edit calls are owned by the dedicated card surface
  // (MessageList's ProposedEditQueue → ProposedEditCard), fed by the persisted
  // store so the card survives reload. They are `transcriptHidden`, so they
  // never actually reach the tool-activity grouping; this branch is a
  // defensive guard — if a propose_edit call ever surfaces here, defer to the
  // card rather than rendering a raw tool-call row.
  if (group.toolName === 'propose_edit') return null
