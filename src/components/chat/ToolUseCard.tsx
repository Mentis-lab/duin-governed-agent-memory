    ...
    <span className="flex flex-none items-center gap-1">
      {visible.map((r) => (
        <span
          ...
          title={RISK_LABEL[r]}
          className={
            'rounded-full border bg-transparent px-1.5 py-[1px] text-[11px] font-mono uppercase tracking-wider ' +
    ...
    <div className="my-2 mx-auto w-full max-w-[80%]">
      ...
      {expanded && (
        ...
        >
          ...
          <pre className="mb-3 max-h-64 overflow-auto whitespace-pre-wrap break-words text-[12px] code-font text-[var(--text-secondary)]">
            {argsJson}
          </pre>
          ...
          {(result || isError || isDenied) && (
            <>
              <div className="mb-1 flex items-center gap-2 text-[11px] font-medium uppercase tracking-wider">
                <span className="text-[var(--text-muted)]">{t('Result')}</span>
                {isError && (
                  <span className="rounded-full border border-[var(--error)]/40 px-1.5 py-[1px] text-[11px] text-[var(--error)]">
                    error
                  </span>
                )}
                {isDenied && (
                  <span className="rounded-full border border-[var(--text-muted)]/40 px-1.5 py-[1px] text-[11px] text-[var(--text-muted)]">
                    denied
                  </span>
                )}
              </div>
              <pre
                className={
                  'max-h-64 overflow-auto whitespace-pre-wrap break-words text-[12px] code-font ' +
                  (isError
                    ? 'text-[var(--error)]'
                    : isDenied
              ...
              {preview.truncated && (
                <div className="mt-1 text-[11px] font-mono uppercase tracking-wider text-[var(--text-muted)]">
