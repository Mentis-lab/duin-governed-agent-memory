            ...
            <div className="space-y-4">
              ...
              <label className="block text-[12px] text-[var(--text-muted)]">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder={t('One or two sentences describing when this skill should activate.')}
                  className="mt-1 w-full resize-y rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] px-2 py-1.5 text-[12px] text-[var(--text-primary)] outline-none focus:border-[var(--accent)]"
                />
              </label>
              <label className="block text-[12px] text-[var(--text-muted)]">
            ...
            <div className="space-y-4">
              <fieldset className="rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] p-3">
                ...
                <label className="mt-1 flex items-start gap-2 text-[12px] text-[var(--text-primary)]">
                  ...
                  />
                  <span>
                    <strong>{t('Auto-invoke')}</strong> — the model decides when to use this skill based on
                    the description.
                  </span>
                </label>
                {/* Honesty: autoInvoke is recorded in frontmatter and round-trips, but nothing
                    consumes it yet (see electron/shared/skill-contract.ts). Promising an effect the
                    engine does not implement is the defect class this whole pass exists to remove. */}
                <p className="mt-1 text-[11px] italic text-[var(--text-muted)]">
                  Not active yet — today every skill applies only while it is switched on in
                  Customize → Skills. Your choice is saved and will take effect when auto-invoke ships.
                </p>
                <label className="mt-2 flex items-start gap-2 text-[12px] text-[var(--text-primary)]">
                  ...
                  />
                  <span>
                    <strong>{t('Manual only')}</strong> — the user must reference the skill by name before
                    it's loaded.
                  </span>
                </label>
              </fieldset>
              ...
              <label className="block text-[12px] text-[var(--text-muted)]">
                Suggested tools — a HINT to the model, not a restriction (one glob per line
                or comma-separated; leave empty to suggest nothing)
                <textarea
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {toolSuggestions.map((t) => (
                    <button
                      ...
                      className="rounded border border-[var(--panel-border)] bg-[var(--bg-secondary)] px-2 py-0.5 font-mono text-[11px] hover:border-[var(--accent)]"
                    >
                      + {t}
                    </button>
                  ))}
              ...
              <label className="block text-[12px] text-[var(--text-muted)]">
                Model override (optional, e.g. `qwen-max`) — saved, not yet honoured
                <input
                  value={draft.model}
                  onChange={(e) => update({ model: e.target.value })}
                  placeholder="recorded, but not applied yet — the conversation model is used"
              ...
              <fieldset className="rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] p-3">
                ...
                <label className="flex items-start gap-2 text-[12px] text-[var(--text-primary)]">
                  <Toggle
                    ...
                    onChange={(v) => update({ directoryMode: v })}
                    aria-label={t('Directory mode')}
                    className="mt-1"
                  />
                  <span>
                ...
                {draft.directoryMode && (
                  <label className="mt-2 flex items-start gap-2 text-[12px] text-[var(--text-primary)]">
                    <Toggle
                      ...
                      onChange={(v) => update({ scaffoldReference: v })}
                      aria-label={t('Scaffold reference')}
                      className="mt-1"
                    />
                    <span>
