...
export function AgentChip({ chip }: Props): ReactElement {
  ...
  return (
    <div
      ...
      data-tier={chip.tier ?? ''}
      className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[12px] ${tint} ${ring}`}
      title={
    ...
    >
      ...
      {chip.cached && <span className="text-[11px] opacity-70">[cached]</span>}
      {chip.tier && chip.tier !== 'unknown' && (
        <span className="text-[11px] opacity-70">[{chip.tier}]</span>
      )}
      {chip.durationMs !== undefined && (
        <span className="text-[11px] opacity-70">{durationLabel(chip.durationMs)}</span>
      )}
      {chip.tokensUsedEstimate !== undefined && chip.tokensUsedEstimate > 0 && (
        <span className="text-[11px] opacity-70">~{chip.tokensUsedEstimate}t</span>
      )}
    </div>
  )
}
