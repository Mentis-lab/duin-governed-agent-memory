...
const DEFAULTS: RagSettingsValue = {
  enabled: true,
  defaultEmbedderId: 'multilingual-e5-small',
    ...
    <div className="space-y-4">
      ...
      <Section title="OCR">
        <label className="flex cursor-pointer items-start gap-2 font-mono text-[11px]">
          <Toggle checked={ocrEnabled} onChange={(v) => void updateSettings({ ocrEnabled: v })} aria-label={t('OCR (image and scanned-doc text)')} />
          <div className="flex flex-col">
            <span className="text-[var(--text-primary)]">OCR (image &amp; scanned-doc text)</span>
            <span className="text-[var(--text-muted)]">Extract text from screenshots and scanned documents so they&apos;re searchable — runs locally, on your device.</span>
          </div>
        </label>
        <label className="flex items-center justify-between gap-2 font-mono text-[11px]">
          <span className="text-[var(--text-secondary)]">{t('Engine')}</span>
          <select
            value={ocrEngine}
            disabled={!ocrEnabled}
            onChange={(e) =>
              void updateSettings({
                ocrEngine: e.target.value as 'tesseract' | 'paddle'
              })
            }
            className="w-40 rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] px-2 py-1 font-mono text-[11px] text-[var(--text-primary)] disabled:opacity-50"
          >
            <option value="tesseract">{t('Tesseract (default)')}</option>
            <option value="paddle">{t('PaddleOCR (better CJK)')}</option>
          </select>
        </label>
      </Section>

      <Section title={t('Embeddings model')}>
        <select
      ...
      <Section title={t('Chunking')}>
        <NumericRow
          label={t('Chunk size (chars)')}
      ...
      <Section title={t('Rerank')}>
        ...
        >
          ...
          <option value="local-cross-encoder">
            {t('Local cross-encoder (slow, highest quality)')}
          </option>
      ...
      <label className="flex cursor-pointer items-start gap-2 font-mono text-[11px]">
        <Toggle checked={value.multiQueryRewrite} onChange={(v) => update({ multiQueryRewrite: v })} aria-label={t('Multi-query rewrite')} />
        <div className="flex flex-col">
          <span className="text-[var(--text-primary)]">{t('Multi-query rewrite')}</span>
          <span className="text-[var(--text-muted)]">Planner rewrites your query into 2–3 phrasings; results unioned via RRF.</span>
        </div>
      </label>
      <label className="flex cursor-pointer items-start gap-2 font-mono text-[11px]">
        <Toggle checked={value.autoRagInConversations} onChange={(v) => update({ autoRagInConversations: v })} aria-label={t('Auto-RAG in conversations')} />
        <div className="flex flex-col">
          <span className="text-[var(--text-primary)]">{t('Auto-RAG in conversations')}</span>
          <span className="text-[var(--text-muted)]">{t('When on, every chat turn in a conversation with attached collections runs retrieval automatically.')}</span>
        </div>
      </label>
      <label className="flex cursor-pointer items-start gap-2 font-mono text-[11px]">
        <Toggle checked={value.citationRequired} onChange={(v) => update({ citationRequired: v })} aria-label={t('Citation required')} />
        <div className="flex flex-col">
          <span className="text-[var(--text-primary)]">{t('Citation required')}</span>
          <span className="text-[var(--text-muted)]">{t('Model is instructed to refuse if no source supports a claim.')}</span>
        </div>
      </label>
    </div>
  )
