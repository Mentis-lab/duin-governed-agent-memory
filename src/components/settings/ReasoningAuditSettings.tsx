  ...
  return (
    <div className="space-y-5">
      ...
      <label
        ...
        className="flex cursor-pointer flex-col gap-2 rounded border border-[var(--border)] bg-[var(--bg-primary)] p-4 text-[12px] text-[var(--text-secondary)]"
      >
        <div className="flex items-baseline justify-between gap-3">
          <span className="font-medium text-[var(--text-primary)]">
            {t('Include past reasoning in API context')}
          </span>
          <Toggle
            id="includePastReasoning"
            checked={enabled}
            onChange={(v) => void updateSettings({ includePastReasoningInContext: v })}
            aria-label={t('Include past reasoning in API context')}
          />
        </div>
        <p className="text-[12px] leading-relaxed text-[var(--text-muted)]">
