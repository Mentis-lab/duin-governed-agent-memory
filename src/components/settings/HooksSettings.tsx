...
const EVENT_OPTIONS: HookEvent[] = [
  ...
  'postToolUse',
  'agentStop',
  // F3 autonomy lifecycle. These fire in production (loop-controller,
  // automations-runner, ipc/workflows) but were absent from this list, so no user hook
  // could ever subscribe to the app's unattended activity.
  'loopStarted',
  'loopIterationDone',
  'automationStarted',
  'automationDone',
...
const EVENT_DESCRIPTIONS: Record<HookEvent, string> = {
  ...
  loopStarted:
    'Fires when a loop iteration begins. Sandbox: `sourceId` (loop id), `label`, `promptBody`.',
  loopIterationDone:
    'Fires when a loop iteration ends, including a failed one. Sandbox: `sourceId`, `label`.',
  automationStarted:
    'Fires when a scheduled automation begins. Sandbox: `sourceId` (automation id), `label`, `promptBody`.',
  automationDone:
    'Fires when an automation finishes. Sandbox: `sourceId`, `label`.',
  workflowStarted:
    'Fires when a workflow run begins. Sandbox: `sourceId` (run id), `label`.',
  workflowFinished:
    'Fires when a workflow run ends. Sandbox: `sourceId`, `label`.'
}
...
const STARTER_TEMPLATE: Record<HookEvent, string> = {
  ...
  loopStarted: '// A loop iteration started.' + NEWLINE + 'log("loop start:", label)',
  loopIterationDone: '// A loop iteration ended.' + NEWLINE + 'log("loop done:", label)',
...
export function HooksSettings() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <h2 className="text-[14px] font-medium text-[var(--text-primary)]">{t('Hooks')}</h2>
      ...
      <div className="grid grid-cols-[260px_1fr] gap-3">
        <div className="rounded border border-[var(--panel-border)] bg-[var(--bg-primary)]">
          ...
          <div className="max-h-[420px] overflow-y-auto p-1">
            {!loaded && (
              <div className="px-2 py-3 text-[11px] text-[var(--text-muted)]">{t('Loading...')}</div>
            )}
                ...
                >
                  <span onClick={(event) => event.stopPropagation()} className="shrink-0">
                    <Toggle
                      ...
                      aria-label={hook.enabled ? 'enabled' : 'disabled'}
                    />
                  </span>
        ...
        <div className="rounded border border-[var(--panel-border)] bg-[var(--bg-primary)] p-3">
          ...
          <label className="mt-3 block text-[11px] text-[var(--text-muted)]">{t('Body (JS)')}</label>
          <textarea
            value={editor.code}
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            {editor.hookId && (
              <Button variant="danger" className="bg-[var(--bg-secondary)] hover:border-[var(--error)]"
