    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="w-[min(480px,92vw)] rounded-xl border border-[var(--panel-border)] bg-[var(--bg-secondary)] p-6 shadow-2xl text-center">
          <p className="text-[var(--text-secondary)]">{t('Project not found.')}</p>
          <button
            onClick={onClose}
            className="mt-3 rounded-md px-4 py-2 text-[12px] font-medium text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]"
          >
            {t('Close')}
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {projectConversations.length === 0 ? (
            <p className="py-8 text-center text-[12px] text-[var(--text-muted)]">
              No sessions yet.
              <br />
              {t('Start a new session in this project to begin working.')}
            </p>
          ) : (
