...
describe('workflows-store — save/delete surface', () => {
  // These two bindings existed in main with nothing calling them: `overwrite` never
  // left the renderer (so a saved workflow could be created once and never edited)
  // and workflows:delete had no binding at all. The store is the layer that has to
  // carry the refusal CODE through — flattening it to null is what left the
  // overwrite path unreachable from the editor.
  const api = (impl: Record<string, unknown>): void => {
    ;(globalThis as unknown as { window: unknown }).window = { api: { workflows: impl } }
  }

  it('forwards overwrite to the handler', async () => {
    const save = vi.fn(async () => ({ success: true, data: { name: 'w', description: '', origin: 'user' } }))
    api({ save, list: async () => ({ success: true, data: [] }) })
    await useWorkflowsStore.getState().saveWorkflow('script', true)
    expect(save).toHaveBeenCalledWith({ script: 'script', overwrite: true })
  })

  it('passes a conflict refusal through with its code, not as a bare failure', async () => {
    api({
      save: async () => ({ success: false, code: 'conflict', name: 'dupe', error: 'already exists' }),
      list: async () => ({ success: true, data: [] })
    })
    const outcome = await useWorkflowsStore.getState().saveWorkflow('script')
    expect(outcome).toEqual({ ok: false, code: 'conflict', error: 'already exists', name: 'dupe' })
  })
  ...
  it('deletes through the binding and refreshes the library', async () => {
    const del = vi.fn(async () => ({ success: true, data: true }))
    const list = vi.fn(async () => ({ success: true, data: [] }))
    api({ delete: del, list })
    await expect(useWorkflowsStore.getState().deleteWorkflow('w')).resolves.toBe(true)
    expect(del).toHaveBeenCalledWith({ name: 'w' })
    expect(list).toHaveBeenCalled()
  })

  it('reports a failed delete rather than claiming success', async () => {
    api({
      delete: async () => ({ success: false, error: 'builtin' }),
      list: async () => ({ success: true, data: [] })
    })
    await expect(useWorkflowsStore.getState().deleteWorkflow('w')).resolves.toBe(false)
    expect(useWorkflowsStore.getState().libraryError).toBe('builtin')
  })
})
