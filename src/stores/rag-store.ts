...
export const useRagStore = create<RagState>((set, get) => ({
  ...
  submitIngest: async (collectionId, files) => {
    ...
    const data = res.data as { jobId: string | null; skippedMarkdown?: string[] }
    // Markdown is skipped server-side (it belongs in the Brain, not the
    // Library). Tell the user rather than silently dropping their file.
    const skipped = data.skippedMarkdown ?? []
    if (skipped.length > 0) {
      const label =
        skipped.length === 1
          ? `“${skipped[0]}” opens in the Brain — Markdown isn't added to the Library.`
          : `${skipped.length} Markdown files open in the Brain — not added to the Library.`
      toast.info(label)
    }
    // Only reload the doc list when something was actually queued (an
    // all-markdown drop returns a null jobId and adds no rows).
    if (data.jobId) {
      // Optimistically reload so the queued/loading row appears immediately.
      // Progress events will mutate the row in-place after.
