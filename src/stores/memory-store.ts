...
import type { BrokenMemoryLink, MemoryEntry, MemoryFile, MemorySource, MemoryType } from '@/lib/types'

interface WriteInput {
  name: string
  type: MemoryType
  /** Where this memory came from. Omitted → the backend records 'unknown'. */
  source?: MemorySource
  description?: string
  body: string
  projectSlug?: string
  /**
   * 'create' tells the backend this is a NEW entry, so a name that slug-normalizes
   * onto an existing memory gets a disambiguated slug rather than replacing it.
   * The renderer cannot check for the collision itself — memorySlug() lives in the
   * electron layer — so the decision has to travel with the write.
   */
...
export const useMemoryStore = create<MemoryState>((set, get) => ({
  ...
  loadMemories: async () => {
    ...
    try {
      ...
      // A rejected invoke must not leave the panel stuck on its spinner.
      toast.error(`Failed to load memory${err instanceof Error ? `: ${err.message}` : ''}`)
    } finally {
      set({ loading: false })
    }
  ...
  writeMemory: async (input: WriteInput) => {
    ...
    await get().loadMemories()
    const file = res.data as MemoryFile
    if (file?.slugRedirectedFrom) {
      // The user asked to create something whose name collided with an existing
      // memory. We kept both; say so, because otherwise the entry appears under a
      // name they did not type and the near-miss looks like nothing happened.
      toast.success(
        `Saved as "${file.name}" — a memory named "${file.slugRedirectedFrom}" already exists and was left unchanged.`
      )
    }
    return file
  },
  ...
  duplicateEntry: async (name: string) => {
    ...
    // The loop above compares against entries[].name, which holds backend SLUGS, so it
    // cannot see a collision the slug function would introduce. `mode: 'create'` is the
    // authoritative guard; this stays as a nicer first guess at the name.
    return get().writeMemory({
      name: candidate,
      type: source.type,
      // A duplicate inherits the original's provenance — the copy came from
      // wherever the original did, not from the act of duplicating it.
