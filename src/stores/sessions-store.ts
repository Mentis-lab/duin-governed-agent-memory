...
function getApi():
  | {
      ...
      conversation?: {
        // The handler answers `{ conversationId }` (ipc/conversation.ts), NOT a
        // SessionEntry. Declaring it as one is what let `res.data.id` typecheck while
        // always being undefined at runtime.
        fork: (
          id: string
        ) => Promise<{ success: boolean; data?: { conversationId: string }; error?: string }>
...
export const useSessionsStore = create<SessionsState>((set, get) => ({
  ...
  duplicate: async (id) => {
    ...
    await get().loadFirstPage()
    // The fork handler answers `{ conversationId }` (ipc/conversation.ts) — reading
    // `.id` here yielded undefined, so Duplicate copied the session and then returned
    // nothing for the caller to open. The copy existed; the UI just never navigated to
    // it, which reads as "Duplicate did nothing".
