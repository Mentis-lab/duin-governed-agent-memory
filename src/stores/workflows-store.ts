...
/** The save result the editor needs: on refusal, WHICH refusal. */
export type SaveOutcome =
  | { ok: true; entry: WorkflowLibraryEntry }
  | { ok: false; code: string; error: string; name?: string }
...
interface WorkflowsStoreState {
  ...
  saveWorkflow: (script: string, overwrite?: boolean) => Promise<SaveOutcome>
...
export const useWorkflowsStore = create<WorkflowsStoreState>((set, get) => ({
  ...
  saveWorkflow: async (script: string, overwrite = false) => {
    if (typeof window === 'undefined' || !window.api?.workflows?.save) {
      return { ok: false, code: 'unavailable', error: 'workflow save unavailable' }
    }
    const result = await window.api.workflows.save({ script, overwrite })
    if (!result.success) {
      const error = result.error ?? 'workflow save failed'
      set({ libraryError: error })
      // Pass the REFUSAL CODE through. workflow-library throws a typed
      // WorkflowSaveError ('conflict' vs 'scaffold-name') specifically so the
      // renderer can prompt to overwrite rather than prompt for a name — the
      // store used to flatten both to null, which left the editor with a dead
      // "save failed" toast and no way to reach the overwrite path at all.
      return { ok: false, code: result.code ?? 'unknown', error, name: result.name }
    }
    await get().refreshLibrary()
    return { ok: true, entry: result.data as WorkflowLibraryEntry }
  },

  deleteWorkflow: async (name: string) => {
    if (typeof window === 'undefined' || !window.api?.workflows?.delete) return false
    const result = await window.api.workflows.delete({ name })
    if (!result.success) {
      set({ libraryError: result.error ?? 'workflow delete failed' })
      return false
    }
    await get().refreshLibrary()
    return true
