...
// ──────────────────── embedder switch guard (load-bearing) ────────────────────

describe('rag:embedder:setActive dimension guard', () => {
  // Proves the guard is WIRED into the production handler, not just importable.
  // The mocked `app.getPath` throws, so a request that PASSES the guard fails
  // with the "electron app" message, while a request the guard REJECTS fails
  // with the dimension message BEFORE ever reaching app.getPath — which is how
  // this test distinguishes "guard fired" from "reached the service".

  it('rejects a wider embedder (bge-m3, 1024-dim) before touching the service', async () => {
    const res = await ipcRegistered.get('rag:embedder:setActive')!(undefined, 'bge-m3')
    expect(res.success).toBe(false)
    // The refusal names the 1024→384 mismatch, NOT the app-not-available error
    // the pre-guard code path produced.
    expect(res.error).toMatch(/1024/)
    expect(res.error).toMatch(/384/)
    expect(res.error).not.toMatch(/electron app not available/i)
  })

  it('lets a 384-dim embedder through the guard (reaches the service)', async () => {
    const res = await ipcRegistered.get('rag:embedder:setActive')!(
      undefined,
      'multilingual-e5-small'
    )
    // Guard passes → handler proceeds to app.getPath (mocked to throw), proving
    // the guard does NOT over-reject valid same-width embedders.
    expect(res.success).toBe(false)
    expect(res.error).toMatch(/electron app not available/i)
  })
})
...
// ──────────────────── markdown exclusion at ingest ────────────────────

describe('rag:document:ingest — markdown belongs in the Brain, not the Library', () => {
  // Markdown is the vault's native format; ingesting it into RAG is redundant
  // (the sidecar bridge round-trips it back into the vault anyway). The handler
  // skips .md/.mdx and reports them under skippedMarkdown so the UI can steer
  // the user to the Explorer.
  it('skips an all-markdown batch before touching the ingest manager', async () => {
    const create = await ipcRegistered.get('rag:collection:create')!(undefined, {
      name: 'Lib',
      embedderId: 'bge-small-en-v1.5'
    })
    const collectionId = create.data.id
    const res = await ipcRegistered.get('rag:document:ingest')!(undefined, collectionId, [
      { path: '/tmp/notes.md', name: 'notes.md' },
      { path: '/tmp/spec.mdx', name: 'spec.mdx' }
    ])
    // Success, not error: the skip fires BEFORE ensureIngestWired() (whose
    // app.getPath is mocked to throw in this file), so an all-markdown batch
    // returning success proves markdown never reaches the ingest manager.
    expect(res.success).toBe(true)
    expect(res.data.jobId).toBeNull()
    expect(res.data.skippedMarkdown).toEqual(['notes.md', 'spec.mdx'])
