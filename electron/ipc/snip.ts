...
export function registerSnipHandlers(): void {
  ipcMain.handle('snip:stats', async () => {
    try {
      ...
      return { success: false, error: friendly(err, 'snip:stats failed') }
    }
  })
  ...
  ipcMain.handle('snip:recent', async (_event, payload?: { limit?: number }) => {
    try {
      ...
      return { success: false, error: friendly(err, 'snip:recent failed') }
    }
  })
  ...
  ipcMain.handle(
    ...
    async (): Promise<{ success: true; data: FilterListEntry[] } | { success: false; error: string }> => {
      try {
        ...
        return { success: false, error: friendly(err, 'snip:listFilters failed') }
      }
    }
  )
  ...
  ipcMain.handle('snip:reloadFilters', async () => {
    try {
      ...
      return { success: false, error: friendly(err, 'snip:reloadFilters failed') }
    }
  })
  ...
  ipcMain.handle('snip:discover', async (_event, payload?: { sinceDays?: number; limit?: number }) => {
    try {
      ...
      return { success: false, error: friendly(err, 'snip:discover failed') }
    }
  })
  ...
  ipcMain.handle('snip:clearHistory', async () => {
    try {
      ...
      return { success: false, error: friendly(err, 'snip:clearHistory failed') }
    }
  })
  ...
  ipcMain.handle('snip:openFilterDir', async () => {
    try {
      ...
      return { success: false, error: friendly(err, 'snip:openFilterDir failed') }
    }
  })
}
