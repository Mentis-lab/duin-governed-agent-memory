...
export function registerPlanHandlers(): void {
  ipcMain.handle('plan:get', async (_event, conversationId?: string) => {
    try {
      ...
      return { success: false, error: friendly(err, 'plan:get failed') }
    }
  })
  ...
  ipcMain.handle('plan:listAllState', async () => {
    try {
      ...
      return { success: false, error: friendly(err, 'plan:listAllState failed') }
    }
  })
  ...
  ipcMain.handle('plan:clearConversationState', async (_event, conversationId: string) => {
    try {
      ...
      return { success: false, error: friendly(err, 'plan:clearConversationState failed') }
    }
  })
  ...
  ipcMain.handle('plan:clearAllState', async () => {
    try {
      ...
      return { success: false, error: friendly(err, 'plan:clearAllState failed') }
    }
  })
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        return { success: false, error: friendly(err, 'plan:update failed') }
      }
    }
  )
  ...
  ipcMain.handle('plan:isModeActive', async (_event, conversationId: string) => {
    try {
      ...
      return { success: false, error: friendly(err, 'plan:isModeActive failed') }
    }
  })
  ...
  ipcMain.handle('plan:enterMode', async (_event, conversationId: string) => {
    try {
      ...
      return { success: false, error: friendly(err, 'plan:enterMode failed') }
    }
  })
  ...
  ipcMain.handle('plan:exitMode', async (_event, conversationId: string) => {
    try {
      ...
      return { success: false, error: friendly(err, 'plan:exitMode failed') }
    }
  })
}
