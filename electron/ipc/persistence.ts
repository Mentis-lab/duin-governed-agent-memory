...
export function registerPersistenceHandlers(): void {
  ...
  ipcMain.handle('persistence:restoreFromBackup', async (_event, backupPath: unknown) => {
    ...
    try {
      const dbPath = join(app.getPath('userData'), 'lamprey.db')
      // F12: confine the renderer-supplied backupPath to the backups dir — the filename/health
      // checks alone would accept any backup-named SQLite anywhere on disk as a DB replacement.
      const backupsDir = join(app.getPath('userData'), 'backups')
      closeDb({ checkpoint: false })
      setPersistenceReadOnlyMode(false)
      const info = await restoreFromBackup(dbPath, backupPath, { allowedDir: backupsDir })
      return { success: true, data: info }
    } catch (err) {
      return { success: false, error: messageOf(err) ?? String(err) }
    }
