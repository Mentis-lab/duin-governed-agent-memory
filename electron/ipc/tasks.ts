...
import { setExecutorNotify } from '../services/executor/executor-notify'
...
export function broadcastAgentRunEvent(event: AgentRunNotifyEvent): void {
  ...
  // Wake any `wait_tasks` waiters: a tracked background fork changed state, which
  // shifts the owning conversation's task-graph node (running ↔ terminal). This
  // is the DUIN substitute for turn-control signalling — task waits ride the
  // agent-run notify bridge rather than turn events.
  try {
    notifyTaskChange({
    ...
    console.error('[tasks] task-change notify failed:', err)
  }
}
...
// Services may not import ipc, so the external executor reaches this fan-out through a seam.
setExecutorNotify(broadcastAgentRunEvent)
