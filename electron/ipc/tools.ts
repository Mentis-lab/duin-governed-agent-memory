...
export function registerToolsHandlers(): void {
  ...
  ipcMain.handle('tools:list', async () => {
    try {
      ...
      return { success: false, error: friendly(err, 'tools:list failed') }
    }
  })
  ...
  ipcMain.handle('tools:resolve', async (_event, names: string[]) => {
    try {
      ...
      return { success: false, error: friendly(err, 'tools:resolve failed') }
    }
  })
  ...
  ipcMain.handle(
    ...
    async (_event, payload: { query: string; maxResults?: number }) => {
      try {
        ...
        return { success: false, error: friendly(err, 'tools:search failed') }
      }
    }
  )
  ...
  ipcMain.handle('tools:getRecentCalls', async (_event, limit?: number) => {
    try {
      ...
      return {
        success: false,
        error: friendly(err, 'tools:getRecentCalls failed')
      }
    }
  })
  ...
  ipcMain.handle(
    ...
    async (_event, conversationId: string, limit?: number) => {
      try {
        ...
        return {
          success: false,
          error: friendly(err, 'tools:getCallsForConversation failed')
        }
      }
    }
  )
