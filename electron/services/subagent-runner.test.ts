...
describe('resolveAllowedTools', () => {
  ...
  it("returns '*' (not []) when parent is unknown and every layer is wildcard — " +
    "collapsing to [] here silently starved every '*'-typed agent (e.g. the " +
    "default \"general\" type) of every tool, since no production caller wires " +
    "deps.parentTools", () => {
    expect(resolveAllowedTools(null, '*', '*')).toBe('*')
  })
})
...
describe('forkAgent — happy paths', () => {
  ...
  it("doesn't expose the parent's added tool to the child if the type doesn't allow it", async () => {
    ...
    // Uses the REAL search ids: `grep_search`/`glob_search` were never registered
    // anywhere, so asserting the child received one was asserting a tool that could not
    // have run (backlog finding 51).
    const parentTools = ['read_file', 'search_files', 'glob_files', 'run_command', 'apply_patch']
    const handle = forkAgent(
    ...
    expect(observedAllowed).toContain('search_files')
  })

  it("does not starve a '*'-typed agent of every tool when deps.parentTools is unset " +
    "— matches production exactly: ipc/workflows.ts buildForkDeps() never sets " +
    "parentTools, and workflow-runner.ts defaults agentType to 'general' " +
    "(allowedTools: '*') when a workflow's agent() call doesn't specify a type", async () => {
    let observedAllowed: string[] | '*' | null = null
    const runner: ForkAgentRunner = async (input) => {
      observedAllowed = input.allowedTools
      return 'ok'
    }
    // No `parentTools` in deps at all — this is the production shape.
    const handle = forkAgent(
      { prompt: 'go', agentType: 'general' },
      makeDeps({ runner, loadType: builtinResolver() })
    )
    await handle.promise
    expect(observedAllowed).toBe('*')
  })
