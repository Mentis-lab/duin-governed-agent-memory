...
// shape. The model-driven summary now exists as A3
// (model-compaction.ts, DUIN_MODEL_COMPACTION=1): it runs post-turn as
// a prefix-extension request and persists through the same
// `persistCompressionSummary`; this deterministic path stays as the
...
export function compressOldestMessages(
  ...
  return persistCompressionSummary(conversationId, selection, summaryText, 'deterministic')
}

/**
 * Persist a compression summary transactionally: insert the summary as a
 * `role: 'system'` row placed just before the oldest compressed message
 * and stamp each original's `compressed_into`. Shared by the deterministic
 * path above and the model-driven path (model-compaction.ts, A3).
 *
 * The stamp is guarded with `AND compressed_into IS NULL`; if any selected
 * row was compressed by a concurrent run since selection, the whole
 * transaction rolls back and this returns null — the conversation already
 * got compressed, so dropping this summary is the correct outcome.
 */
export function persistCompressionSummary(
  conversationId: string,
  ...
  const originalTokens = selection.reduce((s, r) => s + estimateTokens(r.content), 0)
  const summaryTokens = estimateTokens(summaryText)
  const reductionPct = originalTokens > 0 ? 1 - summaryTokens / originalTokens : 0
  ...
  const markCompressed = db.prepare(
    'UPDATE messages SET compressed_into = ? WHERE id = ? AND compressed_into IS NULL'
  )

  const tx = db.transaction((rows: CompressorRow[]) => {
    summaryInsert.run(summaryId, conversationId, summaryText, summaryCreatedAt)
    for (const r of rows) {
      const res = markCompressed.run(summaryId, r.id)
      if (res.changes !== 1) {
        throw new Error(`row ${r.id} already compressed by a concurrent run`)
      }
    }
  })
  ...
  try {
    ...
    console.warn('[compressor] summary persist skipped:', err)
    return null
  }
...
export function getEffectiveMessages(conversationId: string): CompressorMessage[] {
  ...
  const rows = db
    .prepare(
      `SELECT id, conversation_id, role, content, model, tool_call_id,
              tool_calls, draft, reasoning, content_parts, created_at, compressed_into
         FROM messages
        WHERE conversation_id = ?
  ...
  return rows.map((r) => {
    ...
    if (r.tool_calls) {
      ...
      } catch (e) { console.debug('[context-compressor] drop corrupt JSON:', messageOf(e)) }
    }
    let contentParts: { type: 'image_url'; image_url: { url: string } }[] | undefined
    if (r.content_parts) {
      try {
        const parsed = JSON.parse(r.content_parts)
        if (Array.isArray(parsed) && parsed.length) contentParts = parsed
      } catch (e) { console.debug('[context-compressor] drop corrupt parts:', messageOf(e)) }
    }
    return {
      id: r.id,
      ...
      ...(toolCalls ? { toolCalls } : {}),
      // Vision attachments. Needed here because the `raw:` provider path rebuilds
      // its request from THESE rows — without them a persisted image would be
      // dropped on that path even though it survives on the brain path.
