...
toolRegistry.registerNative(
  {
    ...
    // `network`, not just `read`: web_find LOOKS like a read of an already-fetched
    // page, and that reading of the name is what hid this for so long — but on a
    // cache MISS `executeWebFind` fetches the model-supplied URL itself (it calls
    // `executeWebOpen`, web-tools.ts), and the LRU is keyed on the exact URL string,
    // so any novel URL misses. Nothing requires a prior web_open. Declaring only
    // 'read' silently disarmed all three risk-derived gates on that fetch — approval
    // (`GATING_RISKS`), the unattended CAP floor (`CAP_RISKS`) and the taint floor
    // (`EXFIL_OR_IRREVERSIBLE`) — leaving an unapproved, unfloored outbound GET to a
    // fully model-chosen URL in exactly the contexts where web_open is refused.
    risks: ['network', 'read'],
    requiresApproval: false,
    enabled: true,
    parallelizable: true
