...
describe.skipIf(!HAS_NATIVE_SQLITE)('failure ledger', () => {
  ...
  describe('recordFailure', () => {
    ...
    it('derives a stable fingerprint from kind + command + contractId + diffHash', () => {
      ...
      // >= not >: two back-to-back records can share a millisecond on a fast machine, and a
      // same-ms re-record legitimately keeps lastSeenAt equal — it must never REGRESS.
      expect(r2.lastSeenAt).toBeGreaterThanOrEqual(r1.lastSeenAt)
    })

    // The event->ledger sync re-asserts an all-time count for EVERY historical fingerprint on
    // every pass. The count was idempotent; lastSeenAt was not — it was stamped `now`, so any
    // pass re-dated failures that had stopped happening weeks earlier. That is what kept a
    // July incident reading as current (and as the top 'runaway' alarm) through late August.
    it('an authoritative re-sync with an UNCHANGED count does not re-date the failure', () => {
      const first = recordFailure(
        { kind: 'runtime_failed', fingerprint: 'runtime:x:y', message: 'x failed', count: 5 },
        db
      )
      const resync = recordFailure(
        { kind: 'runtime_failed', fingerprint: 'runtime:x:y', message: 'x failed', count: 5 },
        db
      )
      expect(resync.count).toBe(5)
      expect(resync.lastSeenAt).toBe(first.lastSeenAt)
    })

    it('…but a re-sync whose count GREW does advance it — that is a real recurrence', () => {
      const first = recordFailure(
        { kind: 'runtime_failed', fingerprint: 'runtime:grow:1', message: 'm', count: 5 },
        db
      )
      const grown = recordFailure(
        { kind: 'runtime_failed', fingerprint: 'runtime:grow:1', message: 'm', count: 6 },
        db
      )
      expect(grown.count).toBe(6)
      expect(grown.lastSeenAt).toBeGreaterThanOrEqual(first.lastSeenAt)
    })

    it('an explicit lastSeenAt wins — the sync knows the real event time', () => {
      const realTime = Date.now() - 40 * 24 * 60 * 60 * 1000
      recordFailure(
        { kind: 'runtime_failed', fingerprint: 'runtime:old:1', message: 'm', count: 1 },
        db
      )
      const synced = recordFailure(
        {
          kind: 'runtime_failed',
          fingerprint: 'runtime:old:1',
          message: 'm',
          count: 2,
          lastSeenAt: realTime
        },
      ...
      expect(synced.lastSeenAt).toBe(realTime)
    })
