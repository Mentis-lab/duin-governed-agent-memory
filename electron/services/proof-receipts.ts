...
// The separator group allows an optional quote (`["']?`) between the key name
// and the `:`/`=` so JSON-object members like `"api_key": "sk-..."` redact. The
// key character class [A-Z0-9_.-] deliberately excludes the quote character, so
// without this a closing key-quote sat between the key and the separator and
// silently aborted the match -- leaving secrets in the single most common
// structured-output shape ("token": "...") unredacted in stored previews.
const SECRET_ASSIGNMENT_PATTERN =
  /\b([A-Z0-9_.-]*(?:API[_-]?KEY|SECRET|TOKEN|PASSWORD|PASSWD|AUTHORIZATION|BEARER|PRIVATE[_-]?KEY|CLIENT[_-]?SECRET|REFRESH[_-]?TOKEN|ACCESS[_-]?TOKEN|CREDENTIAL|COOKIE|SESSION[_-]?ID)[A-Z0-9_.-]*)(\s*["']?\s*[:=]\s*)(["']?)(?:(?:Bearer|Basic|Digest|Token)\s+)?[^\s"',;}]+/gi
...
export function createProofReceipt(
  ...
  if (record.status === 'failed') {
    ...
    } catch (e) { console.debug('[proof-receipts] best-effort; ledger table may not exist yet:', messageOf(e)) }
  }
