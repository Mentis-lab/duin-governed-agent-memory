...
/** Reset control — the brain is about to REPLACE the visible answer body it has
 *  streamed so far (the answer-completeness re-generation: a tool-call preamble
 *  was streamed, then discarded server-side, and clean prose re-streamed). Without
 *  a client-honored reset the renderer appended the retry prose to the preamble →
 *  duplicated/garbled visible + persisted text. On receipt the renderer clears its
 *  streaming buffer and the bridge zeroes its accumulator so only the post-reset
 *  prose is shown and persisted. Reasoning/tool cards are left intact. */
...
/** Reviewable / reversible proposed-edit CARD. Fires when the model calls
 *  `propose_edit` (a new card) AND whenever a card's status changes via the
 *  `proposedEdit:*` IPC (accept / reject / edit) so any open chat view updates
 *  without polling. The full proposal row is carried so the renderer store can
 *  upsert by id — the same row is also persisted (proposed_edit_proposals), so
 *  a reload rehydrates via `proposedEdit:list`. */
export interface EditProposedPayload {
  conversationId: string
  proposal: {
    id: string
    conversationId: string
    ...
    anchors: Array<{ path: string; existed: boolean; sha256: string | null }>
    status: 'pending' | 'accepted' | 'rejected' | 'conflict' | 'error'
    result: string | null
    createdAt: number
    updatedAt: number
  }
...
// Emitted when the brain's render_artifact tool produces a validated artifact —
// the renderer opens it in the artifact panel (see useChat -> window.__openArtifact).
...
export interface ChatEventMap {
  'chat:artifact': ChatArtifactPayload
  ...
  'chat:edit-proposed': EditProposedPayload
...
function getMainWindow(): BrowserWindow | null {
  ...
  // Prefer a visible window — transient hidden windows (e.g. the artifact
  // validation render) must never intercept chat events.
  return windows.find((w) => !w.isDestroyed() && w.isVisible()) ?? windows[0] ?? null
}
