...
describe('BUILT_IN_SUBAGENT_TYPES', () => {
  ...
  it('gives Explore read-only tools', () => {
    ...
    // The REAL ids. This used to assert grep_search/glob_search, which the enforcement
    // ceiling (SUBAGENT_TOOLS) has never contained — so it was pinning a tool list that
    // left Explore with read_file and nothing else (backlog finding 51).
    expect(explore.allowedTools).toEqual(
      expect.arrayContaining(['read_file', 'search_files', 'glob_files'])
    )
