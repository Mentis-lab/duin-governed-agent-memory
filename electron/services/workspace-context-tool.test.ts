...
describe('inferVerificationCommands', () => {
  ...
  it('keeps typecheck/test/lint when the cap has to drop something', () => {
    // Declaration order puts the ones that matter LAST. Before ranking, the cap sliced
    // raw key order and threw them away -- and the same truncated list is the enforced
    // allow-list, so the model could neither see nor request them.
    const scripts: Record<string, string> = {}
    for (let i = 0; i < 12; i++) scripts[`test:shard-${i}`] = 'echo'
    scripts.lint = 'eslint .'
    scripts.test = 'vitest run'
    scripts.typecheck = 'tsc --noEmit'
    const cmds = inferVerificationCommands(root, { scripts })
    expect(cmds.length).toBe(8)
    expect(cmds[0]).toBe('npm run typecheck')
    expect(cmds).toContain('npm test')
    expect(cmds).toContain('npm run lint')
    // and the ranked ones come before the filler shards
    expect(cmds.indexOf('npm test')).toBeLessThan(cmds.indexOf('npm run lint'))
  })
})
