...
export async function getConnectionStatus(): Promise<GitHubConnectionStatus> {
  ...
  try {
    ...
    return {
      ...
      avatarUrl: null,
      installationId: null,
      reason: `Probe error: ${friendly(err, 'unknown')}`
    }
  }
}
 ...
 * default to "Connect with DUIN" or to the BYO form.
 */
...
export async function startOAuthLogin(input: StartOAuthLoginInput = {}): Promise<OAuthLoginResult> {
  ...
  const code = await new Promise<string>((resolveCode, rejectCode) => {
    const timeout = setTimeout(() => {
      try { server.close() } catch (e) { console.debug('[github-service] noop:', messageOf(e)) }
    ...
    server.on('request', (req, res) => {
      ...
      res.end('<html><body><h2>DUIN connected to GitHub.</h2><p>You can close this tab.</p></body></html>')
      clearTimeout(timeout)
      try { server.close() } catch (e) { console.debug('[github-service] noop:', messageOf(e)) }
...
async function tryBindCallbackServer(
  ...
  throw new Error(
    `All OAuth callback ports unavailable (${errors.join(', ')}). ` +
      `Quit any other DUIN instance and retry. If the problem persists, ` +
