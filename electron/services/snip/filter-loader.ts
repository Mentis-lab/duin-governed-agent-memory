...
function discoverFiles(dir: string): string[] {
  ...
  const walk = (d: string): void => {
    for (const entry of readdirSync(d)) {
      ...
      } catch (e) { console.debug('[filter-loader] skip unreadable:', messageOf(e)) }
    }
  }
  walk(dir)
  return out
...
function loadOne(path: string): void {
  try {
    ...
    // JSON_SCHEMA: filter YAML is plain data; never construct language types from watched files.
...
function broadcast(): void {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    } catch (e) { console.debug('[filter-loader] window may already be closing:', messageOf(e)) }
  }
}
...
export const __filterLoaderTest = {
  loadOneFromString: (path: string, contents: string): { filter?: Filter; error?: string } => {
    try {
      const parsed = yaml.load(contents, { schema: yaml.JSON_SCHEMA })
      const result = validateFilter(parsed)
