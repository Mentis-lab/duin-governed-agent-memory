...
export const ALL_WEB_SEARCH_PROVIDERS: ReadonlyArray<{
  ...
  /** Whether this adapter implements imageSearch(), i.e. whether the
   *  `image_search` TOOL can run at all while this provider is selected.
   *  The two providers that need no key are exactly the two without it, so a
   *  default install has image_search permanently dead — which used to show up
   *  only as a tool error mid-conversation. Settings reads this to say so. */
  supportsImages: boolean
}> = [
  { id: 'duckduckgo', label: 'DuckDuckGo · no key required (unreliable)', requiresKey: false, requiresEndpoint: false, supportsImages: false },
  { id: 'brave', label: 'Brave Search', requiresKey: true, requiresEndpoint: false, supportsImages: true },
  { id: 'tavily', label: 'Tavily', requiresKey: true, requiresEndpoint: false, supportsImages: true },
  { id: 'serpapi', label: 'SerpAPI', requiresKey: true, requiresEndpoint: false, supportsImages: true },
  { id: 'searxng', label: 'SearXNG', requiresKey: false, requiresEndpoint: true, supportsImages: true },
  { id: 'wikipedia', label: 'Wikipedia · no key required', requiresKey: false, requiresEndpoint: false, supportsImages: false }
]

/** Providers whose adapter implements image search, for "configure one of these" copy. */
export const IMAGE_CAPABLE_PROVIDERS: readonly WebSearchProviderId[] =
  ALL_WEB_SEARCH_PROVIDERS.filter((p) => p.supportsImages).map((p) => p.id)

export function providerSupportsImages(id: WebSearchProviderId): boolean {
  return ALL_WEB_SEARCH_PROVIDERS.find((p) => p.id === id)?.supportsImages ?? false
}
