...
export interface WorktreeContext {
  ...
  branch: string
  /** The commit the branch started at (worktree HEAD right after creation). finalize keeps the
   *  worktree when HEAD has moved past this — otherwise an agent that COMMITTED its work would
   *  leave a clean tree, and finalize would delete the worktree AND the branch, destroying the
   *  only copy of the commit. Absent on a context from before this field existed → commits-ahead
   *  is not checked (the pre-existing uncommitted-only behaviour). */
  baseSha?: string
}
...
export function createAgentWorktreeManager(opts: AgentWorktreeManagerOptions): WorktreeManager {
  ...
  async function create(runId: string): Promise<WorktreeContext> {
    ...
    // The branch's starting commit, so finalize can tell "the agent committed" (HEAD moved)
    // from "nothing happened" (HEAD unchanged). Best-effort: a repo with no commits yet has no
    // HEAD to resolve, and finalize simply falls back to the uncommitted-only check.
    const head = await runGit(['rev-parse', 'HEAD'], path).catch((): GitResult => ({ stdout: '', stderr: '', code: 1 }))
    const baseSha = head.code === 0 ? head.stdout.trim() || undefined : undefined
  ...
  async function finalize(ctx: WorktreeContext): Promise<FinalizeResult> {
    ...
    // "Changed" = an uncommitted working tree OR commits the agent made on its branch (HEAD moved
    // past baseSha). The second half is what stops committed work from being deleted with the
    // worktree: without it, `git add -A && git commit` inside the run leaves a clean tree, and the
    // branch — the only place the commit lives — would be `git branch -D`'d below.
    let changed = hasUncommittedChanges(status.stdout)
    if (!changed && ctx.baseSha) {
      const ahead = await runGit(['rev-list', '--count', `${ctx.baseSha}..HEAD`], ctx.path).catch(
        (): GitResult => ({ stdout: '', stderr: '', code: 1 })
      )
      // If we cannot tell, PRESERVE (a false "clean" here is the data-loss path).
      if (ahead.code !== 0 || Number(ahead.stdout.trim()) > 0) changed = true
    }
    if (changed) {
      return {
        keep: true,
    ...
    // Nothing produced — remove worktree + dedicated branch.
