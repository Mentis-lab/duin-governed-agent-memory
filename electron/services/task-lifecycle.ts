...
// Recoverable + destructive lifecycle operations on a task (= a conversation
// tree). DUIN adaptation:
//   - the graph is built from conversations + agent runs only (empty identity /
//     turn arrays);
//   - `close` / `restore` ride DUIN's new setConversationClosed, alongside the
//     archived/pinned setters DUIN already shipped;
//   - the delete cascade's auxiliary cleanup removes only agent_runs — DUIN has
//     no agent_identities table. The DeleteTaskPreview keeps identityIds /
//     turnIds fields (always empty) for shape parity with the tool + UI.
...
function productionDeleteAuxiliary(preview: DeleteTaskPreview): void {
  ...
  // DUIN: only agent_runs carry per-conversation aux rows in the graph. There is
  // no agent_identities table.
