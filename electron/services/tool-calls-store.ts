...
export function updateToolCall(
  ...
  params.push(id)
  // Same withWriteRetry as insertToolCall, and for the same reason. This is the write
  // that fires after EVERY tool execution and carries the terminal state — status,
  // result, error, duration. Without the retry a transient SQLITE_BUSY (past the
  // busy_timeout) left the row stuck at its 'running' insert: the Activity Timeline
  // shows a tool that never finished, and the policy-resolution path that consults
  // previous calls reads an outcome that never landed. The insert was hardened for
  // exactly this hazard; its terminal counterpart was not.
  withWriteRetry(() => {
