...
export function applyDarwinProfile(input: SandboxInput): SandboxOutput | null {
  ...
  return {
    // Emit the ABSOLUTE path the locator resolved, never the bare name.
    // WHY: the consumer (executeShellCommand) spawns this `cmd` with a child
    // env of `{ ...process.env, ...args.env }`, where `args.env` is
    // model-supplied. spawn() without `shell: true` resolves a bare file
    // against the CHILD's PATH, so a caller-supplied `env.PATH` would decide
    // which binary "enforces" the sandbox — and $TMPDIR is writable under our
    // own profile, so a shim named `sandbox-exec` can be dropped there and
    // will simply re-exec its trailing argv, discarding `-p <profile> --`.
    // Using the absolute path takes PATH resolution out of the loop.
    //
    // This was invisible because the availability probe and the exec disagreed
    // by construction: findOnPath reads the PARENT's process.env.PATH, so the
    // check always passed while the child resolved something else entirely —
    // and the result still reported sandboxTier 'darwin-sbx', so the very
    // field designed to surface an escape reported containment.
    cmd: sandboxExec,
