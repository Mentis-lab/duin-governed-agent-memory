...
describe('applyDarwinProfile (dispatcher)', () => {
  ...
  it('returns the wrapped invocation when sandbox-exec is on PATH', () => {
    ...
    // Tightened: the LOCATED ABSOLUTE PATH, not the bare name. See the
    // PATH-hijack regression test below.
  ...
  // ─── Regression: PATH-hijack of the sandbox wrapper itself ────────────
  //
  // The consumer (executeShellCommand) spawns `cmd` with a child env of
  // `{ ...process.env, ...args.env }` where `args.env` is model-supplied,
  // and spawn() without `shell: true` resolves a BARE cmd against the
  // CHILD's PATH. Emitting 'sandbox-exec' therefore let `env.PATH` choose
  // which binary "enforces" the sandbox — while the availability probe
  // (findOnPath) read the PARENT's PATH and happily said yes.
  it('emits the located absolute path, so the child PATH cannot select the wrapper', () => {
    __setSandboxExecLocatorForTest(() => '/opt/homebrew/bin/sandbox-exec')
    const result = applyDarwinProfile(baseInput)
    expect(result!.cmd).toBe('/opt/homebrew/bin/sandbox-exec')
    // The load-bearing property: never a bare name, always absolute.
    expect(result!.cmd).not.toBe('sandbox-exec')
    expect(isAbsolute(result!.cmd)).toBe(true)
  })
