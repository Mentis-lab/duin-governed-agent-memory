...
describe('applyLinuxProfile / buildBwrapArgs (S5)', () => {
  ...
  it('wraps the original cmd + args in a bwrap invocation', () => {
    ...
    expect(r!.cmd).toBe('/usr/bin/bwrap')
    expect(r!.sandboxTier).toBe('linux-bwrap')
  })

  // ─── Regression: PATH-hijack of the sandbox wrapper itself ────────────
  //
  // executeShellCommand spawns `cmd` with a child env of
  // `{ ...process.env, ...args.env }` where `args.env` is model-supplied,
  // and spawn() without `shell: true` resolves a BARE cmd against the
  // CHILD's PATH. Emitting 'bwrap' let `env.PATH` decide which binary
  // enforced the sandbox; buildBwrapArgs --binds the tmpdir rw, so a shim
  // could be dropped there. The tier still read 'linux-bwrap' either way.
  it('emits the located absolute path, so the child PATH cannot select the wrapper', () => {
    const r = applyLinuxProfile(baseInput(), () => '/opt/bin/bwrap', existsAll, TMPDIR)
    expect(r!.cmd).toBe('/opt/bin/bwrap')
    // The load-bearing property: never a bare name, always absolute.
    expect(r!.cmd).not.toBe('bwrap')
    expect(isAbsolute(r!.cmd)).toBe(true)
  })
