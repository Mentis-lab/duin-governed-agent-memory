...
export function applyLinuxProfile(
  ...
  return {
    // Emit the ABSOLUTE path the locator resolved, never the bare name — see
    // the matching comment in ./darwin.ts. executeShellCommand spawns this
    // `cmd` with a child env carrying the model-supplied `args.env`, and
    // spawn() resolves a bare file against the CHILD's PATH, so `env.PATH`
    // would select which binary enforces the sandbox. buildBwrapArgs `--bind`s
    // the tmpdir rw, so a shim named `bwrap` can be dropped there and re-exec
    // its trailing argv, dropping every isolation flag while the result still
    // reports sandboxTier 'linux-bwrap'.
    cmd: bwrap,
    args,
