...
describe('AskUserRuntime', () => {
  ...
  it('cancelForConversation leaves another live turn"s prompt alone', async () => {
    // Concurrent streams: conversation A and B each have an ask_user prompt in
    // flight. Stopping A must not answer B's question — chat:cancel used to
    // call cancelAll(), which resolved B with the `cancelled` sentinel and made
    // B's turn take a branch the user never picked.
    const { runtime } = makeRuntime()
    const a = runtime.ask({
      question: 'A?',
      header: 'A',
    ...
    expect(runtime.cancelForConversation('conv-a')).toBe(1)
    expect(runtime.size()).toBe(1)
    await expect(a).resolves.toEqual({ kind: 'cancelled' })

    // B is still pending and still answerable.
    expect(runtime.respond(runtime.list()[0]!.requestId, {
      kind: 'single',
      label: 'a',
      header: 'B'
    })).toBe(true)
    await expect(b).resolves.toEqual({ kind: 'single', label: 'a', header: 'B' })
  })

  it('cancelForConversation ignores asks that belong to no conversation', async () => {
    // Workflow / renderer-initiated asks carry no conversationId; no chat
    // cancel owns them.
    const { runtime } = makeRuntime()
    void runtime.ask({
      question: 'Workflow?',
    ...
    expect(runtime.cancelForConversation('conv-a')).toBe(0)
    expect(runtime.size()).toBe(1)
  })
