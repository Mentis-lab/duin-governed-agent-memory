...
toolRegistry.registerNative(
  {
    ...
    description:
      'Report the runtime dependencies available to DUIN: the embedded Node runtime version + path, an optional Python interpreter (probed from PATH), and a list of bundled helper script paths. Returns a JSON-encoded summary.',
    providerKind: 'native',
    providerId: 'internal',
    inputSchema: {
      type: 'object',
...
toolRegistry.registerNative(
  {
    ...
    title: 'Transition goal lifecycle',
    description:
      'Drive an operational goal through its lifecycle: start | pause | resume | block | ' +
      'complete | abort | clear | record_usage. The lifecycle is ' +
      'open -> active -> (paused|blocked) -> active -> completed|aborted. ' +
      'NOTE: the TERMINAL actions (complete, abort, clear) require EARNED authority — a ' +
      'model may only issue them once the ANS "goal-terminal-transition" capability has ' +
      'been promoted; otherwise the call errors and a human must confirm. Returns the ' +
      'updated goal as JSON (null for `clear`, which deletes the goal).',
    providerKind: 'native',
    providerId: 'internal',
    inputSchema: {
      type: 'object',
      properties: {
        goal_id: { type: 'string', description: 'Id of the goal to transition.' },
        action: {
          type: 'string',
          enum: [
            'start',
          ...
          description: 'The lifecycle action to apply.'
        },
        reason: { type: 'string', description: 'Optional human-readable reason for the transition.' },
        blocker: { type: 'string', description: 'Required for action=block: what is blocking the goal.' },
        completion: {
          type: 'string',
          description: 'Required for action=complete: a one-line summary of what was completed.'
        },
        tokens_used: {
          type: 'number',
          description: 'For action=record_usage: tokens consumed to add to the goal budget accrual.'
        },
        elapsed_ms: {
          type: 'number',
          description: 'For action=record_usage: elapsed ms to add to the goal time accrual.'
        }
      },
  ...
  async (args, ctx) => {
    const a = args as Record<string, unknown>
    if (typeof a.goal_id !== 'string' || a.goal_id.length === 0) {
      throw new Error('transition_goal: "goal_id" is required.')
    }
    if (typeof a.action !== 'string') {
      throw new Error('transition_goal: "action" is required.')
    }
    // A model-issued transition; terminal actions are routed through the ANS
    // capability-ledger authority gate inside transitionGoal (throws when unearned).
