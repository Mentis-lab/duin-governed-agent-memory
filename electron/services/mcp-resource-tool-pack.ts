// mcp-resource-tool-pack.ts — native, lazy, read-only tools that expose the MCP
// Resources surface (list/read/templates) to the agentic executors. Each tool
// preserves exact server provenance, binds pagination cursors to a single
// server, and returns JSON.stringify(...) so it inherits DUIN's existing chat
// spill valve (maybeSpillToolResult) with no per-tool wiring. mutates:false —
// resources are read-only; never gated behind approval.
