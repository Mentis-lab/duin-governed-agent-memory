...
// Background-shell plumbing, not kernel confinement: pin the sandbox kill switch
// so these tests do not depend on a win32 restricted token, which cannot
// initialise inside a vitest worker (0xC0000142) though it runs clean from a real
// node/PowerShell parent. Same rationale as shell-tool.test.ts.
