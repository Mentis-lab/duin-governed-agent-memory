...
toolRegistry.registerNative(
  {
    ...
    description:
      'Record the outcome of the current in-progress backlog task. Use this when you finish the task the loop handed you, so the progress ledger stays accurate and the work is not repeated. The loop itself closes the task out once your work is durably committed and verified.',
    providerKind: 'native',
    providerId: 'internal',
    inputSchema: {
      type: 'object',
  ...
  async (args, ctx) => {
    ...
    return JSON.stringify(
      applyLoopCompleteTask(productionLoopToolStore(), ctx.conversationId, String(args.result ?? ''))
    )
  }
)
  ...
  async (args, ctx) => {
    ...
    return JSON.stringify(
      applyLoopControl(productionLoopToolStore(), ctx.conversationId, action, {
        ...
        // applyLoopControl takes a minDelaySeconds and the production handler never
        // supplied one, so every loop_control{delaySeconds} was floored at the hardcoded
        // MIN_LOOP_DELAY_SECONDS instead of the operator's configured minIntervalSeconds.
        // Raising that setting changed nothing for the model's own reschedules.
        minDelaySeconds: readLoopConfig().minIntervalSeconds,
        now: Date.now()
      })
    )
  }
