...
describe('resolveDecisionFromPolicies — allow requires full gating-risk coverage', () => {
  // Regression for the co-occurring-risk leak: a risk-subject ALLOW used to
  // resolve the whole call the instant ctx.risks.includes(p.subject) was true
  // for ANY one risk, so a grant for a single risk silently authorized every
  // other, never-granted risk the same multi-risk tool also carries.

  it('does NOT allow apply_patch (write+destructive) off a "write_workspace" grant that only covers write', () => {
    // Exactly the exploit shape: request_permissions(scope='write_workspace')
    // persists ONLY a 'write' risk policy (native-aux-tools.ts SCOPE_RISKS) —
    // nothing ever grants 'destructive'. apply_patch declares
    // risks: ['write', 'destructive'] (apply-patch-tool-pack.ts). Before the
    // fix this returned {decision:'allow'} because 'write' alone matched;
    // the call must instead fall through to the modal (null here).
    const policies = [
      makePolicy({
        id: 'global-write',
        scope: 'global',
  ...
  it('does NOT allow browser_click (destructive+write+network) off a lone network grant', () => {
    const policies = [
      makePolicy({
        id: 'global-network',
        scope: 'global',
  ...
  it('allows once every gating risk the call carries has its own allow, even split across levels', () => {
    const policies = [
      makePolicy({
        id: 'conv-network',
        scope: 'conversation',
    ...
    expect(decision).toEqual({ decision: 'allow', policyId: 'conv-network' })
  })

  it('an explicit tool-subject allow still resolves the call regardless of risk coverage', () => {
    // A named-tool grant is already complete, precise consent — it must not
    // be held to the same per-risk coverage bar as a risk-subject grant.
    const policies = [
      makePolicy({
        id: 'global-apply-patch',
        scope: 'global',
        subjectKind: 'tool',
        subject: 'apply_patch',
        decision: 'allow'
    ...
    expect(decision).toEqual({ decision: 'allow', policyId: 'global-apply-patch' })
  })

  it('a non-gating risk (write-only) still resolves off its own single-risk allow', () => {
    // No gating risk is in play at all, so the coverage requirement is
    // vacuous — this must behave exactly as it did before the fix.
    const policies = [
      makePolicy({
        id: 'global-write',
        scope: 'global',
    ...
    expect(decision).toEqual({ decision: 'allow', policyId: 'global-write' })
  })
})
