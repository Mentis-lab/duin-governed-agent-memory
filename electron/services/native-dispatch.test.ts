...
describe('dispatchNativeTool', () => {
  ...
  // R4 (Phase-4) — the per-tool wall-clock timeout + abort at the seam.
  it('a hung handler becomes a synthetic error row after the timeout (never hangs)', async () => {
    const out = await dispatchNativeTool(
      () => new Promise<string>(() => {}), // never settles
      { timeoutMs: 20, toolName: 'hung_tool' }
    )
    expect(out.status).toBe('error')
    expect(out.result).toContain('hung_tool')
    expect(out.result).toContain('20ms')
  })

  it('an aborted handler becomes a synthetic error row', async () => {
    const ctrl = new AbortController()
    const p = dispatchNativeTool(() => new Promise<string>(() => {}), {
      signal: ctrl.signal,
      timeoutMs: 10_000,
    ...
    const out = await p
    expect(out.status).toBe('error')
    expect(out.result).toContain('cancelled_tool')
  })

  it('a fast handler under the timeout is unaffected', async () => {
    const out = await dispatchNativeTool(async () => 'Saved to memory.', {
      timeoutMs: 1000
    })
    expect(out.status).toBe('done')
