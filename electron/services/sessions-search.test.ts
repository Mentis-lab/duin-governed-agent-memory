...
describe('E3 — sessions sidebar buckets + FTS search', () => {
  ...
  // FTS5 reads -, : and ^ as syntax. Before these were escaped, every such query raised
  // "no such column" inside searchSessions, was swallowed by its catch, and reached the user
  // as an empty result set — indistinguishable from a genuine no-match.
  it.skipIf(!nativeOk())('searches punctuated text without falling back to empty', () => {
    const a = convStore.createConversation('deepseek-chat')
    convStore.updateConversationTitle(a.id, 'Punctuation probe')
    convStore.saveMessage({
      id: 'msg-punct',
      conversationId: a.id,
      role: 'user',
      content: 'Deploy duin-brain on 2026-08-08 with ratio 3:1 — see safeStorage notes.'
    })

    expect(convStore.searchSessions('duin-brain').length).toBeGreaterThan(0)
    expect(convStore.searchSessions('2026-08-08').length).toBeGreaterThan(0)
    expect(convStore.searchSessions('3:1').length).toBeGreaterThan(0)

    // Syntax callers already rely on must survive escaping: an explicit phrase, a trailing-*
    // prefix search, and the uppercase OR that agui-grounding builds its recall query from.
    expect(convStore.searchSessions('"safeStorage"').length).toBeGreaterThan(0)
    expect(convStore.searchSessions('duin*').length).toBeGreaterThan(0)
    expect(convStore.searchSessions('"duin-brain" OR "nothingmatchesthis"').length).toBeGreaterThan(0)

    // A genuine miss must still be an empty result, not a swallowed parse error.
    expect(convStore.searchSessions('absent-token-9912')).toEqual([])
  })
})
