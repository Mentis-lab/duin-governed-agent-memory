...
async function fetchPageBytes(url: string): Promise<{ html: string }> {
  ...
  try {
    ...
    try {
      await reader.cancel()
    } catch (e) { console.debug('[web-tools] ignore:', messageOf(e)) }
...
export async function executeImageSearch(args: ImageSearchArgs): Promise<string> {
  ...
  if (!adapter.imageSearch) {
    // Name the remedy. "DuckDuckGo does not support image search" is true and
    // useless — the operator cannot tell from it that ANY keyless provider has
    // the same gap, or which ones do not.
    return `Error: ${adapter.label} has no image search endpoint, so image_search cannot run. Switch the provider in Settings → Web Tools to one of: ${IMAGE_CAPABLE_PROVIDERS.join(', ')}.`
  }
