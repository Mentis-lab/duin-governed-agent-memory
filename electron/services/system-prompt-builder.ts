...
import { resolveToneDirective } from './agent-tones'
import { readSettings } from './settings-helper'
import { renderLanguageDirective } from './local-brain/language-directive'
import type { LanguageChoice } from '../shared/chat-send-contract'
import { FALLBACK_TOOL_INSTRUCTION } from './fallback-tool-parser'
...
const CONTRACT_SECTIONS: ContractSection[] = [
  {
    ...
    bullets: [
      ...
      'Use create_document for a NEW artifact the user keeps — a plan, draft, report, or code file. One call per file with an accurate mimeType; do not also paste the body inline. To change an existing workspace file, edit it (apply_patch) or propose the change (propose_edit) — not create_document.',
      'Default to propose_edit over apply_patch when the user should review a change before it lands (the norm for non-coders): it writes nothing, shows a diff card with Apply / Discard / Edit, and only their Apply applies the patch atomically. Use apply_patch directly only with standing approval to edit.',
      'Never refuse a "test" or throwaway request: use your tools (apply_patch writes files, shell_command runs the terminal) and act — do not defer to the user.',
      ...
      'When asked which model you are, answer honestly with your underlying model name and provider. DUIN is the harness, not the model.'
    ]
  },
...
export const COMPOSER_SYSTEM = [
  'You are the final-response composer for a DUIN agent run.',
...
function identityHead(modelId?: string): string {
  ...
  if (modelId) {
    ...
    return (
      `You are ${desc.name} (served by ${providerLabel}), the model powering DUIN — a ` +
      `second-brain agent for a knowledge worker. You help them think and act on their own ` +
      `work: capture and connect knowledge, surface insights, foresee what's at risk, weigh ` +
      `decisions, and produce real outputs — always grounded in THEIR notes, decisions, ` +
      `people, and projects, citing what you draw on. When asked which model you are, answer ` +
      `honestly with your underlying model name and provider — DUIN is the interface, not the ` +
      `model. Be concrete and grounded; when a tool exists, use it. (You can also write and ` +
      `verify code when the task is technical.)`
    )
  }
  return (
    `You are the model powering DUIN — a second-brain agent for a knowledge worker. You help ` +
    `them capture and connect knowledge, surface insights, foresee risk, weigh decisions, and ` +
    `produce real outputs, grounded in THEIR notes, decisions, people, and projects. When asked ` +
    `which model you are, answer honestly with your underlying model name and provider — DUIN is ` +
    `the interface, not the model. Be concrete and grounded; when a tool exists, use it. (You can ` +
    `also write and verify code when the task is technical.)`
  )
}
...
export function buildSystemPrompt(
  ...
  // The `<retrieved_context>` block from augmentForChat (RAG). The inter-block
  // order comment above has named this slot since D2, but the parameter never
  // existed — so even had a caller wanted to inject retrieved chunks, there was
  // nowhere to put them. Appended last to keep every existing positional caller
  // valid; the RENDER position below is what the documented order governs.
  ...
  // Voice/tone: a user-chosen persona directive (warm / concise / caveman / …),
  // injected right after the identity so it colors the whole reply. Empty for
  // the default 'balanced' voice.
  const settings = readSettings()
  const voice = resolveToneDirective(
    settings.agentTone as string | undefined,
    settings.agentToneCustom as string | undefined
  )
  if (voice) parts.push(`<voice>\n${voice}\n</voice>`)

  // Response-language directive — the raw/headless counterpart to agui-grounding's floor-tier unit.
  // The DEFAULT chat path is /agui; this covers the raw:-bypass and headless-runner paths, which
  // never see the composer. Read from the same persisted `language` setting; 'auto'/unset → no
  // block → byte-identical to today.
  const lang = settings.language
  const language = lang === 'en' || lang === 'zh' || lang === 'ja' ? (lang as LanguageChoice) : undefined
  if (language) parts.push(`<language>\n${renderLanguageDirective(language)}\n</language>`)
  ...
  // Retrieved context sits directly above chapters, per the locked order
  // (memory_index → … → retrieved_context → chapters → conversation).
  if (retrievedContextBlock && retrievedContextBlock.trim()) {
    parts.push(retrievedContextBlock.trim())
  }
  ...
  for (const skill of activeSkillContents) {
    // Customize C3: when the skill declares a tool list, surface it as an
    // attribute rather than leaking it into the body.
    //
    // 'suggested-tools', NOT 'allowed-tools'. This is advisory: no gate reads
    // it (see SKILL_FIELD_DISPOSITION.allowedTools in shared/skill-contract.ts
    // — agui-gate.resolveAguiGate and the subagent tool filter both ignore it).
    // The brain path already renamed it for exactly that reason; this path kept
    // the old wording, so the two renderers described the same field to the
    // model in contradictory terms — one of them promising enforcement that
    // does not exist. Wording that implies a gate is worse than no attribute.
    const attrs = [`name="${skill.name}"`]
    if (skill.allowedTools && skill.allowedTools.length) {
      attrs.push(`suggested-tools="${skill.allowedTools.join(',')}"`)
    }
    if (lazySkillBodies) {
  ...
  if (supportsNativeTools) {
    result = result
      ...
      .replace(/\n{3,}/g, '\n\n')
  } else if (supportsNativeTools === false) {
    // ...and when it explicitly does NOT, tell it the JSON contract it is expected to
    // emit. fallback-tool-parser exports FALLBACK_TOOL_INSTRUCTION for exactly this, and
    // parseFallbackToolCalls parses exactly that shape at both call sites (ipc/chat.ts,
    // agui-dispatch.ts) — but nothing in production ever injected the instruction. So a
    // model configured supportsTools:false was parsed for a contract it had never been
    // told about: tools were unreachable for that entire model class, with nothing
    // saying why.
    //
    // `=== false` rather than a bare else: the parameter is optional and `undefined`
    // means "not stated" (subagent/loop callers never pass it). Only an explicit false
    // is a model we know lacks native calling.
    result = `${result}

${FALLBACK_TOOL_INSTRUCTION}`.trimEnd()
  }
...
function slimIdentityHead(modelId?: string): string {
  if (modelId) {
    ...
    return (
      `You are ${desc.name} (served by ${providerLabel}), the model powering DUIN — a ` +
      `second-brain agent for a knowledge worker. Be honest about which underlying model you are.`
    )
  }
  return (
    `You are the model powering DUIN — a second-brain agent for a knowledge worker. ` +
