...
describe('runWorkflow — pipeline + parallel', () => {
  it('3-stage pipeline runs concurrently across stages without stage barriers', async () => {
    ...
    // Pinned, like the concurrency-cap case below, rather than inheriting
    // defaultConcurrencyCap(). The default is `cpus().length - 2`, so on a
    // 2-core host it collapses to 1 and this test fails with "expected 1 to be
    // greater than 1" — a host-size failure, not a code failure. 3 is
    // deliberately NON-BINDING here: the pipeline carries 3 items and each item
    // holds at most one agent slot at a time, so 3 is the natural ceiling. This
    // test is about stages OVERLAPPING, not about the cap; the cap itself is
    // covered by 'enforces the configured cap' below.
    const handle = runWorkflow({ script, concurrencyCap: 3 }, { forkSeam: seam })
    const result = await handle.promise
  ...
  it('parallel is a barrier — all thunks resolve before continuation runs', async () => {
    ...
    // Pinned for the same reason as the pipeline case above: with the inherited
    // default cap this test serialises on a 2-core host and `order` comes back
    // ['A','B','C'] instead of ['B','C','A'] — the staggered delays can only be
    // observed if all three thunks are in flight together. 3 thunks, cap 3, so
    // the cap is non-binding and the assertion measures the barrier, not the host.
...
//
// runWorkflow attaches an internal `promise.finally(...)` chain to fire the
// workflowFinished hooks. `.finally()` returns a NEW promise that re-rejects
// with the run's error, so voiding it abandoned that rejection: every failing
// or aborted workflow raised an unhandledRejection in the main process even
// though the caller correctly awaited/caught the handle's own promise.
// ---------------------------------------------------------------------------

describe('runWorkflow — no unhandled rejections', () => {
  async function collectUnhandled(fn: () => Promise<void>): Promise<unknown[]> {
    ...
    try {
      await fn()
      // Unhandled rejections are reported on a later macrotask turn — give the
      // abandoned promise (if any) a chance to surface before we assert.
  ...
  it('a caught workflow failure does not surface an unhandledRejection', async () => {
    const script = `${META}
      throw new Error('boom from the workflow body')
    `
    const seam = makeSeam(async () => 'never')
    const seen = await collectUnhandled(async () => {
      await expect(runWorkflow({ script }, { forkSeam: seam }).promise).rejects.toThrow(
        /boom from the workflow body/
      )
    })
    expect(seen).toEqual([])
  })

  it('an aborted workflow does not surface an unhandledRejection', async () => {
    const script = `${META}
    ...
    const seen = await collectUnhandled(async () => {
      const handle = runWorkflow({ script }, { forkSeam: seam })
      setTimeout(() => handle.abort('user'), 20)
      await expect(handle.promise).rejects.toBeInstanceOf(WorkflowAbortError)
    })
    expect(seen).toEqual([])
  })

  it('a failing nested workflow() child does not surface an unhandledRejection', async () => {
    // The child runs through the same runWorkflow, so it gets its own
    // internal finally-chain — the exact path the flaky suite tripped on.
    const childScript = `${META}
      throw new Error('child exploded')
    `
    const parentScript = `${META}
    ...
    const seen = await collectUnhandled(async () => {
      await expect(
        runWorkflow(
          { script: parentScript },
          { forkSeam: seam, loadNamedWorkflow: () => childScript }
        ).promise
...
describe('runWorkflow — sandbox guards', () => {
  ...
  it('does NOT pollute host Math.random when blocking it in the sandbox', async () => {
    // (1) host Math.random works before running any workflow
    const before = Math.random()
    expect(typeof before).toBe('number')
    expect(before).toBeGreaterThanOrEqual(0)
    expect(before).toBeLessThan(1)

    // (2) run a workflow whose body touches Math.random inside a try/catch so
    // the block is exercised but the workflow still completes normally.
    const script = `${META}
      let blocked = false
      try { Math.random() } catch (e) { blocked = /is blocked/.test(String(e && e.message)) }
      return blocked
    `
    const seam = makeSeam(async () => 'never')
    const result = await runWorkflow({ script }, { forkSeam: seam }).promise
    // (4) inside the sandbox, Math.random still throws "is blocked"
    expect(result.output).toBe(true)

    // (3) host Math.random STILL returns a number in [0,1) after the workflow ran
