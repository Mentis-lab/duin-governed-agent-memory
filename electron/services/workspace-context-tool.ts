...
export function inferVerificationCommands(
  ...
  // De-dup, then RANK before capping.
  //
  // The cap used to slice raw package.json key order, and the same truncated list is the
  // enforced allow-list for verify_workspace — so on a repo with many matching scripts
  // (this one has 33) whichever verification commands happened to be declared late were
  // both hidden from the model AND refused if it asked for them. Losing `typecheck` or
  // `test` to declaration order is the worst possible way to spend eight slots.
  //
  // Ranking by what a verification pass actually needs first means the cap now drops the
  // least important commands rather than the most recently added ones.
  ...
  return deduped.slice().sort((x, y) => verifyRank(x) - verifyRank(y)).slice(0, VERIFY_CMD_CAP)
}

/** Lower sorts first. Ties keep their original relative order (Array#sort is stable). */
function verifyRank(cmd: string): number {
  // Rank the SCRIPT NAME, not the whole command line. `npm run test:shard-3` contains the
  // word "test" too, so a loose match lets a dozen shards tie with -- and by declaration
  // order outrank -- plain `npm test`. When the cap bites, one `npm test` is worth more
  // than twelve shards of it, so sub-variants rank below the primary they derive from.
  const script =
    cmd === 'npm test' ? 'test' : cmd.startsWith('npm run ') ? cmd.slice(8).trim() : cmd
  const s = script.toLowerCase()
  if (/^(typecheck|type-check|type:check)$/.test(s)) return 0
  if (/^npx tsc\b/.test(s)) return 1
  if (s === 'test') return 2
  if (s === 'lint') return 3
  if (/^(check|verify)$/.test(s)) return 4
  // `type:check:app` too — VERIFY_SCRIPT_PATTERNS and haveTypecheckScript both
  // accept that spelling, and without it here it falls to the catch-all and sorts
  // BELOW lint:/test: variants, which is the drop this ranking exists to prevent.
  if (/^type[-:]?check[:.]/.test(s)) return 5
  if (/^test[:.]/.test(s)) return 6
  if (/^lint[:.]/.test(s)) return 7
  // `verify:all` / `check:*` had no tier and fell to the catch-all, sorting BELOW ten
  // lint: shards — so on this very repo (16 matching scripts, cap 8) the full gate was
  // the thing the cap dropped. Same failure the ranking exists to prevent, one prefix over.
  if (/^(verify|check)[:.]/.test(s)) return 8
  return 9
}
...
export async function buildProofContext(input: {
  ...
  try {
    ...
    notes.push(`proof active contract unavailable: ${messageOf(err) ?? String(err)}`)
  }
  try {
    recent =
    ...
    notes.push(`proof receipts unavailable: ${messageOf(err) ?? String(err)}`)
  }
