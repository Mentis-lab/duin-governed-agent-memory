...
export interface AskUserQuestion {
  ...
  timeoutMs?: number
  /** Owning chat turn, when the ask came from one. Lets `chat:cancel` resolve
   *  ONLY the cancelled conversation's pending question — with concurrent
   *  streams a blanket cancelAll() answered another live turn's prompt with the
   *  `cancelled` sentinel, silently derailing it. Absent for workflow /
   *  renderer-initiated asks, which no chat cancel owns. */
...
export class AskUserRuntime {
  ...
  /** Cancel only the questions raised by one conversation's turn. `chat:cancel`
   *  uses this instead of cancelAll(): concurrent streams mean another
   *  conversation can have its own prompt on screen, and answering it with the
   *  `cancelled` sentinel makes that unrelated turn take a phantom "user
   *  cancelled" branch. Entries with no conversationId (workflow / renderer
   *  asks) are never owned by a chat cancel and are left alone. */
