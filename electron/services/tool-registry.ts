...
// One definition of "this call gates on its risks" — see
// `isParallelizableDescriptor`. Safe direction: permissions-store imports this
// module for TYPES only, and tool-registry already loads it at runtime through
// native-aux-tools, so this adds an edge that already existed, not a cycle.
import { shouldGateOnRisks } from './permissions-store'
...
export interface LampreyToolDescriptor {
   ...
   * true, the dispatcher refuses to parallelize if the descriptor would hit
   * the approval gate (`requiresApproval` OR a gating risk — network /
   * destructive / secret / sandboxBypass) or carries the `write` risk — see
   * `isParallelizableDescriptor` below for the authoritative predicate. So
   * setting it on an approval-gated tool (web_search) is harmless but inert
   * until a future change makes concurrent approvals safe. Set this on
   * read-only tools (workspace_context, view_image, etc.); leave it off for
   * state-mutating tools even when they don't gate (memory_add, update_plan).
   */
 ...
 *   2. Descriptor would not be routed to an approval modal — neither by its
 *      `requiresApproval` flag NOR by a gating risk (modal races between
 ...
 *
 * `read` risks are fine — parallel reads are the motivating case.
 */
export function isParallelizableDescriptor(
  ...
  // Condition 2 previously tested ONLY the `requiresApproval` flag, and that
  // read as complete because approval LOOKS declared. It isn't: the
  // authoritative dispatch-time predicate is `descriptorNeedsApproval`, which
  // ALSO gates on the risk array (`shouldGateOnRisks` — network, destructive,
  // secret, sandboxBypass). Every network tool ships `requiresApproval: false`
  // with `parallelizable: true` (web_search, web_open, image_search,
  // finance_quote, weather_lookup, sports_lookup, the mcp_resource reads), so
  // the fan-out fired two approval requests at once. The renderer holds a
  // SINGLE approval slot (`approvalRequest` in src/App.tsx), so the second
  // request overwrote the first; the dropped one stayed in
  // `PermissionsService.pending`, which has no timeout by design, and the
  // turn's Promise.allSettled never settled — the chat hung until Stop.
  // We mirror `shouldGateOnRisks` rather than calling `descriptorNeedsApproval`
  // on purpose: that predicate exempts `selfApproves` tools, whose own handler
  // pops the very modal this condition exists to keep unique.
  if (shouldGateOnRisks(descriptor.risks)) return false
...
/**
 * MCP server ids whose tools actuate the OS / desktop (mouse / keyboard / shell) — the
 * computer-use executors. For these, classification is FAIL-CLOSED: any tool that is not
 * explicitly `readOnlyHint:true` is treated as destructive + approval-gated. The write-verb
 * heuristic below misses bare actuation names (`click`, `type`, `press_key`, `run_command`),
 * so it must NOT be the only guard for a desktop actuator. Kept in sync with the
 * 'Computer Use' entries in `src/data/connectors-catalog.ts`.
 */
export const COMPUTER_USE_SERVER_IDS = new Set(['terminator', 'computer-use', 'windows-mcp', 'desktop'])

/**
 * MCP server ids whose tools EXECUTE CODE (a REPL / interpreter / shell-adjacent runtime).
 * Same fail-closed rule as computer-use: anything not explicitly `readOnlyHint:true` is
 * destructive + approval-gated.
 *
 * Why the verb heuristic cannot cover these: the bundled Node REPL's tools are named `js`,
 * `js_reset`, `js_add_node_module_dir` — no verb in MCP_MUTATING_VERB matches any of them, so
 * all three classified as plain network reads and AUTO-RAN under the default `trusted-afk`
 * interactive posture (2026-08-14 estate-audit escalation; unattended runs were still refused
 * by the CAP risk floor). A code-exec tool's NAME carries no signal about what the code does,
 * so name-based classification is structurally the wrong guard — the server id is the signal.
 * The bundled server now also self-declares destructiveHint on its eval tools; this set is the
 * defence that survives a server whose annotations are lost, stripped, or never written.
 */
export const CODE_EXEC_SERVER_IDS = new Set(['node-repl'])

// Write/actuation/exec verbs in an MCP tool name that imply a mutating action. Extends the
// original write-verb set with GUI-actuation and shell-exec verbs so un-annotated servers
// (and any stray computer-use tool) still get gated.
const MCP_MUTATING_VERB =
  /(delete|remove|destroy|send|create|update|write|post|insert|drop|archive|revoke|cancel|publish|deploy|merge|upload|rename|edit|reply|append|overwrite|move|click|type|press|keypress|scroll|drag|paste|launch|activate|execute|exec|shell|command|kill|terminate|mouse|keyboard|tap|swipe)/i
...
/**
 * Classify an MCP tool's risk (pure — unit-tested in tool-registry.test.ts). Priority:
 *  1. `readOnlyHint:true` → read-only (ungated).
 *  2. a computer-use OR code-exec server → fail-closed: anything not read-only is destructive.
 *  3. `destructiveHint`, the Chrome destructive set, or a mutating/actuation-verb name → destructive.
 *  4. otherwise → network-only (a plain read/query against a remote server).
 */
export function classifyMcpTool(
  ...
  const isDestructive =
    !readOnly &&
    (COMPUTER_USE_SERVER_IDS.has(serverId) ||
      CODE_EXEC_SERVER_IDS.has(serverId) ||
      annotations?.destructiveHint === true ||
      (serverId === 'chrome' && CHROME_DESTRUCTIVE.has(toolName)) ||
      MCP_MUTATING_VERB.test(toolName))
  const risks: ToolRisk[] = isDestructive ? ['destructive', 'write', 'network'] : ['network']
  return { risks, requiresApproval: isDestructive, mutates: isDestructive }
}
  ...
  getDescriptors(): LampreyToolDescriptor[] {
    ...
    for (const { serverId, tools } of mcpGroups) {
      for (const tool of tools) {
        ...
        // Risk-classify each MCP tool so writes are gated (was chrome-only,
        // leaving every other server's delete/send tool ungated). Priority:
        //   1. MCP annotations (readOnlyHint / destructiveHint) — the spec signal.
        //   2. the hardcoded Chrome destructive set (Chrome sends no annotations).
        //   3. a write-verb heuristic on the tool name (fallback for un-annotated
        //      servers). Read tools stay ungated; anything mutating gets approval.
        const { risks, requiresApproval, mutates } = classifyMcpTool(
          serverId,
          tool.name,
          tool.annotations
        )
...
toolRegistry.registerNative(
  {
    ...
    title: 'Shell command',
    description: [
      'Run a one-shot shell command inside the DUIN workspace.',
      '',
      'PLATFORM SHELL:',
      ...
      '',
      'BACKGROUND PROCESSES: pass `run_in_background: true` to start a long-running process and return immediately with `{ processId, pid, status }` instead of waiting for it to finish. Manage it with `shell_list` / `shell_monitor` / `shell_output` / `shell_stop`.',
      '',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        run_in_background: {
          type: 'boolean',
          description:
            'Start this command as a detached background shell and return immediately with { processId, pid, status } instead of waiting for it to finish. Manage it with shell_monitor / shell_list / shell_stop / shell_output.'
        }
      },
      required: ['command'],
      additionalProperties: false
  ...
  async (args, ctx) => {
    ...
    // S8 gap-close: shell_monitor/shell_list/shell_stop/shell_output
    // (native-aux-tools.ts) have always read shell-tool.ts's bgSessions
    // registry, but this handler only ever called the blocking
    // executeShellCommand — the only writer to that registry was
    // ipc/monitor.ts's non-model IPC channel, so the model had no way to
    // populate a session for the aux tools to manage. Branch into the
    // same registry here instead of adding a second tool; requiresApproval
    // above still gates this call like any other shell_command invocation.
    if (args.run_in_background === true) {
      const handle = executeShellCommandInBackground(shellArgs, workspaceRoot)
      return {
        result: JSON.stringify({ processId: handle.id, pid: handle.pid, status: handle.status }),
        status: handle.status === 'failed' ? 'error' : 'done'
      }
    }
