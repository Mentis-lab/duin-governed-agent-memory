...
describe('renderContract', () => {
  ...
  it('CR-1: includes the canonical project vocabulary (STS / P-SPR / Stem to Stern)', () => {
    const out = renderContract()
    expect(out).toContain('STS')
    expect(out).toContain('P-SPR')
    // Re-baselined 2026-08-21 (backlog C5 executed): 'Bucket' LEFT the
    // vocabulary when scripts/bucket.ps1 was retired — the bullet told every
    // agent turn to run a pipeline CLAUDE.md forbids. The absence is now the
    // pinned state; retired-sidecar.test.ts holds the file-level half.
...
describe('buildSystemPrompt — default base', () => {
  it('includes the honest-identity sentence', () => {
    ...
    expect(out).toContain('DUIN is the interface, not the model')
  })
  ...
  //
  // RE-BASELINED 2026-07-24 to 3,760 (measured 3,687) for the upstream-adoption
  // propose_edit port: the reviewable-edit workflow (create_document for new
  // artifacts vs propose_edit for review-before-apply on existing files) is a
  // deliberate DUIN feature that MUST be described to the model or it stays dark.
  // The two "How you work" bullets were trimmed first (saved ~141 bytes); the
  // remaining ~380-byte growth is the irreducible signal for a genuinely new
  // capability, not bloat. Kept tight (~73 bytes headroom) so the next addition
  // still has to be a conscious act. If this trips again, re-measure and decide —
  // do not reflexively raise it, and do not delete it.
  it('renders under the UB-8 size target (< 3,760 bytes, re-baselined 2026-07-24 for propose_edit)', () => {
    const out = renderContract()
    expect(out.length).toBeLessThan(3760)
  })
...
describe('Lampshade L9 — envelope shape guards', () => {
  ...
  //
  // RE-BASELINED 2026-07-21 to 4,050 (measured 3,974). The guard was RED, and had been for a
  // while: 244d87b (identity-head rewrite) and 50c23c9 — whose message claims "byte-neutral" —
  // pushed it 74 bytes over the 3,900 floor. It stayed red long enough to stop being read as a
  // signal, which is exactly how the next prompt addition would have landed unnoticed.
  //
  // The floor is re-baselined, NOT deleted, and kept deliberately tight (~76 bytes of headroom):
  // its whole value is that a prompt addition has to be a conscious act. If this trips again,
  // re-measure and decide — do not reflexively raise it, and do not delete it.
  //
  // RE-BASELINED 2026-07-24 to 4,480 (measured 4,408) for the upstream-adoption propose_edit
  // port — same conscious decision as the UB-8 renderContract guard above (bullets trimmed first,
  // remaining growth is the load-bearing propose_edit / create_document signal). ~72 bytes headroom.
  it('size: coding-mode single-agent prompt stays under 4,480 bytes (re-baselined 2026-07-24 for propose_edit)', () => {
    const out = buildSystemPrompt([], '', undefined, undefined, undefined, 'coding')
    expect(out.length).toBeLessThan(4480)
  })
...
describe('buildSystemPrompt — supportsNativeTools strips the <think> bullet (L3)', () => {
  ...
  // The inter-block order comment has named `retrieved_context` since D2, but
  // the parameter did not exist — so the RAG block had no slot in the prompt at
  // all. Pin both the emission and its documented position.
  it('emits the retrieved_context block above chapters', () => {
    const out = buildSystemPrompt(
      [{ name: 'test-skill', content: 'skill body' }],
      '<memory>fact</memory>',
      undefined,
      ...
      undefined,
      undefined,
      '<retrieved_context>\n  <source id="1" name="spec.pdf" page="4">retry budget is 3</source>\n</retrieved_context>'
    )
    expect(out).toContain('<retrieved_context>')
    expect(out).toContain('retry budget is 3')
    const notifyIdx = out.indexOf('<task-notifications>')
    const retrievedIdx = out.indexOf('<retrieved_context>')
    const chaptersIdx = out.indexOf('<chapters>')
    expect(retrievedIdx).toBeGreaterThan(notifyIdx)
    expect(chaptersIdx).toBeGreaterThan(retrievedIdx)
  })

  it('omits the retrieved_context slot when retrieval produced nothing', () => {
    const out = buildSystemPrompt(
      [],
      '',
      undefined,
    ...
    expect(out).not.toContain('<retrieved_context>')
  })
})
...
describe('buildSystemPrompt — override path', () => {
  ...
  it('treats a whitespace-only override as absent', () => {
    const out = buildSystemPrompt([], '', '   \n\t  ')
    expect(out).toContain('DUIN is the interface')
...
describe('buildAgentSystemPrompt (multi-agentic primitive)', () => {
  ...
  it('uses the slim identity head, not the full contract (L5)', () => {
    const out = buildAgentSystemPrompt('coder')
    expect(out).toContain('a second-brain agent for a knowledge worker')
...
describe('HY6 — exemplar-based steering (CR-7 added the reviewer exemplar)', () => {
  ...
  it('keeps both exemplars inside <contract> and stays under the size guard', () => {
    ...
    // RE-BASELINED 2026-07-24 to 3,760 — same renderContract envelope + rationale
    // as the UB-8 guard above (propose_edit port). Kept in lockstep with it.
    expect(out.length).toBeLessThan(3760)
  })
})
...
describe('buildSystemPrompt — models without native tool calling get the contract', () => {
  const build = (supportsNativeTools?: boolean): string =>
    buildSystemPrompt([], '', undefined, undefined, 'm', undefined, undefined, undefined, undefined, supportsNativeTools)

  it('injects the fallback JSON contract when the model explicitly lacks native calling', () => {
    // parseFallbackToolCalls parses this exact shape at both call sites, but nothing in
    // production ever injected the instruction — so a supportsTools:false model was being
    // parsed for a contract it had never been told about, and tools were unreachable for
    // that whole model class with nothing saying why.
    const out = build(false)
    expect(out).toContain('"action"')
    expect(out).toContain('"final"')
  })

  it('does NOT inject it for a native-tool model', () => {
    expect(build(true)).not.toContain('You do not have native function calling')
  })

  it('does NOT inject it when the caller never states it', () => {
    // undefined means "not stated" (subagent/loop callers), not "lacks native calling".
    expect(build(undefined)).not.toContain('You do not have native function calling')
  })
})
