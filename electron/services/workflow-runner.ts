...
export function runWorkflow(input: WorkflowRunInput, deps: WorkflowRunnerDeps): WorkflowRunHandle {
  ...
  // F3 (A6) — user hooks see workflow lifecycle.
  // Fire-and-forget, but never abandoned: fireHooks only guards its own hook
  // listing, so a throwing shell/js hook would reject this promise with nobody
  // attached (an unhandledRejection in the main process).
  void fireHooks('workflowStarted', { trigger: 'workflow', sourceId: runId }).catch((err) => {
    console.error('[workflow-runner] workflowStarted hooks failed (continuing):', err)
  })
  ...
  const promise = (async (): Promise<WorkflowRunResult> => {
    ...
    try {
      ...
      sandbox.JSON = JSON
      // Sandbox-local Math: prototype-chains to host Math so all methods/constants remain,
      // but with an OWN random() that throws — shadows WITHOUT mutating the shared host Math
      // (vm shares object refs; mutating Math.random here poisoned the whole main process).
      const sandboxMath = Object.create(Math) as typeof Math
      sandboxMath.random = () => { throw new Error('workflow: Math.random() is blocked inside workflows; vary inputs by index') }
      sandbox.Math = Object.freeze(sandboxMath)
      ...
      const wrapped = `
        (async () => {
          ...
          // Math.random is blocked at the sandbox level (frozen sandbox-local Math), not mutated here.
          ${parsed.body}
        })()
      `
  ...
  // `.finally()` returns a NEW promise that re-rejects with the run's error.
  // Voiding that derived promise abandons the rejection, so every failed or
  // aborted workflow raised an unhandledRejection even though the caller
  // correctly awaited/caught `promise` itself. Terminate the hook chain with
  // its own catch: `promise` below is returned untouched and still rejects
  // for the caller — only this internal fire-and-forget branch is settled.
  void promise
    .finally(() => void fireHooks('workflowFinished', { trigger: 'workflow', sourceId: runId }))
    .catch(() => {})
