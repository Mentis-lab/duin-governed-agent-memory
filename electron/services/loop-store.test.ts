...
describe('LP-2 loop-store CRUD', () => {
  // Tracks the migration registry head rather than a literal: database.ts throws on any drift
  // between the two, so pinning a number here only re-stales the moment a migration lands.
  it.skipIf(!nativeOk())('migrations bring the schema to the registry head', () => {
