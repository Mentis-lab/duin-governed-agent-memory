...
import { ipv4FirstFetch } from './ipv4-first-fetch'
...
async function fetchJsonWithTimeout(
  ...
  try {
    // ipv4FirstFetch, not bare fetch: open-meteo's geocoding host advertises an
    // AAAA record that black-holes on some networks, and the default address
    // selection sits on it for ~10s before giving up. See ipv4-first-fetch.ts.
    const res = await ipv4FirstFetch(url, { ...init, signal: controller.signal })
    if (!res.ok) {
    ...
    return { ok: false, error: friendly(err, 'Network error') }
  } finally {
    clearTimeout(timer)
  }
}
...
export async function executeWeatherLookup(args: WeatherLookupArgs): Promise<string> {
  ...
  if (settings.weatherProvider === 'open-meteo') {
    ...
    if (parsed) {
      ...
      // Name the bypass on a NETWORK failure too, not just on a no-match. The
      // forecast host and the geocoding host are different machines, so
      // geocoding can be unreachable while the forecast API is perfectly fine —
      // in which case passing "lat,lon" skips the broken half and works. The
      // old message reported the raw transport error and pointed nowhere.
      if (!geoRes.ok) {
        return `Error: weather_lookup could not reach open-meteo geocoding (${geoRes.error}). Retry with "lat,lon" (e.g. "31.22,121.46") to skip the geocoding step — the forecast API is a separate host and may still be reachable.`
      }
