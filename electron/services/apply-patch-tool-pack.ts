import { toolRegistry } from './tool-registry'
import { executeApplyPatch, type ApplyPatchArgs } from './apply-patch-tool'
import { readSettings } from './settings-helper'

// The workspace root defaults to the vault (workspace-state vaultWorkspaceFallback)
// and may also be a folder inside it, so apply_patch routinely targets
// hand-authored notes. Hand the executor the vault path so its destructive
// branches tombstone into <vault>/.trash — the same recovery surface delete_file
// and the renderer's Delete button use — instead of a per-workspace one.
function vaultDir(): string | undefined {
  try {
    const d = readSettings().localBrainNotesDir
    return typeof d === 'string' && d.trim() !== '' ? d : undefined
  } catch {
    return undefined
  }
}

toolRegistry.registerNative(
  {
    id: 'apply_patch',
    name: 'apply_patch',
    title: 'Apply patch',
    description:
      'Apply a structured patch envelope to the workspace. Supports add, update, and delete file directives. The patch payload must start with "*** Begin Patch" and end with "*** End Patch"; each file block is "*** Add File: <path>" (body lines start with "+"), "*** Update File: <path>" (optional "@@ <context>", then "+"/"-"/" " body lines per hunk), or "*** Delete File: <path>" (no body). All paths must resolve inside the workspace root. Returns a per-file summary or an error explaining why the patch was rejected.',
    providerKind: 'native',
    providerId: 'internal',
    inputSchema: {
      type: 'object',
      properties: {
        patch: {
          type: 'string',
          description:
            'The full patch envelope including "*** Begin Patch" header and "*** End Patch" footer. Each file block is "*** Add File: <path>" (body lines start with "+"), "*** Update File: <path>" (optional "@@ <context>", then "+"/"-"/" " body lines per hunk), or "*** Delete File: <path>" (no body). All paths must resolve inside the workspace root.'
        }
      },
      required: ['patch'],
      additionalProperties: false
    },
    risks: ['write', 'destructive'],
    requiresApproval: true,
    enabled: true
  },
  async (args, ctx) => {
    const workspaceRoot = ctx.workspacePath ?? process.cwd()
    const { result } = await executeApplyPatch(
      args as unknown as ApplyPatchArgs,
      workspaceRoot,
      { vaultDir: vaultDir() }
    )
    return result
  }
)
