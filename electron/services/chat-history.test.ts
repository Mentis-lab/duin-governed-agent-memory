...
describe('buildApiMessagesFromStoredMessages', () => {
  ...
  it('compression summaries reach the model; other stored system rows stay invisible', () => {
    // Regression (review 2026-08-15): summaries are stored as system rows
    // standing in for hidden history. Skipping them made every compaction
    // silent history DELETION — the effective view already dropped the
    // originals, and the summary was dropped here.
    const api = buildApiMessagesFromStoredMessages('system', [
      {
        role: 'system',
        content: '<conversation_summary>\nCompressed 12 earlier messages.\n</conversation_summary>'
      },
      { role: 'system', content: 'renderer-facing note that must NOT ship' },
      { role: 'user', content: 'so where were we?' }
    ])

    expect(api.map((m) => m.role)).toEqual(['system', 'system', 'user'])
    expect(api[1].content).toContain('Compressed 12 earlier messages.')
    expect(JSON.stringify(api)).not.toContain('renderer-facing note')
  })

  it('a summary row mid-stream flushes a pending tool block before inserting', () => {
    const api = buildApiMessagesFromStoredMessages('system', [
      { role: 'user', content: 'q' },
      { role: 'assistant', content: '', toolCalls: [toolCall('call-a')] },
      { role: 'tool', content: 'A done', toolCallId: 'call-a' },
      { role: 'system', content: '<conversation_summary>S</conversation_summary>' },
