...
describe('toolRegistry lazy surface (integration)', () => {
  ...
  // Gate finding F6 — the meta-tool's own description tells the model to search
  // by what it wants to do ("take a screenshot", "generate an image", ...).
  // Phrased as a natural sentence, the 1-character token "a" scored +3 against
  // every tool whose NAME merely contains the letter, twice over, and pushed
  // browser_screenshot — which has no "a" in its name — out of the top 8.
  it('resolves natural task phrasings, not just keyword-dense ones', async () => {
    await import('./tool-packs')
    const { toolRegistry } = await import('./tool-registry')

    // The exact query from the gate report.
    expect(
      toolRegistry.resolveToolSearch('take a screenshot of a web page').map((m) => m.name)
    ).toContain('browser_screenshot')

    // The other phrasings the meta-tool description recommends verbatim.
    expect(
      toolRegistry.resolveToolSearch('generate an image').map((m) => m.name)
    ).toContain('image_generate')
    expect(
      toolRegistry.resolveToolSearch('run sub-agents in parallel').map((m) => m.name.length)
    ).not.toHaveLength(0)
  })
