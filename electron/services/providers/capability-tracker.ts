...
const TOOL_LIKE_PATTERNS = [
  ...
  /<terminal>/i,
  /\{"action"\s*:\s*"/i
  // Removed the fenced-code pattern (```bash|sh|...): a markdown code fence is a
  // normal answer — the model DISPLAYING code, not attempting a tool call. It
  // false-flagged ordinary coding conversations and, after a few turns, permanently
  // downgraded tools for the rest of the conversation.
]
