...
// Master kill-switch check, resilient to an unreadable settings file / missing app
// context (default: NOT disabled — hooks are on unless the user explicitly turns them
// off; the real RCE boundary is create-time approval in ipc/hooks).
function hooksHardDisabled(): boolean {
  try {
    return readSettings().enableHooks === false
  } catch {
    return false
  }
}
...
export interface HookContext {
  ...
  promptBody?: string
  cwd?: string
  // F3 — autonomy lifecycle context (loop/automation/workflow events).
  /** What triggered the turn: 'loop' | 'automation' | 'workflow'. */
  trigger?: string
  /** The triggering entity's id (loopId / automationId / workflow run id). */
  sourceId?: string
  /** Human label of the triggering entity. */
  label?: string
}
...
function runJsHook(
  ...
  try {
    ...
    return { thrown: String(messageOf(err) ?? err) }
  }
}
...
function runShellHook(hook: Hook, event: HookEvent, ctx: HookContext): void {
  try {
    ...
    proc.on('error', (err) => {
      console.error(`[hook ${event}:${hook.label}] error:`, messageOf(err))
    })
    proc.on('exit', (code) => {
      if (code && code !== 0) {
    ...
    console.error(`[hook ${event}:${hook.label}] spawn failed:`, messageOf(err))
  }
}
...
export async function fireHooks(
  ...
  // Master kill-switch (default ON; only an explicit `false` disables). The RCE
  // boundary is create-time approval in ipc/hooks — a persisted hook only exists
  // because the user approved a native prompt when it was authored.
  if (hooksHardDisabled()) return { blocked: false, logs }
...
export function testHook(input: {
  ...
  // Master kill-switch (default ON). Create-time approval in ipc/hooks gates the
  // test path; this only blocks when the user has hard-disabled hooks.
  if (hooksHardDisabled()) {
    return {
      thrown: 'Hooks are disabled (enableHooks:false). Re-enable hooks in Settings to test them.',
      logs: []
    }
  }
