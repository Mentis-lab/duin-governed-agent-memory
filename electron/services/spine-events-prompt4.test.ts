...
// Mock runHeadlessAgent so automations tests don't reach a real provider.
//
// This used to mock chatOnce. The runner migrated to the tool-capable agent loop
// (automations-runner.ts: "A cron job is a REAL tool-capable agent now, not a text-only
// completion") and the mock was never updated — which nobody caught, because this whole suite
// could not LOAD (a transitive electron CJS import) from the day it was written until 2026-07-21.
//
// NOTE the contract difference that the stale mock hid: chatOnce THREW on failure, runHeadlessAgent
// CATCHES and RETURNS {status:'error'|'aborted', error}. That is why the failure assertions below
// were failing against real behaviour rather than a mocking artifact.
const runHeadlessAgentMock = vi.fn<
  (spec: { prompt: string; model: string; label?: string }) => Promise<{
...
vi.mock('./headless-agent', () => ({
  runHeadlessAgent: (spec: { prompt: string; model: string; label?: string }) =>
    runHeadlessAgentMock(spec)
}))
...
describe('automation runner emits automation.* events with a per-run correlationId', () => {
  it('successful run emits started + completed sharing one correlationId', async () => {
    ...
    // The runner reads .output off the HeadlessAgentResult.
    runHeadlessAgentMock.mockResolvedValueOnce({
      status: 'ok',
      output: 'the briefing',
      turns: 1,
      toolUses: []
    ...
    // The old assertion here checked that chatOnce received an `audit` object carrying this run's
    // correlationId, so its model.request.* events joined the timeline. runHeadlessAgent takes no
    // audit parameter, so that thread genuinely no longer exists on this path — dropped rather
    // than weakened into something that passes without testing anything. The correlationId
    // grouping that survives IS covered, by the two assertions directly above.
    expect(runHeadlessAgentMock.mock.calls[0][0].prompt).toBe('Brief me')
    expect(runHeadlessAgentMock.mock.calls[0][0].model).toBe('deepseek-v4-pro')
  ...
  it('failed run emits started + failed with severity error and errorPreview', async () => {
    ...
    // NOT mockRejectedValueOnce: runHeadlessAgent catches everything and RETURNS an error
    // status. Reproducing a thrown error here would test a path the runner never sees.
    runHeadlessAgentMock.mockResolvedValueOnce({
      status: 'error',
      output: '',
      turns: 0,
      toolUses: [],
      error: 'upstream provider 500'
    })
    ...
    // With no output, last_result records the failure as a NOTE — deliberately not
    // `[error] upstream provider 500`, which the old format produced. That string was
    // stored as the automation's answer and, with a delivery target set, sent to the
    // operator's channel as though the automation had said it. The worst case was a
    // budget-exhausted run, which reported status 'ok' and therefore recorded the
    // literal `[ok] stopped: hit maxToolCalls (24)` as its reply.
    expect(recordRunMock).toHaveBeenCalledWith('auto-2', '(no answer — upstream provider 500)')
  })

  it('an ERROR status is a failure, not a completion (the regression this suite caught)', async () => {
    automationsListMock.mockReturnValue([
      { id: 'auto-3', label: 'Toolfail', cron: '*/5 * * * *', prompt: 'p', model: null, enabled: 1, createdAt: 0 }
    ])
    // The agent produced partial output AND failed. Before 2026-07-21 the runner emitted
    // automation.completed here (severity 'info'), because it only ever inspected result.status to
    // build a display string — so a failed run was invisible to watchJobFailed and to the
    // event-log mining in capability-gap.
    runHeadlessAgentMock.mockResolvedValueOnce({
      status: 'error',
      output: 'partial work',
      turns: 3,
    ...
    await runAutomation('auto-3')

    const events = listEvents({ automationId: 'auto-3', order: 'asc' })
    expect(events.map((e) => e.type)).toEqual(['automation.started', 'automation.failed'])
    expect(events[1].severity).toBe('error')
    expect((events[1].payload as { errorPreview: string }).errorPreview).toContain('tool exploded')
    expect((events[1].payload as { errorClass: string }).errorClass).toBe('HeadlessAgentError')
  })

  it('an ABORTED run is failed but classed apart from an error', async () => {
    automationsListMock.mockReturnValue([
      { id: 'auto-4', label: 'Slow', cron: '*/5 * * * *', prompt: 'p', model: null, enabled: 1, createdAt: 0 }
    ])
    // A timeout/cancellation is not a completion, but it is also not a fault worth paging on —
    // hence a distinct errorClass rather than folding it into 'error'.
    runHeadlessAgentMock.mockResolvedValueOnce({ status: 'aborted', output: '', turns: 1, toolUses: [] })
    await runAutomation('auto-4')

    const events = listEvents({ automationId: 'auto-4', order: 'asc' })
    expect(events.map((e) => e.type)).toEqual(['automation.started', 'automation.failed'])
    expect((events[1].payload as { errorClass: string }).errorClass).toBe('HeadlessAgentAborted')
  })

  it('unknown automation id → no events, no agent call', async () => {
    automationsListMock.mockReturnValue([])
