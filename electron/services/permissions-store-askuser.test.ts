...
describe('askUser — modal round-trip', () => {
  ...
  it('"just this once" allow resolves as modal and persists no policy', async () => {
    ...
    expect(outcome).toEqual({ decision: 'allow', source: 'modal', scope: 'once' })
  ...
  it('a denial is reported as modal with no persisted policy', async () => {
    ...
    const outcome = await promise
    expect(outcome).toEqual({ decision: 'deny', source: 'modal', scope: 'once' })
    expect(listPolicies()).toHaveLength(0)
  })
...
describe('askUser — no timeout, explicit cancellation only', () => {
  ...
  it('never auto-denies, no matter how long the user is away', async () => {
    ...
    expect(final).toEqual({ decision: 'allow', source: 'modal', scope: 'once' })
    expect(settled).toEqual({ decision: 'allow', source: 'modal', scope: 'once' })
  })
...
// R7 (Phase-4) — cancel-aware approvals: an AFK modal can be cancelled instead
// of deadlocking the tool loop.
describe('askUser — abort-aware (Phase-4)', () => {
  it('an abort while pending resolves the approval as a one-time deny', async () => {
    const ctrl = new AbortController()
    const req = makeReq({ signal: ctrl.signal, conversationId: 'conv-abort' })
    const promise = permissionsService.requestApprovalDetailed(req)
    // The modal was dispatched...
    expect(h.sent.some((e) => e.channel === 'tools:approvalRequired')).toBe(true)
    ctrl.abort()
    const outcome = await promise
    expect(outcome.decision).toBe('deny')
    expect(outcome.source).toBe('aborted')
    expect(listPolicies()).toHaveLength(0) // abort never persists a policy
  })

  it('an already-aborted signal denies immediately without popping a modal', async () => {
    const ctrl = new AbortController()
    ctrl.abort()
    const req = makeReq({ signal: ctrl.signal, conversationId: 'conv-pre' })
    const outcome = await permissionsService.requestApprovalDetailed(req)
    expect(outcome).toEqual({ decision: 'deny', source: 'aborted' })
    expect(h.sent.some((e) => e.channel === 'tools:approvalRequired')).toBe(false)
  })

  it('the AbortSignal is stripped before crossing the IPC boundary', async () => {
    const ctrl = new AbortController()
    const req = makeReq({ signal: ctrl.signal, conversationId: 'conv-strip' })
    const promise = permissionsService.requestApprovalDetailed(req)
    const sent = h.sent.find((e) => e.channel === 'tools:approvalRequired')
    expect(sent).toBeTruthy()
    expect((sent!.payload as Record<string, unknown>).signal).toBeUndefined()
    ctrl.abort()
    await promise
  })
})

describe('cancelPendingForConversation (Phase-4)', () => {
  it('cancels only the in-flight approvals of the target conversation', async () => {
    const a = makeReq({ conversationId: 'conv-A' })
    const b = makeReq({ conversationId: 'conv-B' })
    const pa = permissionsService.requestApprovalDetailed(a)
    const pb = permissionsService.requestApprovalDetailed(b)

    const cancelled = permissionsService.cancelPendingForConversation('conv-A')
    expect(cancelled).toBe(1)

    const outA = await pa
    expect(outA.decision).toBe('deny')

    // conv-B is still pending — answer it to confirm it was untouched.
    permissionsService.respond({ callId: b.callId, decision: 'allow', scope: 'once' })
    const outB = await pb
    expect(outB.decision).toBe('allow')
  })

  it('returns 0 when no approval matches the conversation', () => {
    expect(permissionsService.cancelPendingForConversation('nobody-home')).toBe(0)
  })
})
