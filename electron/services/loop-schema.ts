...
export const LOOP_SCHEMA_SQL = `
  CREATE TABLE IF NOT EXISTS loops (
    ...
    -- Goal-owned loop bridge (migration v36). Fresh installs get these here; the
    -- migration's ALTER then no-ops (duplicate-column swallowed) on pre-v36 DBs.
    goal_id TEXT,
    goal_conversation_id TEXT,
  ...
  CREATE TABLE IF NOT EXISTS loop_backlog (
    ...
    task TEXT NOT NULL,
    -- No SQL CHECK on status: the valid set (incl. 'awaiting-ratification', the
    -- governor 4a held-output state) is enforced by the BacklogStatus TS union.
    -- Mirrors the v19/v20 "drop the brittle CHECK" precedent; migration v25 rebuilds
    -- existing installs to match.
