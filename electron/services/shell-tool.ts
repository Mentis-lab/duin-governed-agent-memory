...
export function executeShellCommand(
  ...
  return new Promise((resolveResult) => {
    ...
    let cwd = resolveCwdWithinWorkspace(workspaceRoot, cwdCandidate)

    // Self-heal a stale session cwd. The persisted entry is anchored to the
    // workspace that was active when the last `cd` ran; nothing invalidates it
    // when the user switches workspace (files:setWorkdir → setActiveWorkspace).
    // After a switch the executor is called with a DIFFERENT workspaceRoot, so
    // the old sessionCwd points outside the new tree → resolves to null → this
    // conversation's every subsequent no-cwd shell_command would error forever
    // without ever spawning. Because the caller didn't pin an explicit cwd,
    // drop the stale entry and fall back to the (new) workspace root instead of
    // bricking. Only reached when args.cwd is absent — an explicit out-of-tree
    // cwd is still a genuine error handled below.
    if (cwd === null && args.cwd === undefined && conversationId && sessionCwd !== null) {
      cwdSessions.delete(conversationId)
      cwd = resolveCwdWithinWorkspace(workspaceRoot, undefined)
    }
    ...
    if (args.dangerously_disable_sandbox === true) {
      ...
      const wrapped = applyProfile({
        ...
        // Operator allowlist — see sandbox/operator-write-paths.ts. Empty by default.
        opts: { workspaceRoot, fsWritePaths: operatorWritePaths() }
      })
      cmd = wrapped.cmd
    ...
    try {
      ...
      resolveResult({
        ...
        durationMs: Date.now() - startedAt,
        timedOut: false,
        error: friendly(err, 'spawn failed'),
        sandboxTier,
        sandboxNote
      })
      return
    ...
    // across two chunk boundaries decodes to replacement characters when each chunk is
    // converted independently — and CJK output is entirely ordinary here (a git log, a
    // filename, any localised tool message), so this corrupted real output rather than
    // being a theoretical edge.
    const outDec = new StringDecoder('utf8')
    const errDec = new StringDecoder('utf8')
    proc.stdout?.on('data', (buf: Buffer) => {
      const r = appendCapped(stdoutBuf, outDec.write(buf), STDOUT_CAP)
      stdoutBuf = r.next
      stdoutTruncated = stdoutTruncated || r.truncated
    })
    proc.stderr?.on('data', (buf: Buffer) => {
      const r = appendCapped(stderrBuf, errDec.write(buf), STDERR_CAP)
      stderrBuf = r.next
    ...
    proc.on('error', (err: Error) => finish(null, null, messageOf(err)))
...
export function executeShellCommandInBackground(
  ...
  try {
    ...
    const session: BackgroundSession = {
      ...
      signal: null,
      stdout: '',
      stderr: friendly(err, 'spawn failed'),
      bytesWritten: 0,
      stdoutLineBuf: '',
      stderrLineBuf: ''
    }
  ...
  // Same stateful-decoder treatment for the background/session path.
  const sessOutDec = new StringDecoder('utf8')
  const sessErrDec = new StringDecoder('utf8')
  proc.stdout?.on('data', (buf: Buffer) => {
    const chunk = sessOutDec.write(buf)
  ...
  proc.stderr?.on('data', (buf: Buffer) => {
    const chunk = sessErrDec.write(buf)
    session.bytesWritten += chunk.length
  ...
  proc.on('error', (err: Error) => {
    ...
    session.stderr = (session.stderr + '\n' + messageOf(err)).slice(-STDERR_CAP)
  })
