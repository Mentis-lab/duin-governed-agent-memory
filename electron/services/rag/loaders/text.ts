...
// Markdown is the vault/Brain's NATIVE format — it belongs in the Explorer,
// not the RAG Library (ingesting it even duplicates it back into the vault via
// the library-brain sidecar bridge). The Library ingest path uses this to skip
// markdown and steer the user to the Brain instead.
export function isMarkdownExtension(pathOrName: string): boolean {
  const e = extname(pathOrName).toLowerCase()
  return e === '.md' || e === '.mdx'
}
