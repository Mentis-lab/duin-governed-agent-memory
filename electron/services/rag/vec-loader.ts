...
export function loadSqliteVec(db: Database.Database): void {
  try {
    ...
    // In a packaged Electron app, require.resolve hands back the asar VIRTUAL
    // path for the native vec0.dll (…/app.asar/node_modules/…). LoadLibrary can't
    // load a DLL from inside an asar → Windows error 126 ("找不到指定的模块"),
    // which silently disabled vector search in every build. Rewrite to the real
    // unpacked path before loadExtension. Falls back to the package's own load()
    // in dev (no asar) or if the helper is absent.
    const loadable = sqliteVec.getLoadablePath?.()
    if (loadable && /app\.asar[\\/]/.test(loadable)) {
      db.loadExtension(loadable.replace(/app\.asar([\\/])/, 'app.asar.unpacked$1'))
    } else {
      sqliteVec.load(db)
    }
