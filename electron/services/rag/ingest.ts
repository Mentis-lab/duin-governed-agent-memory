...
export interface IngestFile {
  ...
  /** Re-derive INTO this existing document row instead of inserting a new
   *  one. Set by the reingest handler; see the reuse branch in runOneFile. */
...
export interface EmbeddingsLike {
  ...
  /** Embed in a NAMED embedding space. Ingest must write a collection's vectors
   *  in the space that collection is stamped with, not in whatever the process
...
/** Fired when a document finishes ingesting into the library (status 'ready').
 *  Carries the full extracted text so a consumer can bridge the doc into the
 *  brain (P1: library doc → vault sidecar note → graph node). */
export interface DocumentReadyInfo {
  ...
  sourcePath?: string
  mime: string
  /** Full extracted text (pages joined for paged docs). */
  text: string
}

export interface IngestManagerDeps {
  embeddings: EmbeddingsLike
  /** Optional bridge — called (best-effort) when a doc reaches 'ready'. Kept
   *  optional + injected so the RAG service stays decoupled from the vault. */
  onDocumentReady?: (doc: DocumentReadyInfo) => void
}
...
export class IngestManager extends EventEmitter {
  ...
  private async runOneFile(
    ...
    // 2. Reingest re-derives into its own row; everything else dedupes by hash.
    //
    // A reingest MUST land back on the same document id: the table
    // conversation_rag_attachments stores a bare document_id with no FK, so
    // forking a new row leaves a document-scoped attachment pointing at an id
    // nothing indexes any more — it keeps rendering as a chip in the chat
    // context bar while silently retrieving nothing.
    //
    // Dropping the old chunks belongs here rather than in the IPC handler:
    // the handler runs long before this async job does, so deleting there
    // would leave the row chunkless (but still 'queued') for the whole ingest
    // and permanently so if it failed. This is also why the hash dedupe below
    // cannot double as the reset — it only short-circuits on 'ready', and a
    // reingest is by definition asking for a re-derive of a row it just moved
    // off 'ready'.
    let doc: RagDocument
    const replacing = file.replaceDocumentId
      ? getDocument(file.replaceDocumentId)
      : null
    if (replacing) {
      deleteChunksForDocument(replacing.id)
      doc =
        updateDocument(replacing.id, {
          status: 'loading',
          statusDetail: null,
          chunkCount: 0,
          // Carry the freshly-read source metadata onto the row — the file may
          // well have changed on disk, which is the usual reason to reingest.
          hashSha256,
          bytes,
          mtime
        }) ?? replacing
    ...
    // Embed in the COLLECTION's space, not the process-global active embedder.
    //
    // `rag_chunk_vec` is one shared physical index and every collection is stamped
    // with the `embedder_id` its vectors live in. Ingest never read that stamp, so
    // adding a document after the active embedder changed wrote foreign-space
    // vectors into the same collection: two embedding spaces inside one index, no
    // error, silently mis-ranked KNN. The documented remedy ("reindex the
    // collection") could not fix it either -- reingest is per-document and nothing
    // re-stamps, so a reindex just rewrote each document in whatever space was
    // active at that moment.
    //
    // Conforming to the stamp removes the mixing at the source: a collection's
    // vectors stay in one space for its whole life, whatever the user later picks
    // as their default.
    //
    // An unresolvable stamp FAILS the document. It must not degrade to the active
    // embedder: the query side re-targets on the same stamp, so writing active-space
    // vectors under a stamp claiming otherwise makes the two sides disagree
    // permanently — three of four catalogue embedders are 384-dim, so nothing errors,
    // the vector leg is silently wrong or dead, and there is no collection-level
    // reindex to repair it. A visible failure carrying the offending id is
    // recoverable; a collection that keeps reporting successful ingests while its
    // vectors are unqueryable is not.
    const stamped = collection.embedderId
    const embedderId = stamped && getEmbedder(stamped) ? stamped : null
    if (stamped && !embedderId) {
      this.failDoc(
        jobId,
        doc,
        displayName,
        startedAt,
        `collection is stamped with unknown embedder "${stamped}" — this build's catalogue cannot resolve it, so its vectors cannot be written or queried`
      )
      return
    }
    let vectors: Float32Array[]
    try {
      // Breadcrumb-embed (P2): prepend the chunk's heading path to the EMBED
      // input so a section embeds WITH its structural context (docling
      // `contextualize()` / Smart Connections). The stored `c.text` (and thus
      // display + lexical FTS) is left untouched — the prefix is embed-only.
      const texts = chunks.map((c) => (c.headingPath ? `${c.headingPath}\n${c.text}` : c.text))
      // 'none', not 'passage': the E5 instruction prefixes are NOT applied on the
      // library leg. Existing collections' passages were stored without them and
      // there is no collection-level reindex to re-embed those, so prefixing new
      // passages would put two conventions in one index -- and prefixing queries
      // against unprefixed passages is the asymmetry E5 handles worst. Symmetric
      // and slightly under-tuned beats asymmetric and mis-ranked. (The brain's own
      // notes index is separate and DOES prefix, on both sides. See catalog.ts.)
      vectors = embedderId
        ? await this.deps.embeddings.embedWith(embedderId, texts, 'none', signal)
        : await this.deps.embeddings.embed(texts, signal)
    } catch (err) {
      this.failDoc(jobId, doc, displayName, startedAt, (err as Error)?.message ?? String(err))
      return
    }
    ...
    // P1 bridge — hand the ready doc (with full text) to the brain so it
    // becomes a graph node. Best-effort: a bridge failure never fails ingest.
    try {
      this.deps.onDocumentReady?.({
        documentId: doc.id,
        collectionId,
        displayName,
        sourcePath: file.path,
        mime: loaded.mime,
        text: loaded.kind === 'paged' ? loaded.pages.map((p) => p.text).join('\n\n') : loaded.text
      })
    } catch (err) {
      console.warn('[rag-ingest] onDocumentReady bridge failed:', (err as Error)?.message)
    }
  }
