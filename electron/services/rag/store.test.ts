...
// ──────────────────── getDocumentText (P3b viewer) ────────────────────

describe('getDocumentText', () => {
  const chunk = (documentId: string, chunkIndex: number, text: string) => ({
    documentId,
    collectionId: 'c',
    chunkIndex,
    startOffset: 0,
  ...
  it('concatenates a document\'s chunks in chunk order, scoped to that doc', () => {
    // Insert out of order + a sibling doc; getDocumentText must sort by index
    // and only return the requested document's text.
    insertChunks([
      chunk('gdt-a', 1, 'second'),
      chunk('gdt-a', 0, 'first'),
      chunk('gdt-b', 0, 'other doc')
    ])
    expect(getDocumentText('gdt-a')).toBe('first\n\nsecond')
    expect(getDocumentText('gdt-b')).toBe('other doc')
  })

  it('returns empty string for an unknown document', () => {
    expect(getDocumentText('does-not-exist')).toBe('')
  })
})
