...
//   </retrieved_context>
//
//   Instruction: attribute each claim to its source BY NAME. If no source
...
export function buildContext(input: ContextBuildInput): ContextBuildOutput {
  ...
  const body = accepted
    ...
    .join('\n')
  // Attribution is asked for BY NAME, not as a bracketed source id.
  //
  // Why: `[1]` is only meaningful next to the id→source table, and no surface
  // in the transcript shows one. The renderer halves that would have shown it
  // (src/lib/citation-parser.ts, components/chat/{CitationChip,SourcePreviewPane})
  // exist and are unit-tested but have zero production importers, and `Message`
  // carries no sourceMap field — so the sourceMap this function returns dies at
  // the chat handler (ipc/chat.ts reads only `context.block`). That made the gap
  // invisible from either end: the prompt half looked finished, the render half
  // looked finished, and the missing wire between them showed up only as
  // unexplained `[1]` noise in every retrieval answer.
  //
  // The source NAME is already in the block (`<source name="...">`), so naming it
  // needs no transport and is legible on its own. Restore the bracket form only
  // together with the sourceMap→Message→chip wiring (LAMPREY_RAG_ROSTER R12).
  const instruction = input.citationRequired
    ? 'Attribute each claim to the source it came from by name, e.g. Q3-planning.md. If NO source supports a claim, you MUST say "No source supports an answer to this." rather than answering from prior knowledge.'
    : 'Attribute each claim to the source it came from by name, e.g. Q3-planning.md. If no source supports a claim, say so explicitly.'
