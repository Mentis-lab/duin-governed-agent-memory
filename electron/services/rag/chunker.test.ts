...
describe('recursive splitter — 5000-char prose input', () => {
  ...
  it('produces multiple chunks within the size budget, indices are sequential', () => {
    ...
    // Upper bound accommodates real overlap: with chunkOverlap working, windows
    // advance by less than chunkSize, so more chunks cover the same text. (The old
    // bound of 10 implicitly assumed the overlap-drop bug where windows didn't
    // rewind.)
  ...
  it('covers the full input — no text dropped at overlap boundaries', () => {
    const text = makeProse(5000)
    const result = chunk({ text, sourceKind: 'paste' }, DEFAULT_OPTS)
    // Walk chunks in offset order; the union of their [startOffset, endOffset]
    // ranges must span the whole input with no gap. This is the property the
    // overlap-window bug violated (it skipped the piece that should start the
    // next window, dropping a run of text from the index entirely).
    const ranges = result
      .map((c) => [c.startOffset, c.endOffset] as [number, number])
      .sort((a, b) => a[0] - b[0])
    let covered = 0
    for (const [s, e] of ranges) {
      expect(s).toBeLessThanOrEqual(covered) // no gap before this chunk
