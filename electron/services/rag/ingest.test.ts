...
const fakeEmbeddings: EmbeddingsLike = {
  // The stubs delegate embedWith -> embed: the orchestrator's contract is
  // "one vector per chunk", and the space id only decides WHICH model produces
  // them. The collections below are stamped, so this IS the path under test.
...
describe("IngestManager — embeds in the COLLECTION’s space", () => {
  it("names the collection's embedder_id, not whatever is process-active", async () => {
    // rag_chunk_vec is ONE physical index shared by every collection. Ingest used
    // to embed with the process-global active embedder, so adding a document after
    // the user changed embedders wrote foreign-space vectors into a collection —
    // two spaces in one index, no error, silently mis-ranked KNN.
    const collection = createCollection({
      name: 'Spaces',
      embedderId: 'bge-small-en-v1.5'
    })
    const seen: string[] = []
    const spyEmbeddings: EmbeddingsLike = {
      embed: (texts) => fakeEmbeddings.embed(texts),
      embedWith: (id, texts) => {
        seen.push(id)
        return fakeEmbeddings.embed(texts)
      }
    }
    const mgr = new IngestManager({ embeddings: spyEmbeddings })
    mgr.submit(collection.id, [{ path: FIXTURE_MD, name: 'sample.md' }])
    await waitFor(() => listDocuments(collection.id).find((d) => d.status === 'ready') ?? null)
    expect(seen).toEqual(['bge-small-en-v1.5'])
  })
})
...
describe('IngestManager — failure paths', () => {
  ...
  it('an embedding failure rolls back any partial chunks (count returns to 0)', async () => {
    ...
    const failingEmbeddings: EmbeddingsLike = {
      ...
      embedWith: () => Promise.reject(new Error('embeddings down'))
    }
  ...
  it('a vector-count mismatch fails the doc with a clear message', async () => {
    ...
    const wrongCountEmbeddings: EmbeddingsLike = {
      ...
      embedWith: () => Promise.resolve([new Float32Array(384)])
    }
