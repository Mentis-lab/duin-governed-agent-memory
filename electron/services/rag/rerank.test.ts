...
import { rerank, resolveRerankMode, DEFAULT_RERANK_MODE } from './rerank'
...
describe('resolveRerankMode (item 6 — rerank on by default)', () => {
  it('defaults to local-cross-encoder when unset/empty', () => {
    expect(resolveRerankMode(undefined)).toBe('local-cross-encoder')
    expect(resolveRerankMode(null)).toBe('local-cross-encoder')
    expect(resolveRerankMode({})).toBe('local-cross-encoder')
    expect(DEFAULT_RERANK_MODE).toBe('local-cross-encoder')
  })
  it('honors explicit off (the opt-out escape hatch) + llm + local-cross-encoder', () => {
    expect(resolveRerankMode({ rerankMode: 'off' })).toBe('off')
    expect(resolveRerankMode({ rerankMode: 'llm' })).toBe('llm')
    expect(resolveRerankMode({ rerankMode: 'local-cross-encoder' })).toBe('local-cross-encoder')
  })
  it('falls back to the default on an unknown value', () => {
    expect(resolveRerankMode({ rerankMode: 'garbage' })).toBe('local-cross-encoder')
  })
})
