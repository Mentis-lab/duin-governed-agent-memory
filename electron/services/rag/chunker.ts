...
function windowPieces(
  ...
  while (i < pieces.length) {
    ...
    // Create overlap: re-include the trailing ~chunkOverlap chars of the window
    // just emitted (pieces[windowStart..j-1]) in the next window. Only in the
    // normal path where j advanced past windowStart — never in the too-big
    // char-split path (which must re-process the shrunk pieces[i]).
    if (i < pieces.length && opts.chunkOverlap > 0 && j > windowStart) {
      let overlap = 0
      let k = j - 1
      // Bound the walk to this window: overlap is drawn only from pieces we
      // just emitted, never before windowStart.
      while (k > windowStart && overlap < opts.chunkOverlap) {
        overlap += pieces[k].text.length
        k--
      }
      const overlapStart = k + 1
      // Rewind only if it makes forward progress past windowStart AND stays
      // before j — so the next window re-includes [overlapStart..j-1] as overlap
      // and then continues from j. This never skips piece[j] (the old bug set
      // i = j+1) and never loops on the same window. A single-piece window keeps
      // i = j (no self-overlap).
      if (overlapStart > windowStart && overlapStart < j) {
        i = overlapStart
      }
    }
  }
