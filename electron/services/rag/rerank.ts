...
/** Default when settings leave rag.rerankMode unset — item 6: rerank ON by default. The bundled
 *  bge cross-encoder is best-effort, so BOTH pipelines get cross-encoder precision with no opt-in;
 *  operators opt OUT via Settings→RAG (rag.rerankMode:'off'). ONE resolver, both pipelines. */
export const DEFAULT_RERANK_MODE: RerankMode = 'local-cross-encoder'
export function resolveRerankMode(cfg?: { rerankMode?: string } | null): RerankMode {
  const m = cfg?.rerankMode
  return m === 'off' || m === 'local-cross-encoder' || m === 'llm' ? m : DEFAULT_RERANK_MODE
}
...
export async function rerank(
  ...
  try {
    if (input.mode === 'local-cross-encoder') {
      ...
      // Stable sort by cross score desc; equal scores keep input order, so a
      // fused-ordered pool preserves the bi-encoder tiebreak (fuse-not-replace).
      indexed.sort((a, b) => b.s - a.s)
      // Attach the calibrated cross score so callers can threshold on it (the
      // relevance floor) — it's the first score commensurable across queries.
      reranked = indexed.map((x) => ({ ...x.c, scores: { ...x.c.scores, cross: x.s } }))
      ok = true
