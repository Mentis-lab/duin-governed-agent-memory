...
describe('buildContext', () => {
  ...
  it('emits the <retrieved_context> envelope with source tags', () => {
    ...
    // Retargeted, not dropped: this used to pin /cite sources by id/i. The block
    // still closes with an attribution instruction — it now asks for the source
    // NAME instead of its bracketed id, because nothing in the transcript
    // resolves `[1]` back to a source (see the regression test below).
    expect(out.block).toMatch(/attribute each claim to the source it came from by name/i)
  })
  ...
  // Regression: the instruction used to read "Cite sources by id in square
  // brackets, e.g. [1] or [1, 2]." Models complied, and the transcript rendered
  // the markers as literal `[1]` text — parseCitations / CitationChip have no
  // production caller and no sourceMap ever reaches the renderer, so there is no
  // id→source table anywhere in the UI to make a bare number mean anything.
  // Only ask for a citation form the transcript can actually resolve; the source
  // NAME is already in the block, so it needs no transport.
  it('never asks the model for bracketed numeric citation markers', () => {
    for (const citationRequired of [false, true]) {
      const out = buildContext({
        chunks: [makeChunk({ chunkId: 'A' })],
        citationRequired,
        maxTokens: 1000
      })
      expect(out.block).not.toMatch(/\[\d+(?:\s*,\s*\d+)*\]/)
      expect(out.block).toMatch(/by name/i)
    }
  })
