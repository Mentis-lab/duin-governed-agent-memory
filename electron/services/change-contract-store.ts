...
function getStoreDb(): ReturnType<typeof getDb> | null {
  ...
  try {
    ...
    activateFallback(friendly(err, 'unknown'))
    return null
  }
}
...
// Degrade-to-memory must also cover DB-resident contracts. A contract created
// while the DB was healthy lives only in `change_contracts`, never in
// `memoryContracts` (insertDb mirrors to memory ONLY on its own DB-write
// failure). So when a *later* write fails — read-only persistence mode
// (persistence:setReadOnlyMode), SQLITE_FULL/IOERR, or a post-busy_timeout
// SQLITE_BUSY — the row exists in the DB but not in memory. The callers reach
// this only after getChangeContract() already returned the current row (the
// SELECT succeeded), so `updated` is a complete, freshly-derived contract:
// upsert it. A findIndex-only `updateMemoryContract` threw a misleading
// "not found" for a row we had just read, silently dropping the close/waive/
// update. Existence of a genuinely-missing id is still guarded upstream, where
// getChangeContract() returns null and the caller throws before we get here.
function upsertMemoryContract(contract: ChangeContract): ChangeContract {
  const idx = memoryContracts.findIndex((c) => c.id === contract.id)
  if (idx < 0) {
    memoryContracts.push(cloneContract(contract))
  } else {
    memoryContracts[idx] = cloneContract(contract)
  }
  return cloneContract(contract)
}
...
export function updateChangeContract(
  ...
  const db = getStoreDb()
  if (!db) return upsertMemoryContract(updated)
  try {
    db.prepare(
    ...
    activateFallback(friendly(err, 'unknown'))
    return upsertMemoryContract(updated)
  }
}
...
export function waiveChangeContract(input: {
  ...
  } catch (e) { console.debug('[change-contract-store] best-effort:', messageOf(e)) }
