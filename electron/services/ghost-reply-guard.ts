...
export function isUserAbortError(err: { name?: string; message?: string } | null | undefined): boolean {
  ...
  // NOTE: `err` is a structural {name?, message?} shape (not necessarily an
  // Error), so read `.message` directly — messageOf() would String()-ify a
  // plain object to "[object Object]" and miss abort-shaped messages.
