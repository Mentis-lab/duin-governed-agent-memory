...
function makeBaselineDb(): Database {
  const db = new BetterSqlite3(':memory:')
  db.pragma('foreign_keys = ON')
  // created_at + projects/automations/goals are part of the app's base schema
  // (schema-init.ts), which several migrations assume exists before they run.
  // Shared with db-migrations.test.ts so the two registry guards cannot drift.
...
describe('redactProofText', () => {
  ...
  it('redacts JSON-quoted keys (the closing key-quote must not abort the match)', () => {
    // Before the fix these returned the input UNCHANGED: the quote between the
    // key name and the `:` broke the match, so JSON-shaped secrets leaked.
    const spaced = redactProofText('"api_key": "sk-live-123"')
    expect(spaced).not.toContain('sk-live-123')
    expect(spaced).toContain('[redacted]')

    const compact = redactProofText('{"access_token":"eyJabc.def.ghi"}')
    expect(compact).not.toContain('eyJabc.def.ghi')
    expect(compact).toContain('[redacted]')
  })
})
