...
import { assertGoalTransitionAuthorized } from './goal-transition-authority'
...
// conversation and survives DUIN restarts. If persistence is unavailable
...
//
// Operational-goal lifecycle (migration v32): a goal is a small state machine —
// open → active → (paused|blocked) → active → completed|aborted — with token/time
// budgets, an active-time accumulator, and a last-actor/transition-reason audit.
// The legacy 4-value `status` is kept in lockstep (legacyStatus) so pre-lifecycle
// callers (native-dev-tool-pack, PlanGoalSettings) keep working unchanged.
//
...
/** Auto-block a goal that has exhausted its token or time budget. Returns true when
 *  the block was applied (a SYSTEM transition — it wins over the caller's actor). */
...
// ─────────────────────── change contract, joined to the goal ───────────────────────
// UB-4 excised WC-3's *implicit* change-contract synthesis from the chat tool loop
// (electron/ipc/chat.ts) along with the M5 proof gate. What it did NOT excise is the
// `plan_goal` contract source — but that source was never wired to anything either:
// `createChangeContractFromGoal` had zero callers, so `change_contracts` never
// received a row in production and every write in the subsystem (IPC handlers,
// preload bridge, DB table, store) was reachable only from tests.
//
// The damage was on the READ side, and it was invisible precisely because every layer
// looked present and correct: workspace_context's `proof.activeContract`,
// verify_workspace's `contractId` receipt stamp, and the after-action report's
// `proof.activeContracts` all reported null / undefined / [] forever — a value that
// is indistinguishable from the healthy "no contract is currently needed" instead of
// "this mechanism has never fired and cannot". An operator (or a reviewing agent)
// reading `mutatingTurnsRequireFreshProof: true` next to `activeContract: null`
// reasonably concludes nothing is pending.
//
// A goal IS the unit of intended change in DUIN's current shape, so the goal lifecycle
// owns the contract: opened with the goal, closed when the goal reaches a terminal
// state. The contract id is derived from the goal id so the two stay joined without a
// second mapping table or a new column.
function contractIdForGoal(goalId: string): string {
  return `ctr_goal_${goalId}`
}

function openGoalContract(conversationId: string | undefined, goal: Goal): void {
  try {
    createChangeContractFromGoal(keyOf(conversationId), goal, {
      id: contractIdForGoal(goal.id)
    })
  } catch (e) {
    // Best effort: a change contract is audit metadata. It must never be the reason a
    // goal fails to open — the store already degrades to memory on its own.
    console.debug('[plan-goal-store] change contract not opened:', messageOf(e))
  }
}

/** Idempotent: a second terminal-ish call (e.g. `edit` on an already-completed goal)
 *  sees a non-active contract and does nothing. */
function closeGoalContract(goalId: string): void {
  try {
    const contract = getChangeContract(contractIdForGoal(goalId))
    if (contract?.status === 'active') closeChangeContract(contract.id)
  } catch (e) {
    console.debug('[plan-goal-store] change contract not closed:', messageOf(e))
  }
}
...
export function createGoal(
  ...
  openGoalContract(conversationId, goal)
  return publicGoal(goal, now)
}

/**
 * The single mutation entry point for a goal's lifecycle. Every model/user/system
 * verb (start / pause / resume / block / complete / abort / clear / edit /
 * record_usage) flows through here so the state-machine invariants + audit are in
 * one place.
 *
 * DUIN ADAPTATION: a MODEL actor's TERMINAL transition (abort / clear / complete) is
 * routed through the ANS capability-ledger rung gate (goal-transition-authority)
 * instead of upstream's static "model cannot abort" rule. user/system bypass.
 */
export function transitionGoal(
  ...
  // Terminal-transition authority: model actors must have EARNED the ANS rung; a
  // user/system actor is the authority and bypasses. Throws on !authorized.
  assertGoalTransitionAuthorized(input.action, input.actor)
  ...
  if (input.action === 'edit') {
    // Model actors (fleet principals via /exec) may not rewrite history on a
    // terminal goal — a lease can outlive an operator-side completion, and
    // "edit the record of what was done" is not a fleet privilege. The
    // operator (user/system actors) keeps the affordance.
    if (input.actor === 'model') assertNonTerminal(goal, input.action)
    ...
    if (goal.lifecycleStatus !== 'paused' && goal.lifecycleStatus !== 'blocked') {
      throw new Error(
        `update_goal: resume requires paused or blocked status, got ${goal.lifecycleStatus}.`
      )
    }
    goal.lifecycleStatus = 'active'
    goal.activeSince = now
...
/** Bind a goal to a loop it owns + record the goal's loop-ceiling overrides. Called
 *  by the goal-automation-loop-bridge (a SYSTEM actor); a goal owns at most one loop. */
...
/**
 * Legacy status-patch entry point (native-dev-tool-pack update_goal tool). Maps the
 * old 4-value status onto the lifecycle machine so the model's coarse tool still
 * works. Defaults the actor to 'model' — so a model-initiated status='abandoned'
 * (→ abort, terminal) is subject to the ANS authority gate via transitionGoal.
 */
...
/** Fire the per-goal teardown side-effects before the goal rows are dropped.
 *
 * Deleting goal state directly (as the clear paths do) skips everything transitionGoal's
 * 'clear' path fires, so each side-effect has to be repeated here:
 *  - the loop stop, or a running loop is orphaned (its loop-store entry left
 *    status:'running', no stopReason) with no goal left to govern it — the
 *    runaway-not-killable gap. Firing the SAME 'clear' transition (abortLoopIteration +
 *    updateLoop status:'stopped', stopReason:'goal-cleared', via the installed handler)
 *    closes it. No-op when the loop bridge isn't installed (default-off).
 *  - the change-contract close, or the contract stays 'active' forever for a goal that
 *    no longer exists, and workspace_context / verify_workspace / the after-action report
 *    keep reporting a pending contract nothing can ever close.
 */
function tearDownGoals(conversationId: string | undefined, goals: Iterable<Goal>): void {
  for (const goal of goals) {
    if (goal.loopId) applyGoalLoopTransition(conversationId, publicGoal(goal), 'clear')
    closeGoalContract(goal.id)
  }
}
...
export function clearConversationState(conversationId: string | undefined): void {
  const key = keyOf(conversationId)
  // getState hydrates from persistence, so this also tears down goals that were
  // never loaded into the session cache.
  tearDownGoals(conversationId, getState(conversationId).goals.values())
  state.delete(key)
...
export function clearAllState(): void {
  // getAllPlanGoalState reads through the write-through persistence layer, so it
  // reflects every conversation with a goal — including ones not in the cache.
  for (const conv of getAllPlanGoalState()) {
    tearDownGoals(conv.conversationId, conv.goals)
  }
  state.clear()
  clearAllPlanGoalState()
}
