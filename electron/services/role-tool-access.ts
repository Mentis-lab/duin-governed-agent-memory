...
const PLANNER_TOOLS = new Set([
  ...
  'ask_user_question',
  'create_document',
  // propose_edit is non-mutating (it only shows a review card; nothing is
  // written until the user clicks Apply), so a Planner may surface a proposed
  // change for the user to approve without breaching read-only role scope.
  'propose_edit',
  'memory_add'
])
...
const REVIEWER_TOOLS = new Set([
  ...
  'frontend_qa',
  'create_document',
  // See PLANNER_TOOLS: propose_edit is non-mutating (review-card only), so a
  // Reviewer may propose a change for the user to approve.
  'propose_edit',
  'get_goal'
])
