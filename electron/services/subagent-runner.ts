...
export function resolveAllowedTools(
  ...
  if (result === null) {
    // Every layer said '*'. If we know the parent's tools, narrow to those.
    // If we DON'T (deps.parentTools is unset — every production caller today:
    // ipc/workflows.ts buildForkDeps() and multi-agent-run-tool.ts both omit
    // it), there is nothing to narrow against, so '*' must survive as '*'
    // rather than collapse to zero tools. Returning [] here silently starved
    // every '*'-typed agent (the built-in "general" default type, and any
    // vault-agent .md declaring allowedTools: '*') of every tool — the type
    // said "no narrowing", the plumbing turned that into "narrow to nothing".
    // Safe to widen: the caller (agenticForkRunner) already has a dedicated,
    // tested '*' branch that offers the full registry, and unattended
    // execution is still gated downstream by the CAP floor in tool-exec.ts
    // (capFloorForDescriptor) regardless of what's merely offered here.
...
/** Abort every in-flight fork — used on app quit so no subagent keeps making
 *  model calls / running tools (incl. file writes) detached from a dead app. */
export function abortAllLive(reason = 'quit'): void {
  for (const h of liveHandles.values()) {
    try {
      h.abort(reason)
    } catch (e) { console.debug('[subagent-runner] already settled:', messageOf(e)) }
  }
}
...
export function forkAgent<T = string | Record<string, unknown>>(
  ...
  const validate = (): { type: SubagentTypeDef; allowedTools: AllowedTools } => {
  ...
  let validated: { type: SubagentTypeDef; allowedTools: AllowedTools }
  try {
    validated = validate()
  } catch (err) {
