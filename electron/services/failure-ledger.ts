...
export type FailureLedgerKind =
  ...
  | 'stale_green_attempt'
  | 'user_reported'
  // Runtime failures bridged from the event log (automation/model/tool `*.failed`).
  // The proof-gate kinds above are only written when proof gates run; this kind
  // captures the systematic runtime failures that otherwise live only as events.
  | 'runtime_failed'
...
export interface RecordFailureInput {
  ...
  replaySeed?: ReplaySeed
  /** AUTHORITATIVE occurrence count (event-sourced sync). When set, the ledger's
   *  count is set to this value instead of the default +1-per-call — so a
   *  re-run of an event→ledger bridge is idempotent (count tracks the events,
   *  not the number of sync passes). Omit for point-of-failure recording. */
  count?: number
  /** When the failure actually last happened, for a sync that knows the real event time.
   *  Without it, an idempotent re-sync had nothing to write but `now`, so every historical
   *  fingerprint it touched looked current — the count was idempotent, the timestamp was not. */
  lastSeenAt?: number
}
...
function parseReplaySeed(json: string): ReplaySeed {
  ...
  } catch (e) { console.debug('[failure-ledger] fall through:', messageOf(e)) }
  return {}
}
...
export function recordFailure(input: RecordFailureInput, db?: Database): FailureLedgerRecord {
  ...
  if (existing) {
    // Authoritative count (event-sourced sync) sets the value; otherwise the
    // point-of-failure default increments by one.
    const newCount = input.count ?? existing.count + 1

    // last_seen_at means "when this failure last HAPPENED", not "when a pass last looked".
    // The reconciliation sync re-asserts an all-time count for every historical fingerprint
    // on every run, and stamping `now` there made a failure that stopped weeks ago read as
    // current — which is exactly how a July incident stayed the top 'runaway' alarm through
    // late August while the genuinely live failures sat underneath it. Rules:
    //   • caller supplied the real event time  -> trust it
    //   • authoritative count did NOT increase -> nothing recurred, keep the old timestamp
    //   • otherwise (a real new failure)       -> now
    const recurred = input.count === undefined || newCount > existing.count
    const lastSeen = input.lastSeenAt ?? (recurred ? ts : existing.last_seen_at)
    d.prepare(`
      UPDATE failure_ledger
        SET count = ?, last_seen_at = ?, updated_at = ?,
            message = CASE WHEN ? != '' THEN ? ELSE message END
        WHERE fingerprint = ?
    `).run(newCount, lastSeen, ts, input.message, input.message, fingerprint)
    ...
    } catch (e) { console.debug('[failure-ledger] best-effort:', messageOf(e)) }
    return rec
  }
  ...
  `).run(
    ...
    input.count ?? 1, JSON.stringify(replaySeed), ts, ts, ts, ts
  )
