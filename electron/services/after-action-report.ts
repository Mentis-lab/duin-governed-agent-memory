...
export function buildAfterActionReport(conversationId: string): AfterActionReport {
  ...
  const events = listTimeline({ conversationId, limit: 1000, window: 'newest' })
    // ...then back to chronological for the report. listTimeline hard-coded
    // order:'asc', so `limit: 1000` selected the OLDEST 1000 events — on any
    // conversation past that length the after-action report described how the
    // conversation BEGAN and never saw the work it was written to summarise.
    .slice()
    .reverse()
  ...
  if (emptyAssistantMessages.length > 0) {
    causes.push({
      ...
      detail:
        ...
        'If those turns only emitted reasoning or tool calls, the chat can look idle even while DUIN is working.'
    })
  }
