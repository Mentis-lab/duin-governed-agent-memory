...
function initCoreDomainTables(db: Database.Database): void {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS memory_index (
      ...
      created_at INTEGER NOT NULL,
      updated_at INTEGER NOT NULL,
      -- Provenance. The type column is what the memory is ABOUT; source is where it
      -- CAME FROM, and only the latter can separate a fact the operator stated from
      -- one DUIN inferred. Defaults to 'unknown' so pre-provenance rows are honestly
      -- unlabelled rather than back-guessed. Existing DBs get this via
      -- db-migrations v45 with the identical DDL -- the two install paths must not
      -- drift, so the CHECK is present in both.
    ...
    -- NOTE: idx_memory_index_source is deliberately NOT created here. It indexes
    -- the source column, which this segment supplies only to FRESH databases via
    -- the CREATE TABLE above. On an EXISTING database that CREATE TABLE is a
    -- no-op, the column does not exist yet, and this statement threw
    -- (no such column: source) BEFORE runMigrations could add it. getDb() had
    -- already published its handle, so every later call short-circuited and
    -- skipped both initLegacySchema AND runMigrations permanently: the ledger
    -- froze at v44 and no migration could ever land again. Self-blocking, and
    -- silent -- the app works and integrity_check passes on a table missing a
    -- column.
    -- The index belongs with the column. db-migrations v45 creates both and runs
    -- after this, so fresh and migrated installs converge there instead of here.
    -- Do not add a statement here that depends on a migration-supplied column;
    -- db-migration-unblock.test.ts keeps a negative control for exactly this.
...
function applyLegacyColumnMigrationsBatchA(db: Database.Database): void {
  ...
  // v28 — task lifecycle `close`/`restore` timestamp (NULL = open). Fresh-install
  // add; existing DBs also get it via db-migrations v28 (both idempotent).
...
function applyLegacyColumnMigrationsBatchB(db: Database.Database): void {
  ...
  // Vision attachments as a JSON array of OpenAI content parts. `content` stays the
  // TEXT PROJECTION of the turn (what search, export and every bubble render), so
  // legacy rows and every existing reader are untouched; this column carries only
  // the parts that have no text form. NULL ⇒ a plain text turn, i.e. every row
  // written before 2026-07-28.
  safeAddColumn(db, 'messages', 'content_parts TEXT')
}
...
function initGithubRagSessionsSnip(db: Database.Database): void {
  db.exec(`
    ...
    -- v28 — task lifecycle close/restore.
