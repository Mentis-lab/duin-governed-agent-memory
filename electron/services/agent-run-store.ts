...
/** At startup, any run still marked 'running' is a crash orphan — the process
 *  died mid-run, so the row would otherwise show as a forever-live task that can
 *  never finish. Mark all such rows aborted. Call once during boot. */
export function reconcileStaleRuns(): void {
  ...
  } catch (e) { console.debug('[agent-run-store] best-effort  a failed reconcile must never block boot:', messageOf(e)) }
}
