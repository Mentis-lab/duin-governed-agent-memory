...
describe.skipIf(!HAS_NATIVE_SQLITE)('database checkpoint (PS2)', () => {
  ...
  it('returns ok:true and reports pages moved when WAL has content', () => {
    ...
    // NOTE: SQLite 3.53 (bundled with better-sqlite3 13) reports log/checkpointed = 0 for
    // wal_checkpoint(TRUNCATE) even though it fully truncates the WAL — a behavior change from
    // 3.45. The page counts are telemetry-only (nothing gates on them); the checkpoint's real
    // effect is proven by ok:true + the WAL shrinking to 0 below. So we assert >= 0, not > 0.
    expect(result.pagesInWal).toBeGreaterThanOrEqual(0)
    expect(result.pagesCheckpointed).toBeGreaterThanOrEqual(0)
    expect(result.durationMs).toBeGreaterThanOrEqual(0)

    // The load-bearing assertion: TRUNCATE actually reclaimed the WAL to zero length.
