...
describe('evaluateMetaLiteral — pure-literal enforcement', () => {
  ...
  it('accepts an ellipsis in prose without mistaking it for a spread', () => {
    const meta = evaluateMetaLiteral(
      '{ name: "x", description: "Summarize a long thread..." }'
    )
    expect(meta.description).toBe('Summarize a long thread...')
  })

  it('accepts a backtick used for inline-code styling without mistaking it for a template literal', () => {
    const meta = evaluateMetaLiteral(
      '{ name: "x", description: "Run `npm test` first" }'
    )
    expect(meta.description).toBe('Run `npm test` first')
  })

  it('still rejects a real spread even when a quoted string elsewhere contains "..."', () => {
    expect(() =>
      evaluateMetaLiteral('{ ...defaults, name: "x", description: "loading..." }')
    ).toThrow(/spread/)
  })

  it('still rejects a real template literal even when a quoted string elsewhere contains a backtick', () => {
    expect(() =>
      evaluateMetaLiteral(
        '{ name: `still-bad`, description: "uses `backticks` for styling" }'
      )
    ).toThrow(/backticks/)
  })
