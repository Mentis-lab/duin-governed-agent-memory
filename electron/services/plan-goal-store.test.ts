...
import type { GoalAction } from './plan-goal-store'

beforeEach(() => {
  __resetPlanGoalStore()
  setGoalLoopTransitionHandler(null)
})
...
describe('persistence — survives a simulated restart', () => {
  ...
  // Runaway-not-killable regression: the operator's Clear must stop a goal-owned
  // background loop, not just delete the goal row. clearConversationState /
  // clearAllState previously deleted state without firing the goal→loop 'clear'
  // side-effect, orphaning a running loop with no goal left to govern it.
  it('clearConversationState fires the goal→loop clear side-effect for loop-owning goals', () => {
    const fired: Array<{ loopId: string | null; action: GoalAction }> = []
    setGoalLoopTransitionHandler((_conv, goal, action) => {
      fired.push({ loopId: goal.loopId, action })
    })
    const goal = createGoal(CONV, { title: 'g' })
    bindGoalLoop(CONV, goal.id, {
      loopId: 'loop-1',
      maxIterations: null,
      maxWallclockMs: null,
      tokenBudget: null
    ...
    expect(fired).toContainEqual({ loopId: 'loop-1', action: 'clear' })
  })

  it('clearAllState fires the goal→loop clear side-effect for loop-owning goals', () => {
    const fired: Array<{ loopId: string | null; action: GoalAction }> = []
    setGoalLoopTransitionHandler((_conv, goal, action) => {
      fired.push({ loopId: goal.loopId, action })
    })
    const goal = createGoal(OTHER, { title: 'g2' })
    bindGoalLoop(OTHER, goal.id, {
      loopId: 'loop-2',
      maxIterations: null,
      maxWallclockMs: null,
      tokenBudget: null
    ...
    expect(fired).toContainEqual({ loopId: 'loop-2', action: 'clear' })
  })
})

// The change-contract write path had zero production callers: `change_contracts`
// never received a row, so workspace_context's `proof.activeContract`,
// verify_workspace's `contractId` receipt stamp and the after-action report's
// `proof.activeContracts` reported "empty" forever — indistinguishable from
// "no contract needed". These tests pin the goal lifecycle as the real caller.
describe('goal lifecycle ↔ change contract', () => {
  beforeEach(() => {
    __resetChangeContractStore()
  })

  it('opens an active plan_goal change contract when a goal is created', () => {
    const goal = createGoal(CONV, { title: 'Ship the fix', description: 'and prove it' })

    const active = getActiveChangeContract(CONV)
    expect(active).not.toBeNull()
    expect(active?.source).toBe('plan_goal')
    expect(active?.implicit).toBe(false)
    expect(active?.goal).toBe('Ship the fix\n\nand prove it')
    // Joined to the goal by a derived id, so no second mapping table is needed.
    expect(getChangeContract(`ctr_goal_${goal.id}`)?.id).toBe(active?.id)
  })

  it('closes the contract when the goal completes', () => {
    const goal = createGoal(CONV, { title: 'Ship the fix' })
    transitionGoal(CONV, { goalId: goal.id, action: 'start', actor: 'user' })
    transitionGoal(CONV, {
      goalId: goal.id,
      action: 'complete',
      actor: 'user',
    ...
    expect(getActiveChangeContract(CONV)).toBeNull()
    expect(getChangeContract(`ctr_goal_${goal.id}`)?.status).toBe('closed')
  })

  it('closes the contract when the goal is aborted', () => {
    const goal = createGoal(CONV, { title: 'Abandon this' })
    transitionGoal(CONV, { goalId: goal.id, action: 'abort', actor: 'user' })

    expect(getActiveChangeContract(CONV)).toBeNull()
    expect(getChangeContract(`ctr_goal_${goal.id}`)?.status).toBe('closed')
  })

  it('closes the contract when the goal is cleared', () => {
    const goal = createGoal(CONV, { title: 'Delete this' })
    transitionGoal(CONV, { goalId: goal.id, action: 'clear', actor: 'user' })

    expect(getActiveChangeContract(CONV)).toBeNull()
    expect(getChangeContract(`ctr_goal_${goal.id}`)?.status).toBe('closed')
  })

  // The clear paths delete goal rows directly, bypassing transitionGoal — without
  // the same teardown the contract stays 'active' for a goal that no longer exists.
  it('closes the contract when the conversation state is cleared', () => {
    const goal = createGoal(CONV, { title: 'Wiped with the conversation' })
  ...
  it('closes the contract when all plan/goal state is cleared', () => {
    const goal = createGoal(OTHER, { title: 'Wiped with everything' })

    clearAllState()

    expect(getActiveChangeContract(OTHER)).toBeNull()
    expect(getChangeContract(`ctr_goal_${goal.id}`)?.status).toBe('closed')
  })

  it('scopes the contract to the goal conversation', () => {
    createGoal(CONV, { title: 'Mine' })

    expect(getActiveChangeContract(OTHER)).toBeNull()
    expect(listChangeContracts({ conversationId: CONV, status: 'active' })).toHaveLength(1)
  })
})
