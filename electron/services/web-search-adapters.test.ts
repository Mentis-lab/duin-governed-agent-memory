...
describe('image-search capability metadata', () => {
  it('matches which adapters actually implement imageSearch', () => {
    // The point of the flag: Settings warns from it, so it must not drift from
    // the adapters. duckduckgo and wikipedia are the two keyless providers and
    // are exactly the two without an image endpoint.
    for (const p of ALL_WEB_SEARCH_PROVIDERS) {
      const adapter = getWebSearchAdapterById(p.id)
      if (!adapter) continue // needs a key/endpoint not present in this env
      expect(Boolean(adapter.imageSearch)).toBe(p.supportsImages)
    }
  })

  it('reports the keyless providers as image-incapable', () => {
    expect(providerSupportsImages('duckduckgo')).toBe(false)
    expect(providerSupportsImages('wikipedia')).toBe(false)
    expect(IMAGE_CAPABLE_PROVIDERS).not.toContain('duckduckgo')
    expect(IMAGE_CAPABLE_PROVIDERS).not.toContain('wikipedia')
  })

  it('names at least one provider a user can switch to', () => {
    // The error copy and the Settings warning both list these; an empty list
    // would tell the operator to switch to nothing.
    expect(IMAGE_CAPABLE_PROVIDERS.length).toBeGreaterThan(0)
    expect(IMAGE_CAPABLE_PROVIDERS).toContain('brave')
  })
})
